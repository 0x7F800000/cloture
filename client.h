...
__align(16) struct tridecal_t
{
	__import_float_vector_types();
	...
	int				decalsequence;
	//preceding four fields add up to 16 bytes
	#if 0
...
//rtlight_particle_t;
//moved here from r_shadow
//typedef
enum r_shadow_shadowmode_t//r_shadow_shadowmode_e
{
...
__align(16) struct rtlight_t
{
	...
	matrix4x4_t matrix_worldtolight;
		...
		float shadoworigin[3];
		/// culling
		float cullmins[3];
		float cullmaxs[3];
		// rendering properties, updated each time a light is rendered
		// this is rtlight->color undefined* d_lightstylevalue
		float currentcolor[3];
		// reduced light cullbox from GetLightInfo
		float cached_cullmins[3];
		float cached_cullmaxs[3];
...
struct entity_persistent_t
{
	...
	bool trail_allowed; // set to false by teleports, true by update code, prevents bad lerps
};
//entity_persistent_t;
...
// intended velocities
	...
	int sequence;
		...
		bool jump;
		bool crouch;
		bool applied; // if false we're still accumulating a move
		bool predicted; // if true the sequence should be sent as 0
	#else
		struct
		{
			unsigned char
...
// entering a map (and clearing client_state_t)
	...
	bool demoplayback;
	bool demostarting; // set if currently starting a demo, to stop -demo from quitting when switching to another demo
	bool timedemo;
...
typedef struct decal_s
{
	...
	float			relativeorigin[3]; // decal at this location in entity's coordinate space
	float			relativenormal[3]; // decal oriented this way relative to entity's coordinate space
}
decal_t;
...
typedef struct particle_s
{
	...
	float          sortorigin[3]; // sort by this group origin, not particle org
	float          org[3];
	float          vel[3]; // velocity of particle, or orientation of decal, or end point of beam
	float           size;
	float           alpha; // 0-255
...
//using cl_locnode_t = cl_locnode_s;
//cl_locnode_t;
...
static constexpr size_t LATESTFRAMENUMS = 32;
//
...
// of each level and after teleporting.
	...
	bool movement_replay;
	// simulated data (this is valid even if cl.movement is false)
	vec3_t movement_origin;
	vec3_t movement_velocity;
	// whether the replay should allow a jump at the first sequence
	bool movement_replay_canjump;
...
//[515]: added for csqc purposes
	...
	bool csqc_wantsmousemove;
	bool csqc_paused; // vortex: int because could be flags
...
extern void CL_AllocLightFlash (entity_render_t *ent, matrix4x4_t *matrix, float radius, float red, float green, float blue, float decay, float lifetime, int cubemapnum, int style, int shadowenable, float corona, float coronasizescale, float ambientscale, float diffusescale, float specularscale, int flags);
...
extern cvar_t qport;
void CL_KeepaliveMessage(bool readmessages); // call this during loading of large content
...
void CL_ParticleTrail(int effectindex, float pcount, const vec3_t originmins, const vec3_t originmaxs, const vec3_t velocitymins, const vec3_t velocitymaxs, entity_t *ent, int palettecolor, bool spawndlight, bool spawnparticles, float tintmins[4], float tintmaxs[4], float fade);
void CL_ParticleBox(int effectindex, float pcount, const vec3_t originmins, const vec3_t originmaxs, const vec3_t velocitymins, const vec3_t velocitymaxs, entity_t *ent, int palettecolor, bool spawndlight, bool spawnparticles, float tintmins[4], float tintmaxs[4], float fade);
void CL_ParseParticleEffect ();
void CL_ParticleCube (const vec3_t mins, const vec3_t maxs, const vec3_t dir, int count, int colorbase, float gravity, float randomvel);
...
typedef struct r_refdef_view_s
{
	...
	using vectorType = cloture::util::math::vector::vector3D;
		...
		vec3_t up;
	#else
		vectorType origin, forward, left, right, up;
	#endif

	int numfrustumplanes;
	mplane_t frustum[6];
	bool useclipplane;
	bool usecustompvs; // uses r_refdef.viewcache.pvsbits as-is rather than computing it
	mplane_t clipplane;
	float frustum_x, frustum_y;
	#if 0
	vec3_t frustumcorner[4];
	#else
	vectorType frustumcorner[4];
	#endif
...
typedef struct r_refdef_scene_s
{
	...
	bool rtworldshadows;
	bool rtdlight;
	bool rtdlightshadows;
	// whether to call S_ExtraUpdate during render to reduce sound chop
	bool extraupdate;
	bool expandtempentities;
	*/
	struct
	{
		unsigned char
...
using RenderState = r_refdef_t;

extern r_refdef_t r_refdef;

//typedef enum waterlevel_e
enum waterlevel_t : int
{
	WATERLEVEL_NONE,
	WATERLEVEL_WETFEET,
	WATERLEVEL_SWIMMING,
...
void CL_FindNonSolidLocation(const vec3_t in, vec3_t out, float radius);
void CL_RelinkLightFlashes();
void Sbar_ShowFPS();
void Sbar_ShowFPS_Update();
void Host_SaveConfig();
void Host_LoadConfig_f();
