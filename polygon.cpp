...
#if !defined(RESTRICT) 
	#if defined(__clang__) || defined(_MSVC_VER) || defined(__GNUC__)
...
{
	float quadright[3], quadup[3];
	if (fabsf(planenormalz) > fabsf(planenormalx) && fabsf(planenormalz) > fabsf(planenormaly))
	{
		quadup[0] = 1.0f;
		quadup[1] = .0f;
		quadup[2] = .0f;
	...
	float d = -(quadup[0] * planenormalx + quadup[1] * planenormaly + quadup[2] * planenormalz);
...
void PolygonD_QuadForPlane(double *RESTRICT outpoints, double planenormalx, double planenormaly, double planenormalz, double planedist, double quadsize)
{
