...
// wipe the entire cl structure
	...
	ent->render.model = cl.worldmodel = nullptr; // no world model yet
	ent->render.alpha = 1;
...
void CL_ExpandEntities(int num)
{
	...
	if (num >= cl.max_entities)
	{
		...
		for ( size_t i = oldmaxentities; i < cl.max_entities; i++ )
		{
			cl.entities[i].state_baseline = defaultstate;
			cl.entities[i].state_previous = defaultstate;
			cl.entities[i].state_current = defaultstate;
...
===============
*/
static void CL_ModelIndexList_f()
{
	int i;
	dp_model_t *model;
...
===============
*/
static void CL_SoundIndexList_f()
{
	int i = 1;
...
static float CL_LerpPoint()
{
	...
	const float f = (cl.time - cl.mtime[1]) / (cl.mtime[0] - cl.mtime[1]);
...
static void CL_DecayLightFlashes()
{
	int i, oldmax;
	dlight_t *dl;
	float time;
...
static void CL_UpdateNetworkEntity(entity_t *e, int recursionlimit, bool interpolate)
{
	...
	else if (e->state_current.colormap > 0 && e->state_current.colormap <= cl.maxclients && cl.scores != nullptr)
	...
	else
	{
		...
		matrix = reinterpret_cast<const matrix4x4_t*>(&identitymatrix);
	}
	...
	else if (interpolate && e->persistent.lerpdeltatime > 0 && (lerp = (cl.time - e->persistent.lerpstarttime) / e->persistent.lerpdeltatime) < 1 + cl_lerpexcess.value)
	{
		...
		{
			vec3_t f0, u0, f1, u1;
			AngleVectors(e->persistent.oldangles, f0, nullptr, u0);
			AngleVectors(e->persistent.newangles, f1, nullptr, u1);
...
static void CL_UpdateNetworkEntityTrail(entity_t *e)
{
	...
	if (e->render.effects & (EF_BRIGHTFIELD | EF_FLAME | EF_STARDUST))
	{
		...
		if (e->render.effects & EF_FLAME)
			CL_ParticleTrail(EFFECT_EF_FLAME, bound(0, cl.time - cl.oldtime, 0.1), origin, origin, vec3_origin, vec3_origin, nullptr, 0, false, true, nullptr, nullptr, 1);
		if (e->render.effects & EF_STARDUST)
			CL_ParticleTrail(EFFECT_EF_STARDUST, bound(0, cl.time - cl.oldtime, 0.1), origin, origin, vec3_origin, vec3_origin, nullptr, 0, false, true, nullptr, nullptr, 1);
	}
	...
	if (trailtype && e->persistent.trail_allowed)
	{
		...
		CL_ParticleTrail(trailtype, bound(0, cl.time - cl.oldtime, 0.1), e->persistent.trail_origin, origin, vel, vel, e, e->state_current.glowcolor, false, true, nullptr, nullptr, 1);
	}
...
===============
*/
static void CL_UpdateNetworkCollisionEntities()
{
	// start on the entity after the world
	cl.num_brushmodel_entities = 0;
	for (size_t i = cl.maxclients + 1; i < cl.num_entities; i++)
	{
		if (!cl.entities_active[i])
			continue;
...
===============
*/
static void CL_UpdateNetworkEntities()
{
	entity_t *ent;
	int i;
...
static void CL_UpdateViewModel()
{
	entity_t *ent;
	ent = &cl.viewent;
...
static void CL_LinkNetworkEntity(entity_t *RESTRICT e)
{
	effectnameindex_t trailtype;
	vec3_t origin;
	vec3_t dlightcolor;
	...
	if (e->render.effects & (EF_BRIGHTFIELD | EF_DIMLIGHT | EF_BRIGHTLIGHT | EF_RED | EF_BLUE | EF_FLAME | EF_STARDUST))
	{
		...
		if (e->render.effects & EF_FLAME)
			CL_ParticleTrail(EFFECT_EF_FLAME, 1, origin, origin, vec3_origin, vec3_origin, nullptr, 0, true, false, nullptr, nullptr, 1);
		if (e->render.effects & EF_STARDUST)
			CL_ParticleTrail(EFFECT_EF_STARDUST, 1, origin, origin, vec3_origin, vec3_origin, nullptr, 0, true, false, nullptr, nullptr, 1);
	}
	...
	if (e->persistent.muzzleflash > 0 && r_refdef.scene.numlights < MAX_DLIGHTS)
	{
		...
		trace = CL_TraceLine(origin, v2, MOVE_NOMONSTERS, nullptr, SUPERCONTENTS_SOLID | SUPERCONTENTS_SKY, collision_extendmovelength.value, true, false, nullptr, false, false);
		...
		R_RTLight_Update(&r_refdef.scene.templights[r_refdef.scene.numlights], false, &tempmatrix, color, -1, nullptr, true, 0, 0.25, 0, 1, 1, LIGHTFLAG_NORMALMODE | LIGHTFLAG_REALTIMEMODE);
	...
	else if (dlightradius > 0 && (dlightcolor[0] || dlightcolor[1] || dlightcolor[2]) && !(e->render.flags & RENDER_VIEWMODEL) && r_refdef.scene.numlights < MAX_DLIGHTS)
	{
		...
		R_RTLight_Update(&r_refdef.scene.templights[r_refdef.scene.numlights], false, &dlightmatrix, dlightcolor, -1, nullptr, true, 1, 0.25, 0, 1, 1, LIGHTFLAG_NORMALMODE | LIGHTFLAG_REALTIMEMODE);
		r_refdef.scene.lights[r_refdef.scene.numlights] = &r_refdef.scene.templights[r_refdef.scene.numlights];r_refdef.scene.numlights++;
	}
	// do trail light
	if (e->render.flags & RENDER_GLOWTRAIL)
	...
	if (e->state_current.traileffectnum)
		trailtype = (effectnameindex_t)e->state_current.traileffectnum;
	if (trailtype)
		CL_ParticleTrail(trailtype, 1, origin, origin, vec3_origin, vec3_origin, nullptr, e->state_current.glowcolor, true, false, nullptr, nullptr, 1);

	// don't show entities with no modelindex (note: this still shows
	// entities which have a modelindex that resolved to a nullptr model)
...
static void CL_RelinkWorld()
{
	...
	if (ent->render.model && ent->render.model->brush.isq2bsp)
		ent->render.framegroupblend[0].frame = static_cast<int>(cl.time * 2.0f);
}

static void CL_RelinkStaticEntities()
{
	int i;
	entity_t *e;
...
static void CL_RelinkEffects()
{
	...
	for (i = 0, e = cl.effects;i < cl.num_effects;i++, e++)
	{
		...
		const float frame = (cl.time - e->starttime) * e->framerate + e->startframe;
		const int intframe = static_cast<int>(frame);
	...
	for (i = 0, b = cl.beams;i < cl.num_beams;i++, b++)
	{
		...
		if (b->lightning)
		{
			if (cl_beams_lightatend.integer && r_refdef.scene.numlights < MAX_DLIGHTS)
			{
				...
				R_RTLight_Update(&r_refdef.scene.templights[r_refdef.scene.numlights], false, &tempmatrix, dlightcolor, -1, nullptr, true, 1, 0.25, 1, 0, 0, LIGHTFLAG_NORMALMODE | LIGHTFLAG_REALTIMEMODE);
				r_refdef.scene.lights[r_refdef.scene.numlights] = &r_refdef.scene.templights[r_refdef.scene.numlights];r_refdef.scene.numlights++;
			}
...
static void CL_RelinkQWNails()
{
	for (size_t i = 0; i < cl.qw_num_nails; i++)
	{
		vec_t *v = cl.qw_nails[i];

		// if we're drawing effects, get a new temp entity
		// (NewTempEntity adds it to the render entities list for us)
		entity_render_t *entrender = CL_NewTempEntity(0);
		if (entrender == nullptr)
			continue;
...
static void CL_LerpPlayer(float frac)
{
	...
	if(likely(!cls.demoplayback && !cl.fixangle[0]))
		return;
	for (size_t i = 0; i < 3; i++)
	{
...
======================
*/
static void CL_Fog_HeightTexture_f()
{
	if (Cmd_Argc() < 11)
	{
...
====================
*/
static void CL_TimeRefresh_f()
{
	r_refdef.scene.extraupdate = false;

	double timestart = Sys_DirtyTime();
	for (int i = 0; i < 128; i++)
	{
	...
	double timedelta = Sys_DirtyTime() - timestart;

	Con_Printf("%f seconds (%f fps)\n", timedelta, 128/timedelta);
}

static void CL_AreaStats_f()
{
...
cl_locnode_t *CL_Locs_FindNearest(const vec3_t point)
{
	...
	for (loc = cl.locnodes;loc;loc = loc->next)
	{
		...
		vec_t dist = VectorDistance2(nearestpoint, point);
...
{
	cl_locnode_t *loc = CL_Locs_FindNearest(point);
	if (loc)
...
static void CL_Locs_RemoveNearest_f()
{
	cl_locnode_t *loc;
...
static void CL_Locs_Clear_f()
{
	while (cl.locnodes)
		CL_Locs_FreeNode(cl.locnodes);
}

static void CL_Locs_Save_f()
{
	cl_locnode_t *loc;
	qfile_t *outfile;
...
void CL_Init()
{
	...
	cls.levelmempool = Mem_AllocPool("client (per-level memory)", 0, nullptr);
	cls.permanentmempool = Mem_AllocPool("client (long term memory)", 0, nullptr);
