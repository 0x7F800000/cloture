...
matrix4x4_t;
	...
	//using matrix4x4_s = cloture::util::math::matrix::matrix4D;
	//using matrix4x4_t = matrix4x4_s;
	struct matrix4x4_s;
	typedef struct matrix4x4_s matrix4x4_t;
	typedef struct matrix4x4_s : public cloture::util::math::matrix::matrix4D
	{
		inline matrix4x4_s() : matrix4D::matrix4D(){}
		inline matrix4x4_s(const matrix4x4_t& other)
		{
			this->m[0] = other.m[0];
			this->m[1] = other.m[1];
			this->m[2] = other.m[2];
		...
		inline matrix4x4_s(const cloture::util::math::matrix::matrix4D& other)
		{
			this->m[0] = other.m[0];
			this->m[1] = other.m[1];
			this->m[2] = other.m[2];
		...
		inline matrix4x4_s(float m0[4], float m1[4], float m2[4], float m3[4]) : matrix4D::matrix4D(m0, m1, m2, m3)
		{
		}
	}matrix4x4_t;
#endif
extern const cloture::util::math::matrix::matrix4D identitymatrix;
...
// functions for manipulating 4x4 matrices
	...
	#define		Matrix4x4_Copy(out, in)												(out)->copyFrom((in))
	#define		Matrix4x4_CopyRotateOnly(out, in)									(out)->copyRotateOnly((in))
	#define		Matrix4x4_CopyTranslateOnly(out, in)								(out)->copyTranslateOnly((in))
	#define		Matrix4x4_Concat(out, in1, in2)										(out)->concat((in1), (in2))
	#define		Matrix4x4_Transpose(out, in1)										(out)->transpose((in1))
	#define		Matrix4x4_Invert_Full(out, in1)										(out)->invertFull((in1))
	#define		Matrix4x4_Invert_Simple(out, in1)									(out)->invertSimple((in1))
	#define		Matrix4x4_Interpolate(out, in1, in2, frac)							(out)->interpolate((in1), (in2), (frac))
	#define		Matrix4x4_Clear(out)												(out)->clear()
	#define		Matrix4x4_Accumulate(out, in, weight)								(out)->accumulate((in), (weight))
	#define		Matrix4x4_Normalize(out, in1)										(out)->normalize((in1))
	#define		Matrix4x4_Normalize3(out, in1)										(out)->normalize3((in1))
	#define		Matrix4x4_Reflect(out, normalx, normaly, normalz, dist, axisscale)	(out)->reflect((normalx), (normaly), (normalz), (dist), (axisscale))
	#define		Matrix4x4_CreateIdentity(out)										(out)->createIdentity()
	#define		Matrix4x4_CreateTranslate(out, x, y, z)								(out)->createTranslate((x), (y), (z))
	#define		Matrix4x4_Abs(out)													(out)->fabs()
	#define		Matrix4x4_Scale(out, rotatescale, originscale)						(out)->scale((rotatescale), (originscale))
#endif
