...
static const unsigned char *fontfilecache_LoadFile(const char *path, bool quiet, fs_offset_t *filesizepointer)
{
	int i;
	unsigned char *buf;
...
static bool Font_Attach(ft2_font_t *font, ft2_attachment_t *attachment)
{
	ft2_attachment_t *na;
...
static bool Font_LoadFile(const char *name, int _face, ft2_settings_t *settings, ft2_font_t *font);
static bool Font_LoadSize(ft2_font_t *font, float size, bool check_only);
bool Font_LoadFont(const char *name, dp_font_t *dpfnt)
{
	int s, count, i;
...
static void Font_Postprocess_Update(ft2_font_t *fnt, int bpp, int w, int h)
{
	...
	bool need_gauss  = (!pp.buf || pp.blur != fnt->settings->blur || pp.shadowz != fnt->settings->shadowz);
	bool need_circle = (!pp.buf || pp.outline != fnt->settings->outline || pp.shadowx != fnt->settings->shadowx || pp.shadowy != fnt->settings->shadowy);
...
static bool Font_LoadMap(ft2_font_t *font, ft2_font_map_t *mapstart, Uchar _ch, ft2_font_map_t **outmap);
static bool Font_LoadSize(ft2_font_t *font, float size, bool check_only)
{
	int map_index;
...
static bool Font_SetSize(ft2_font_t *font, float w, float h)
{
	if (font->currenth == h &&
...
bool Font_GetKerningForMap(ft2_font_t *font, int map_index, float w, float h, Uchar left, Uchar right, float *outx, float *outy)
{
	ft2_font_map_t *fmap;
...
bool Font_GetKerningForSize(ft2_font_t *font, float w, float h, Uchar left, Uchar right, float *outx, float *outy)
{
...
bool Font_LoadMapForIndex(ft2_font_t *font, int map_index, Uchar _ch, ft2_font_map_t **outmap)
{
	if (map_index < 0 || map_index >= MAX_FONT_SIZES)
		return false;
