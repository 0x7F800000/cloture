...
static void PRVM_MEM_Alloc(prvm_prog_t *prog)
{
	...
	// alloc edicts
	prog->edicts = prog->progs_mempool->alloc<prvm_edict_t>(prog->limit_edicts);
	...
	// set edict pointers
	const size_t maxEdicts = static_cast<size_t>(prog->max_edicts);
	for(size_t i = 0; i < maxEdicts; i++)
	{
...
{
	for (size_t i = 0;i < prog->numfielddefs;i++)
	{
		ddef_t *def = &prog->fielddefs[i];
		if (def->ofs == ofs)
			return def;
	}
	return nullptr;
...
=============
*/
static void PRVM_ED_PrintEdict_f ()
{
	prvm_prog_t *prog;
	int		i;
	const char	*wildcard_fieldname;
...
void PRVM_ED_ParseGlobals (prvm_prog_t *prog, const char *data)
{
	...
	while (1)
	{
		...
		if (!PRVM_ED_ParseEpair(prog, nullptr, key, com_token, true))
...
static void PRVM_GameCommand_Server_f()
{
	PRVM_GameCommand("server", "sv_cmd");
}
static void PRVM_GameCommand_Client_f()
{
	PRVM_GameCommand("client", "cl_cmd");
}
static void PRVM_GameCommand_Menu_f()
{
...
=============
*/
static void PRVM_ED_EdictGet_f()
{
	prvm_prog_t *prog;
	prvm_edict_t *ed;
	ddef_t *key;
	...
	if(Cmd_Argc() == 5)
	{
		...
		Cvar_Get(Cmd_Argv(4), s, 0, nullptr);
	}
	else
		Con_Printf("%s\n", s);
...
static void PRVM_ED_GlobalGet_f()
{
	prvm_prog_t *prog;
	ddef_t *key;
	const char *s;
	...
	if(Cmd_Argc() == 4)
	{
		...
		Cvar_Get(Cmd_Argv(3), s, 0, nullptr);
	}
	else
		Con_Printf("%s\n", s);
...
=============
*/
static void PRVM_ED_EdictSet_f()
{
	prvm_prog_t *prog;
	prvm_edict_t *ed;
	ddef_t *key;
...
//
		if(!ent->priv.required->free)
		{
			if (!PRVM_alledictstring(ent, classname))
			{
				...
				PRVM_ED_Print(prog, ent, nullptr);
				PRVM_ED_Free (prog, ent);
				continue;
			}
...
static po_t *PRVM_PO_Load(const char *filename, const char *filename2, mempool_t *pool)
{
	...
	for (i = 0; i < 2; ++i)
	{
		const char *buf = (const char *)
			FS_LoadFile((i > 0 ? filename : filename2), pool, true, nullptr);
...
static constexpr size_t LoadvabufSize	=	1024;

static void PRVM_Prog_Alloc(
	prvm_prog_t * 	prog,
	dprograms_t* 	dprograms,
	fs_offset_t 	filesize,
	const char*		filename,
	char*			instrings,
	const size_t	requiredGlobals,
	const size_t	requiredFields,
...
)
{
	...
	if (LittleLong(dprograms->ofs_strings) + prog->progs_numstrings > static_cast<int>(filesize))
		prog->error_cmd("%s: %s strings go past end of file", prog->name, filename);

	Pool* pool 			= prog->progs_mempool;
	prog->strings 		= pool->alloc<char>(prog->progs_numstrings);
	...
	prog->globaldefs = pool->alloc<ddef_t>(prog->progs_numglobaldefs + requiredGlobals);

	prog->globals.fp = pool->alloc<prvm_vec_t>(prog->progs_numglobals + requiredGlobalSpace + 2);
	...
	prog->fielddefs = pool->alloc<ddef_t>(prog->progs_numfielddefs + requiredFields);
	// we need to convert the statements to our memory format
	prog->statements = pool->alloc<mstatement_t>(prog->progs_numstatements);
...
static void PRVM_Remap_And_Check(prvm_prog_t* prog, dstatement_t *instatements)
{

	// LordHavoc: TODO: support 32bit progs statement formats
	// copy, remap globals in statements, bounds check
	for (size_t i = 0; i < prog->progs_numstatements; i++)
	{
		opcode_t op = (opcode_t)LittleShort(instatements[i].op);
		int a = (unsigned short)LittleShort(instatements[i].a);
		int b = (unsigned short)LittleShort(instatements[i].b);
		int c = (unsigned short)LittleShort(instatements[i].c);
		switch (op)
		{
		case OP_IF:
		case OP_IFNOT:
...
static void PRVM_PO_Thing(prvm_prog_t* prog, const char* filename)
{
	char vabuf[LoadvabufSize];
	char vabuf2[LoadvabufSize];
	...
	bool deftrans = prog == CLVM_prog;
	const char *realfilename = (prog != CLVM_prog ? filename : csqc_progname.string);
	const size_t numGlobalDefs = static_cast<size_t>(prog->numglobaldefs);

	if(deftrans) // once we have dotranslate_ strings, ALWAYS use the opt-in method!
	{
		for (size_t i = 0; i < numGlobalDefs; i++)
		{
			ddef_t * const def = &prog->globaldefs[i];
			const char *name = PRVM_GetString(prog, def->s_name);
			if((def->type & ~DEF_SAVEGLOBAL) == ev_string && name && !strncmp(name, "dotranslate_", 12))
			{
				deftrans = false;
				break;
			}
	...
	if(!strcmp(prvm_language.string, "dump"))
	{
		...
		for (size_t i = 0; i < numGlobalDefs; i++)
		{
			...
			{
				prvm_eval_t *val = PRVM_GLOBALFIELDVALUE(def->ofs);
...
static void PRVM_Fields_f ()
{
	...
	counts = tempmempool->alloc<int>(prog->numfielddefs);
...
static void PRVM_GlobalSet_f()
{
	...
	if( !global )
		...
		PRVM_ED_ParseEpair( prog, nullptr, global, Cmd_Argv(3), true );
}
...
static void PRVM_GlobalWatchpoint_f()
{
	prvm_prog_t *prog;
...
static void PRVM_EdictWatchpoint_f()
{
	prvm_prog_t *prog;
...
int PRVM_SetEngineString(prvm_prog_t *prog, const char *s)
{
	...
	if(prog->leaktest_active)
		prog->knownstrings_origin[i] = nullptr;
	return PRVM_KNOWNSTRINGBASE + i;
}
...
int PRVM_SetTempString(prvm_prog_t *prog, const char *s)
{
	...
	if (prog->tempstringsbuf.maxsize < prog->tempstringsbuf.cursize + size)
	{
		...
		if (prog->tempstringsbuf.maxsize != old.maxsize || prog->tempstringsbuf.data == nullptr)
		{
...
{
	if (num == 0)
		prog->error_cmd("PRVM_FreeString: attempt to free a nullptr string");
...
static bool PRVM_IsStringReferenced(prvm_prog_t *prog, string_t string)
{
	int i, j;
...
static bool PRVM_IsEdictRelevant(prvm_prog_t *prog, prvm_edict_t *edict)
{
	char vabuf[1024];
	char vabuf2[1024];
...
static bool PRVM_IsEdictReferenced(prvm_prog_t *prog, prvm_edict_t *edict, int mark)
{
	int i, j;
