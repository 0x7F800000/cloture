...
constexpr float COLLISION_EDGEDIR_DOT_EPSILON 	= 0.999f;
constexpr float COLLISION_EDGECROSS_MINLENGTH2 	= 1.0f / 4194304.0f;
constexpr float COLLISION_SNAPSCALE 			= 32.0f;
constexpr float COLLISION_SNAP 					= 1.0f / COLLISION_SNAPSCALE;
constexpr float COLLISION_SNAP2 				= 2.0f / COLLISION_SNAPSCALE;
constexpr float COLLISION_PLANE_DIST_EPSILON 	= 2.0f / COLLISION_SNAPSCALE;
...
void Collision_Init()
{
	...
	collision_mempool = Mem_AllocPool("collision cache", 0, nullptr);
...
static void Collision_ValidateBrush(colbrushf_t *brush)
{
	...
	if (brush->numplanes)
	{
		...
		for (k = 0;k < brush->numplanes;k++)
			if (unlikely(DotProduct(brush->planes[k].normal, brush->planes[k].normal) < 0.0001f))
		...
		if (unlikely(pointswithinsufficientplanes))
		{
		...
		if (unlikely(pointsoffplanes == 0)) // all points are on all planes
		{
...
static float nearestplanedist_float(const float *normal, const colpointf_t *points, int numpoints)
{
	if (numpoints == 0)
		return .0f;
	float bestdist = DotProduct(points->v, normal);
	points++;
	while(--numpoints)
	{
		const float dist = DotProduct(points->v, normal);
...
static void Collision_CalcEdgeDirsForPolygonBrushFloat(colbrushf_t *RESTRICT const brush)
{
	const size_t numPoints = static_cast<size_t>(brush->numpoints);
	size_t j = numPoints - 1;
	for (size_t i = 0; i < numPoints; j = i, i++)
...
colbrushf_t *Collision_NewBrushFromPlanes(mempool_t *mempool, int numoriginalplanes, const colplanef_t *originalplanes, int supercontents, int q3surfaceflags, const texture_t *texture, int hasaabbplanes)
{
	...
	int numpointsbuf = 0;
	int maxpointsbuf = 256;
	int numedgedirsbuf = 0;
	int maxedgedirsbuf = 256;
	int numplanesbuf = 0;
	int maxplanesbuf = 256;
	int numelementsbuf = 0;
	int maxelementsbuf = 256;
	...
	if (unlikely(numoriginalplanes >= maxplanesbuf))
	{
	...
	for (j = 0;j < numoriginalplanes;j++)
		maxdist = max(maxdist, math::fabs(originalplanes[j].dist));
	...
	for (j = 0;j < numoriginalplanes;j++)
	{
		// add the new plane
		VectorCopy(
			originalplanes[j].normal, 
			planesbuf[numplanesbuf].normal
		);
		...
		PolygonD_QuadForPlane(
			p[w], 
			originalplanes[j].normal[0], 
			originalplanes[j].normal[1], 
			originalplanes[j].normal[2], 
			originalplanes[j].dist, 
			maxdist
		);
		pnumpoints = 4;
		...
		for (k = 0;k < numoriginalplanes && pnumpoints >= 3 && pnumpoints <= pmaxpoints;k++)
		{
			...
			if (k != j)
			{
				...
				PolygonD_Divide(
					pnumpoints, 
					p[w], 
					-originalplanes[k].normal[0], 
					-originalplanes[k].normal[1], 
					-originalplanes[k].normal[2], 
					-originalplanes[k].dist, 
					COLLISION_PLANE_DIST_EPSILON, 
					pmaxpoints, p[!w], 
					&pnumpoints, 
					0, 
					nullptr, 
		...
		if (unlikely(pnumpoints > pmaxpoints))
		{
		...
		if (unlikely(numelementsbuf + (pnumpoints - 2) * 3 > maxelementsbuf))
		{
		...
		for (k = 0;k < pnumpoints;k++)
		{
			...
			if (m == numpointsbuf)
			{
				...
				if (unlikely(numpointsbuf >= maxpointsbuf))
				{
					Con_DPrint("Collision_NewBrushFromPlanes: failed to build collision brush: too many points for buffer\n");
					return nullptr;
				}
		...
		for (k = 0, l = pnumpoints-1;k < pnumpoints;l = k, k++)
		{
			...
			{
				// check if there are too many and skip the brush
				if (unlikely(numedgedirsbuf >= maxedgedirsbuf))
				{
	...
	if (unlikely(numelementsbuf < 12 || numpointsbuf < 4))
	...
	{
		const float d = furthestplanedist_float(planesbuf[j].normal, pointsbuf, numpointsbuf);
		if (unlikely(math::fabsf(planesbuf[j].dist - d) > COLLISION_PLANE_DIST_EPSILON))
			Con_DPrintf("plane %f %f %f %f mismatches dist %f\n", planesbuf[j].normal[0], planesbuf[j].normal[1], planesbuf[j].normal[2], planesbuf[j].dist, d);
	}

	// allocate the brush and copy to it
	brush = reinterpret_cast<colbrushf_t *>(Mem_Alloc(
		mempool, 
		sizeof(colbrushf_t) + sizeof(colpointf_t) * numpointsbuf + 
		sizeof(colpointf_t) * numedgedirsbuf + 
		sizeof(colplanef_t) * numplanesbuf + 
		sizeof(int) * numelementsbuf
	));
	brush->isaabb 			= isaabb;
	...
	brush->planes 	= reinterpret_cast<colplanef_t *>(brush + 1);
	brush->points 	= reinterpret_cast<colpointf_t *>(brush->planes + brush->numplanes);
	brush->edgedirs = reinterpret_cast<colpointf_t *>(brush->points + brush->numpoints);
	brush->elements = reinterpret_cast<int *>(brush->points + brush->numpoints);
...
void Collision_CalcPlanesForTriangleBrushFloat(colbrushf_t *brush)
{
	...
	else if (collision_triangle_axialsides.integer)
	{
		...
		int i, best;
		float bestdist = math::fabsf(brush->planes[0].normal[0]);
		best = 0;
		for (i = 1;i < 3;i++)
		{
			const float dist = math::fabsf(brush->planes[0].normal[i]);
			if (bestdist < dist)
			{
				bestdist = dist;
				best = i;
	...
	if (unlikely(developer_extra.integer != 0))
	{
		...
		for (i = 0;i < brush->numplanes;i++)
		{
			...
			{
				if (unlikely(DotProduct(p->v, brush->planes[i].normal) > brush->planes[i].dist + COLLISION_PLANE_DIST_EPSILON))
...
{
	colbrushf_t *brush = reinterpret_cast<colbrushf_t *>(
			Mem_Alloc(
				mempool, 
				sizeof(colbrushf_t) + sizeof(colplanef_t) * (numpoints + 2) + 
				sizeof(colpointf_t) * numpoints
		)
	);
	brush->isaabb 			= false;
	...
	brush->planes 			= reinterpret_cast<colplanef_t *>(brush + 1);
	brush->points 			= reinterpret_cast<colpointf_t *>(points);
	brush->edgedirs 		= reinterpret_cast<colpointf_t *>(brush->planes + brush->numplanes);
	brush->q3surfaceflags 	= q3surfaceflags;
	brush->texture 			= texture;
...
void Collision_TraceBrushBrushFloat(trace_t *trace, const colbrushf_t *trace_start, const colbrushf_t *trace_end, const colbrushf_t *other_start, const colbrushf_t *other_end)
{
	...
	vec_t enterfrac = -1.0f, leavefrac = 1.0f, startdist, enddist, ie, f, imove, enterfrac2 = -1.0f;
	vec4_t startplane;
	vec4_t endplane;
	vec4_t newimpactplane;
	const texture_t *hittexture = nullptr;
	vec_t startdepth = 1.0f;
	...
	for (nplane = 0;nplane < numplanes3;nplane++)
	{
		...
		else
		{
			...
			if (bitwise::test(&nplane2, 0))//nplane2 & 1)
			{
		...
		if (startdist <= .0f && nplane < numplanes1 && (startdepth < startdist || startdepth == 1.0f))
		{
...
{
	int nplane;
	int hitq3surfaceflags = 0;
	int numplanes = other_start->numplanes;
	vec_t enterfrac = -1.0f;
	vec_t leavefrac = 1.0f;
	vec_t startdist;
	vec_t enddist;
	vec_t ie;
	vec_t f;
	vec_t imove;
	vec_t enterfrac2 = -1.0f;
	vec4_t startplane;
	vec4_t endplane;
	vec4_t newimpactplane;
	const texture_t *hittexture = nullptr;
	...
	if (unlikely(collision_debug_tracelineasbox.integer))
	{
		colboxbrushf_t thisbrush_start, thisbrush_end;
		Collision_BrushForBox(&thisbrush_start, linestart, linestart, 0, 0, nullptr);
		Collision_BrushForBox(&thisbrush_end, lineend, lineend, 0, 0, nullptr);
	...
	for (nplane = 0;nplane < numplanes;nplane++)
	{
		...
		// aside from collisions, this is also used for error correction
		if (startdist <= .0f && (startdepth < startdist || startdepth == 1))
		{
			startdepth = startdist;
			VectorCopy(startplane, startdepthnormal);
		}
...
bool Collision_PointInsideBrushFloat(const vec3_t point, const colbrushf_t *brush)
{
	int nplane;
...
static void Collision_SnapCopyPoints(
		const size_t 		numpoints, 
		const colpointf_t 	*in, 
		colpointf_t 		*out, 
		const float 		fractionprecision, 
		const float 		invfractionprecision
)
{
	for (size_t i = 0; i < numpoints; i++)
	{
		const colpointf_t* const inPoint 	= &in[i];
		colpointf_t* const outPoint 		= &out[i];
		outPoint->v[0] 	= math::floorf(inPoint->v[0] * fractionprecision + 0.5f) * invfractionprecision;
		outPoint->v[1] 	= math::floorf(inPoint->v[1] * fractionprecision + 0.5f) * invfractionprecision;
		outPoint->v[2] 	= math::floorf(inPoint->v[2] * fractionprecision + 0.5f) * invfractionprecision;
	}
}

void Collision_TraceBrushTriangleMeshFloat(
	trace_t 			*trace, 
...
)
{
	...
	{
		const size_t brushNumPlanes = static_cast<size_t>(brush.numplanes);
		for (size_t i = 0; i < brushNumPlanes; i++)
		{
	...
	if(stride > 0)
	{
		const size_t cnt = (numtriangles + stride - 1) / stride;
		for(size_t i = 0; i < cnt; ++i)
		{
			...
			{
				const size_t tri = static_cast<size_t>(static_cast<int>(i) * stride + static_cast<int>(k));
				if(tri >= numtriangles)
					break;
	...
	{
		for (size_t i = 0;i < numtriangles;i++, element3i += 3)
		{
...
void Collision_TraceBrushTriangleFloat(trace_t *trace, const colbrushf_t *thisbrush_start, const colbrushf_t *thisbrush_end, const float *v0, const float *v1, const float *v2, int supercontents, int q3surfaceflags, const texture_t *texture)
{
	...
	brush.q3surfaceflags = q3surfaceflags;
	brush.texture = texture;
	for (size_t i = 0;i < brush.numplanes;i++)
	{
		brush.planes[i].q3surfaceflags = q3surfaceflags;
		brush.planes[i].texture = texture;
	}
...
)
{
	...
	boxbrush->brush.q3surfaceflags = q3surfaceflags;
	boxbrush->brush.texture = texture;
	VectorSet(boxbrush->brush.mins, mins[0] - 1.0f, mins[1] - 1.0f, mins[2] - 1.0f);
	VectorSet(boxbrush->brush.maxs, maxs[0] + 1.0f, maxs[1] + 1.0f, maxs[2] + 1.0f);
}
...
float Collision_ClipTrace_Line_Sphere(double *linestart, double *lineend, double *sphereorigin, double sphereradius, double *impactpoint, double *impactnormal)
{
	...
	if (deviationdist2 < .0)
		return 1.0f; // miss (off to the side)
	// nudge back to find the correct impact distance
	impactdist -= sqrt(deviationdist2);
	if (impactdist >= linelength)
		return 1.0f; // miss (not close enough)
	if (impactdist < .0)
		return 1.0f; // miss (linestart is past or inside sphere)
...
void Collision_TraceLineTriangleFloat(trace_t *trace, const vec3_t linestart, const vec3_t lineend, const float *point0, const float *point1, const float *point2, int supercontents, int q3surfaceflags, const texture_t *texture)
{
	...
	d = 1.0 / math::sqrt(faceplanenormallength2);
...
static void Collision_TranslateBrush(const vec3_t shift, colbrushf_t *RESTRICT brush)
{
	// now we can transform the data
	{
		const size_t numPlanes = brush->numplanes;
		for(size_t i = 0; i < numPlanes; ++i)
			brush->planes[i].dist += DotProduct(shift, brush->planes[i].normal);
	}
	{
		const size_t numPoints = brush->numpoints;
		for(size_t i = 0; i < numPoints; ++i)
...
static void Collision_TransformBrush(const matrix4x4_t *RESTRICT matrix, colbrushf_t *RESTRICT brush)
{
	vec3_t v;
	...
	// now we can transform the data
	{
		...
		for(size_t i = 0; i < numPlanes; ++i)
		{
			...
			Matrix4x4_TransformPositivePlane(matrix, plane->normal[0], plane->normal[1], plane->normal[2], plane->dist, plane->normal);
		}
	}
	{
		const size_t numEdgeDirs = brush->numedgedirs;
		for(size_t i = 0; i < numEdgeDirs; ++i)
		{
			auto edgedirsv = brush->edgedirs[i].v;
			Matrix4x4_Transform(matrix, edgedirsv, v);
			VectorCopy(v, edgedirsv);
		}
	}
	{
		const size_t numPoints = brush->numpoints;
		...
		for(size_t i = 0; i < numPoints; ++i)
		{
			auto pointv = brush->points[i].v;
			Matrix4x4_Transform(matrix, pointv, v);
			VectorCopy(v, pointv);
		}
	}
	VectorCopy(brush->points[0].v, brush->mins);
	...
	
	{
		...
		for(size_t i = 1; i < numPoints; ++i)
		{
			...
			if(pointv[0] < brush->mins[0]) 
				brush->mins[0] = pointv[0];
				
			if(pointv[1] < brush->mins[1]) 
				brush->mins[1] = pointv[1];
				
			if(pointv[2] < brush->mins[2]) 
				brush->mins[2] = pointv[2];
				
			if(pointv[0] > brush->maxs[0]) 
				brush->maxs[0] = pointv[0];
				
			if(pointv[1] > brush->maxs[1]) 
				brush->maxs[1] = pointv[1];
				
			if(pointv[2] > brush->maxs[2]) 
				brush->maxs[2] = pointv[2];
		}
	}
}
...
static bool collision_cachedtrace_rebuildhash;

void Collision_Cache_Reset(bool resetlimits)
{
...
static void Collision_Cache_RebuildHash()
{
	int index;
...
static unsigned int Collision_Cache_HashIndexForArray(unsigned int *RESTRICT  array, unsigned int size)
{
	...
	unsigned int hashindex = 0;
	{
		...
		for (size_t i = 0; i < sz; i++)
			hashindex += array[i] * (1 + static_cast<unsigned int>(i));
	}
	return hashindex;
}
...
void Collision_Cache_ClipLineToGenericEntitySurfaces(trace_t *trace, dp_model_t *model, matrix4x4_t *matrix, matrix4x4_t *inversematrix, const vec3_t start, const vec3_t end, int hitsupercontentsmask)
{
	...
	Collision_ClipLineToGenericEntity(trace, model, nullptr, nullptr, vec3_origin, vec3_origin, 0, matrix, inversematrix, start, end, hitsupercontentsmask, collision_extendmovelength.value, true);
...
{
	collision_cachedtrace_t *cached = Collision_Cache_Lookup(model, 
	reinterpret_cast<const matrix4x4_t*>(&identitymatrix), reinterpret_cast<const matrix4x4_t*>(&identitymatrix), start, end, hitsupercontents);
	if (cached->valid)
	{
		*trace = cached->result;
		return;
...
void Collision_ClipToGenericEntity(trace_t *trace, dp_model_t *model, const frameblend_t *frameblend, const skeleton_t *skeleton, const vec3_t bodymins, const vec3_t bodymaxs, int bodysupercontents, matrix4x4_t *matrix, matrix4x4_t *inversematrix, const vec3_t tstart, const vec3_t mins, const vec3_t maxs, const vec3_t tend, int hitsupercontentsmask, float extend)
{
	...
	if (model && model->TraceBox)
	{
		if(model->TraceBrush && (inversematrix->m[0][1] || inversematrix->m[0][2] || inversematrix->m[1][0] || inversematrix->m[1][2] || inversematrix->m[2][0] || inversematrix->m[2][1]))
		{
			...
			colboxbrushf_t thisbrush_start, thisbrush_end;
			Collision_BrushForBox(&thisbrush_start, mins, maxs, 0, 0, nullptr);
			Collision_BrushForBox(&thisbrush_end, mins, maxs, 0, 0, nullptr);
		...
		Collision_ClipTrace_Box(trace, bodymins, bodymaxs, starttransformed, mins, maxs, endtransformed, hitsupercontentsmask, bodysupercontents, 0, nullptr);
...
void Collision_ClipToWorld(trace_t *trace, dp_model_t *model, const vec3_t tstart, const vec3_t mins, const vec3_t maxs, const vec3_t tend, int hitsupercontents, float extend)
{
	...
	if (model && model->TraceBox)
		model->TraceBox(model, nullptr, nullptr, trace, extendtraceinfo.extendstart, mins, maxs, extendtraceinfo.extendend, hitsupercontents);
	Collision_ClipExtendFinish(&extendtraceinfo);
}

void Collision_ClipLineToGenericEntity(trace_t *trace, dp_model_t *model, const frameblend_t *frameblend, const skeleton_t *skeleton, const vec3_t bodymins, const vec3_t bodymaxs, int bodysupercontents, matrix4x4_t *matrix, matrix4x4_t *inversematrix, const vec3_t tstart, const vec3_t tend, int hitsupercontentsmask, float extend, bool hitsurfaces)
{
	vec3_t starttransformed, endtransformed;
	extendtraceinfo_t extendtraceinfo;
	Collision_ClipExtendPrepare(&extendtraceinfo, trace, tstart, tend, extend);
	...
	if (model && model->TraceLineAgainstSurfaces && hitsurfaces)
		...
		Collision_ClipTrace_Box(trace, bodymins, bodymaxs, starttransformed, vec3_origin, vec3_origin, endtransformed, hitsupercontentsmask, bodysupercontents, 0, nullptr);
...
void Collision_ClipLineToWorld(trace_t *trace, dp_model_t *model, const vec3_t tstart, const vec3_t tend, int hitsupercontents, float extend, bool hitsurfaces)
{
	extendtraceinfo_t extendtraceinfo;
	Collision_ClipExtendPrepare(&extendtraceinfo, trace, tstart, tend, extend);

	if (model && model->TraceLineAgainstSurfaces && hitsurfaces)
		model->TraceLineAgainstSurfaces(model, nullptr, nullptr, trace, extendtraceinfo.extendstart, extendtraceinfo.extendend, hitsupercontents);
	else if (model && model->TraceLine)
		model->TraceLine(model, nullptr, nullptr, trace, extendtraceinfo.extendstart, extendtraceinfo.extendend, hitsupercontents);
...
void Collision_ClipPointToGenericEntity(trace_t *trace, dp_model_t *model, const frameblend_t *frameblend, const skeleton_t *skeleton, const vec3_t bodymins, const vec3_t bodymaxs, int bodysupercontents, matrix4x4_t *matrix, matrix4x4_t *inversematrix, const vec3_t start, int hitsupercontentsmask)
{
	...
	if (model && model->TracePoint)
		model->TracePoint(model, nullptr, nullptr, trace, starttransformed, hitsupercontentsmask);
	else
		Collision_ClipTrace_Point(trace, bodymins, bodymaxs, starttransformed, hitsupercontentsmask, bodysupercontents, 0, nullptr);
...
void Collision_ClipPointToWorld(trace_t *trace, dp_model_t *model, const vec3_t start, int hitsupercontents)
{
	...
	trace->fraction = 1;
	if (model && model->TracePoint)
		model->TracePoint(model, nullptr, nullptr, trace, start, hitsupercontents);
	VectorCopy(start, trace->endpos);
}

void Collision_CombineTraces(trace_t *cliptrace, const trace_t *trace, void *touch, bool isbmodel)
{
