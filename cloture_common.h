...
typedef int (*COM_LineProcessorFunc) (void *passthrough, const char *line, size_t length, float width, bool isContination);
...
void COM_Init ();
void COM_Shutdown ();
void COM_InitGameType ();

char *va(char *buf, size_t buflen, const char *format, ...) __printf_type(3);
...
extern int dpsnprintf (char *buffer, size_t buffersize, const char *format, ...) __printf_type(3);
...
void COM_ChangeGameTypeForGameDirs();
...
size_t COM_StringLengthNoColors(const char *s, size_t size_s, bool *valid);
bool COM_StringDecolorize(const char *in, size_t size_in, char *out, size_t size_out, bool escape_carets);
void COM_ToLowerString (const char *in, char *out, size_t size_out);
void COM_ToUpperString (const char *in, char *out, size_t size_out);
...
void stringlistsort(stringlist_t *list, bool uniq);
