...
static void R_DrawLightningBeam_TransparentCallback(const entity_render_t *ent, const rtlight_t *rtlight, int numsurfaces, int *surfacelist)
{
	...
	RSurf_ActiveCustomEntity(reinterpret_cast<const matrix4x4_t*>(&identitymatrix),
			reinterpret_cast<const matrix4x4_t*>(&identitymatrix), 0, 0,
			r_lightningbeam_color_red.value, r_lightningbeam_color_green.value,
			r_lightningbeam_color_blue.value, 1, 12, vertex3f, texcoord2f, NULL, NULL, NULL, NULL, 6,
			r_lightningbeamelement3i, r_lightningbeamelement3s, false, false);
