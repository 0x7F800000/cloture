...
static void R_TrackSprite(const entity_render_t *ent, vec3_t origin, vec3_t left, vec3_t up, int *edge, float *dir_angle)
{
	...
	float distance = VectorLength(bCoord);
	...
	for (i = 0;i < MAX_FRAMEBLENDS;i++)
	{
		if (ent->frameblend[i].lerp >= 0.01f)
		{
			...
			RSurf_ActiveCustomEntity(reinterpret_cast<const matrix4x4_t*>(&identitymatrix),
					reinterpret_cast<const matrix4x4_t*>(&identitymatrix), ent->flags, 0, ent->colormod[0], ent->colormod[1], ent->colormod[2], ent->alpha * ent->frameblend[i].lerp, 4, vertex3f, spritetexcoord2f, NULL, NULL, NULL, NULL, 2, polygonelement3i, polygonelement3s, false, false);
			...
			R_DrawCustomSurface_Texture(texture, reinterpret_cast<const matrix4x4_t*>(&identitymatrix), texture->currentmaterialflags, 0, 4, 0, 2, false, false);
		}
	}
