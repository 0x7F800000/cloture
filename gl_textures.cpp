...
static unsigned char *resizebuffer = nullptr, *colorconvertbuffer;
...
void R_FreeTexture(rtexture_t *rt)
{
	...
	case RENDERPATH_D3D9:
		...
		glt->d3dtexture = nullptr;
		glt->d3dsurface = nullptr;
#endif
		break;
...
void R_FreeTexturePool(rtexturepool_t **rtexturepool)
{
	...
	if (rtexturepool == nullptr)
		return;
	if (*rtexturepool == nullptr)
		return;
	pool = (gltexturepool_t *)(*rtexturepool);
	*rtexturepool = nullptr;
...
static int R_CalcTexelDataSize (gltexture_t *glt)
{
	...
	GL_Texture_CalcImageSize(glt->texturetype, glt->flags, glt->miplevel, glt->inputwidth, glt->inputheight, glt->inputdepth, &width2, &height2, &depth2, nullptr);
...
static void r_textures_start(void)
{
	...
	texturemempool = Mem_AllocPool("texture management", 0, nullptr);
	...
	for (i = 0;i < endindex;i++)
	{
		...
		case RENDERPATH_D3D9:
#ifdef SUPPORTD3D
			{
				...
				{
					if (FAILED(d3dresult = IDirect3DDevice9_CreateRenderTarget(vid_d3d9dev, glt->tilewidth, glt->tileheight, (D3DFORMAT)glt->d3dformat, D3DMULTISAMPLE_NONE, 0, false, (IDirect3DSurface9 **)&glt->d3dsurface, nullptr)))
						Sys_Error("IDirect3DDevice9_CreateRenderTarget failed!");
				}
				else if (glt->d3disdepthstencilsurface)
				{
					if (FAILED(d3dresult = IDirect3DDevice9_CreateDepthStencilSurface(vid_d3d9dev, glt->tilewidth, glt->tileheight, (D3DFORMAT)glt->d3dformat, D3DMULTISAMPLE_NONE, 0, false, (IDirect3DSurface9 **)&glt->d3dsurface, nullptr)))
						Sys_Error("IDirect3DDevice9_CreateDepthStencilSurface failed!");
				}
				else if (glt->tiledepth > 1)
				{
					if (FAILED(d3dresult = IDirect3DDevice9_CreateVolumeTexture(vid_d3d9dev, glt->tilewidth, glt->tileheight, glt->tiledepth, glt->miplevels, glt->d3dusage, (D3DFORMAT)glt->d3dformat, (D3DPOOL)glt->d3dpool, (IDirect3DVolumeTexture9 **)&glt->d3dtexture, nullptr)))
						Sys_Error("IDirect3DDevice9_CreateVolumeTexture failed!");
				}
				else if (glt->sides == 6)
				{
					if (FAILED(d3dresult = IDirect3DDevice9_CreateCubeTexture(vid_d3d9dev, glt->tilewidth, glt->miplevels, glt->d3dusage, (D3DFORMAT)glt->d3dformat, (D3DPOOL)glt->d3dpool, (IDirect3DCubeTexture9 **)&glt->d3dtexture, nullptr)))
						Sys_Error("IDirect3DDevice9_CreateCubeTexture failed!");
				}
				else
				{
					if (FAILED(d3dresult = IDirect3DDevice9_CreateTexture(vid_d3d9dev, glt->tilewidth, glt->tileheight, glt->miplevels, glt->d3dusage, (D3DFORMAT)glt->d3dformat, (D3DPOOL)glt->d3dpool, (IDirect3DTexture9 **)&glt->d3dtexture, nullptr)))
...
static void R_UploadFullTexture(gltexture_t *glt, const unsigned char *data)
{
	...
	case RENDERPATH_D3D9:
		...
		if (!(glt->flags & TEXF_RENDERTARGET) && glt->d3dtexture && !glt->d3dsurface)
		{
			...
			{
			case GLTEXTURETYPE_2D:
				if (IDirect3DTexture9_LockRect((IDirect3DTexture9*)glt->d3dtexture, mip, &d3dlockedrect, nullptr, 0) == D3D_OK && d3dlockedrect.pBits)
				{
					if (prevbuffer)
				...
				break;
			case GLTEXTURETYPE_3D:
				if (IDirect3DVolumeTexture9_LockBox((IDirect3DVolumeTexture9*)glt->d3dtexture, mip, &d3dlockedbox, nullptr, 0) == D3D_OK && d3dlockedbox.pBits)
				{
			...
			case GLTEXTURETYPE_CUBEMAP:
				...
				for (i = 0;i < 6;i++)
				{
					...
					mip = 0;
					if (IDirect3DCubeTexture9_LockRect((IDirect3DCubeTexture9*)glt->d3dtexture, (D3DCUBEMAP_FACES)i, mip, &d3dlockedrect, nullptr, 0) == D3D_OK && d3dlockedrect.pBits)
					{
						memcpy(d3dlockedrect.pBits, prevbuffer, width*height*glt->bytesperpixel);
...
static rtexture_t *R_SetupTexture(rtexturepool_t *rtexturepool, const char *identifier, int width, int height, int depth, int sides, int flags, int miplevel, textype_t textype, int texturetype, const unsigned char *data, const unsigned int *palette)
{
	...
	unsigned char *temppixels = nullptr;
	bool swaprb;
	...
	glt->updatecallback = nullptr;
	glt->updatacallback_data = nullptr;
	...
	case RENDERPATH_D3D9:
#ifdef SUPPORTD3D
		{
			...
			{
			case TEXTYPE_PALETTE: 
				d3dformat = (flags & TEXF_ALPHA) ? D3DFMT_A8R8G8B8 : D3DFMT_X8R8G8B8;
				break;
			case TEXTYPE_RGBA: 
				d3dformat = (flags & TEXF_ALPHA) ? D3DFMT_A8B8G8R8 : D3DFMT_X8B8G8R8;
				break;
			case TEXTYPE_BGRA: 
				d3dformat = (flags & TEXF_ALPHA) ? D3DFMT_A8R8G8B8 : D3DFMT_X8R8G8B8;
				break;
			...
			default: 
				d3dformat = D3DFMT_A8R8G8B8;
				Sys_Error("R_LoadTexture: unsupported texture type %i when picking D3DFMT", (int)textype);
				break;
			}
...
{
	return R_SetupTexture(rtexturepool, identifier, width, height, 1, 1, TEXF_RENDERTARGET | TEXF_CLAMP | (filter ? TEXF_FORCELINEAR : TEXF_FORCENEAREST), -1, textype, GLTEXTURETYPE_2D, nullptr, nullptr);
}
...
int R_SaveTextureDDSFile(rtexture_t *rt, const char *filename, bool skipuncompressed, bool hasalpha)
{
	...
	{
	default: ddsfourcc = nullptr;bytesperpixel = 4;break;
...
rtexture_t *R_LoadTextureDDSFile(rtexturepool_t *rtexturepool, const char *filename, bool srgb, int flags, bool *hasalphaflag, float *avgcolor, int miplevel, bool optionaltexture) // DDS textures are opaque, so miplevel isn't a pointer but just seen as a hint
{
...

	{
		...
		ktxerror = ktxLoadTextureM(dds, ddssize, &glt->texnum, &target, &sizes, &isMipmapped, &glerror,
								0, nullptr);// can't CHECKGLERROR, the lib catches it
		...
		if (target != GL_TEXTURE_2D)
		{
			...
			return nullptr; // FIXME: delete the texture from memory
		}
	...
	case RENDERPATH_D3D9:
#ifdef SUPPORTD3D
		{
			...
			d3dusage = 0;
			d3dpool = D3DPOOL_MANAGED;
			IDirect3DDevice9_CreateTexture(vid_d3d9dev, glt->tilewidth, glt->tileheight, glt->miplevels, d3dusage, d3dformat, d3dpool, (IDirect3DTexture9 **)&glt->d3dtexture, nullptr);
		}
#endif
		break;
	case RENDERPATH_D3D10:
...
void R_ClearTexture (rtexture_t *rt)
{
	...
	R_UploadFullTexture(glt, nullptr);
}
