...
static pack_t *fs_selfpack = nullptr;
...
gamedir_t *fs_all_gamedirs = nullptr;
...
/// Handle for Zlib DLL
static dllhandle_t zlib_dll = nullptr;
#endif
...
static dllhandle_t shfolder_dll = nullptr;
...
static dllhandle_t shell32_dll = nullptr;

static HRESULT (WINAPI *qCoInitializeEx)(LPVOID pvReserved, DWORD dwCoInit);
static void (WINAPI *qCoUninitialize)();
...
static dllhandle_t ole32_dll = nullptr;
#endif
...
====================
*/
static bool PK3_GetEndOfCentralDir (const char *packfile, int packhandle, pk3_endOfCentralDir_t *eocd)
{
...
static int PK3_BuildFileList (pack_t *pack, const pk3_endOfCentralDir_t *eocd)
{
	...
	// If the package is empty, central_dir is nullptr here
	if (central_dir != nullptr)
		Mem_Free (central_dir);
...
====================
*/
static pack_t *FS_LoadPackPK3FromFD (const char *packfile, int packhandle, bool silent)
{
...
====================
*/
static bool PK3_GetTrueFileOffset (packfile_t *pfile, pack_t *pack)
{
 ...
 *
 */
static bool FS_AddPack_Fullpath(const char *pakfile, const char *shortname, bool *already_loaded, bool keep_plain_dirs)
{
	searchpath_t *search;
	pack_t *pak = nullptr;
	...
	if(pak)
	{
		...
		if(keep_plain_dirs)
		{
			// find the first item whose next one is a pack or nullptr
			...
			// If insertion_point is nullptr, this means that either there is no
...
static void FS_AddGameDirectory (const char *dir)
{
	...
	for (i = 0;i < list.numstrings;i++)
	{
		...
		{
			FS_AddPack_Fullpath(list.strings[i], list.strings[i] + strlen(dir), nullptr, false);
		}
	}
...
const char *FS_FileExtension (const char *in)
{
	...
	dot = strrchr(in, '.');
	if (dot == nullptr || (separator && (dot < separator)))
		return "";
...
================
*/
static void FS_ClearSearchPath ()
{
...
bool FS_ChangeGameDirs(int numgamedirs, char gamedirs[][MAX_QPATH], bool complain, bool failmissing)
{
	...
	if (numgamedirs > MAX_GAMEDIRS)
	{
		if (complain)
			...
			numgamedirs, static_cast<int>(MAX_GAMEDIRS));
...
{
	PK3_OpenLibrary ();
	fs_mempool = Mem_AllocPool("file management", 0, nullptr);
	if(com_selffd >= 0)
	{
		...
		if(fs_selfpack)
		{
			...
			const char *p;
			FS_AddSelfPack();
			buf = (char *) FS_LoadFile("darkplaces.opt", tempmempool, true, nullptr);
			if(buf)
			{
				const char **new_argv;
				int i = 0;
...
static int FS_ChooseUserDir(userdirmode_t userdirmode, char *userdir, size_t userdirsize)
{
	...
	case USERDIRMODE_MYGAMES:
		...
		mydocsdir[0] = 0;
		if (qSHGetFolderPath && qSHGetFolderPath(nullptr, CSIDL_PERSONAL, nullptr, 0, mydocsdir) == S_OK)
		{
	...
	case USERDIRMODE_SAVEDGAMES:
		...
		{
			savedgamesdir[0] = 0;
			qCoInitializeEx(nullptr, COINIT_APARTMENTTHREADED);

			if (qSHGetKnownFolderPath(&qFOLDERID_SavedGames, qKF_FLAG_CREATE | qKF_FLAG_NO_ALIAS, nullptr, &savedgamesdirw) == S_OK)
			{
				memset(savedgamesdir, 0, sizeof(savedgamesdir));
#if _MSC_VER >= 1400
				wcstombs_s(nullptr, savedgamesdir, sizeof(savedgamesdir), savedgamesdirw, sizeof(savedgamesdir)-1);
#else
...
int FS_SysOpenFD(const char *filepath, const char *mode, bool nonblocking)
{
	int handle = -1;
	int mod, opt;
	unsigned int ind;
...
====================
*/
qfile_t* FS_SysOpen (const char* filepath, const char* mode, bool nonblocking)
{
	qfile_t* file;
...
Return the searchpath where the file was found (or nullptr)
...
===========
*/
static qfile_t *FS_OpenReadFile (const char *filename, bool quiet, bool nonblocking, int symlinkLevels)
{
	searchpath_t *search;
	int pack_ind;
...
============
*/
unsigned char *FS_LoadFile (const char *path, mempool_t *pool, bool quiet, fs_offset_t *filesizepointer)
{
	qfile_t *file;
...
int FS_FileType (const char *filename)
{
	...
	search = FS_FindFile (filename, nullptr, true);
	if(!search)
...
{
	return (FS_FindFile (filename, nullptr, true) != nullptr);
}
