...
void R_BuildLightMap (const entity_render_t *ent, msurface_t *surface)
{
	...
	int smax = (surface->lightmapinfo->extents[0]>>4)+1;
	int tmax = (surface->lightmapinfo->extents[1]>>4)+1;
	...
	if (cl.buildlightmapmemorysize < size*sizeof(int[3]))
	{
		...
		cl.buildlightmapmemory = cls.levelmempool->alloc<uint8>(cl.buildlightmapmemorysize);
	}
	...
	int *intblocklights = reinterpret_cast<int*>(cl.buildlightmapmemory);
	uint8* templight 	= reinterpret_cast<uint8*>(cl.buildlightmapmemory);

	// update cached lighting info
	model->brushq1.lightmapupdateflags[surface - model->data_surfaces] = false;

	uint8* lightmap = surface->lightmapinfo->samples;

// set to full bright if no light data
	int* bl = intblocklights;
...
// add all the lightmaps
	...
	uint8* stain = surface->lightmapinfo->stainsamples;
	bl = intblocklights;
	uint8* out = templight;
	...
	if (stain)
	{
		...
		for (size_t i = 0;i < size;i++, bl += 3, stain += 3, out += 4)
		{
			l = (bl[0] * stain[0]) >> 16;
			out[2] = min(l, 255);
	...
	{
		for (size_t i = 0;i < size;i++, bl += 3, out += 4)
		{
...
static void R_StainNode (mnode_t *RESTRICT node, dp_model_t *model, const vec3_t origin, float radius, const float fcolor[8])
{
	float a, ratio, maxdist2, sdtable[256], td;
...
loc0:
	if (!node->plane)
		return;
	float ndist = PlaneDiff(origin, node->plane);
	if (ndist > radius)
	{
	...
	float dist2 = ndist * ndist;
	float maxdist3 = maxdist - dist2;
	...
	for (surface = model->data_surfaces + node->firstsurface, endsurface = surface + node->numsurfaces;surface < endsurface;surface++)
	{
		...
		int impacts = (int)(DotProduct (impact, surface->lightmapinfo->texinfo->vecs[0]) + surface->lightmapinfo->texinfo->vecs[0][3] - surface->lightmapinfo->texturemins[0]);
		int impactt = (int)(DotProduct (impact, surface->lightmapinfo->texinfo->vecs[1]) + surface->lightmapinfo->texinfo->vecs[1][3] - surface->lightmapinfo->texturemins[1]);

		int s = bound(0, impacts, smax * 16) - impacts;
		int t = bound(0, impactt, tmax * 16) - impactt;
		i = (int)(s * s + t * t + dist2);
			...
			if (td < maxdist3)
			{
				...
				for (s = 0;s < smax;s++)
				{
					if (sdtable[s] < maxdist2)
					{
						...
						a = (fcolor[3] + ratio * fcolor[7]) * (1.0f - math::sqrtf(sdtable[s] + td) * invradius);
						if (a >= (1.0f / 64.0f))
						{
							if (a > 1.0f)
								a = 1.0f;
...
void R_Stain (const vec3_t origin, float radius, int cr1, int cg1, int cb1, int ca1, int cr2, int cg2, int cb2, int ca2)
{
	...
	dp_model_t *model;
	vec3_t org;
	if (r_refdef.scene.worldmodel == nullptr || !r_refdef.scene.worldmodel->brush.data_nodes || !r_refdef.scene.worldmodel->brushq1.lightdata)
		return;
	fcolor[0] = cr1;
	fcolor[1] = cg1;
	fcolor[2] = cb1;
...
static void R_DrawPortal_Callback(const entity_render_t *ent, const rtlight_t *rtlight, int numsurfaces, int *surfacelist)
{
	...
	R_Mesh_PrepareVertices_Generic_Arrays(numpoints, vertex3f, nullptr, nullptr);
	R_SetupShader_Generic_NoTexture(false, false);
	R_Mesh_Draw(0, numpoints, 0, numpoints - 2, polygonelement3i, nullptr, 0, polygonelement3s, nullptr, 0);
}
...
static void R_View_WorldVisibility_CullSurfaces()
{
	int surfaceindex;
	...
	if (!model)
		return;
	if (unlikely(r_trippy.integer != 0))
		return;
...
void R_View_WorldVisibility(bool forcenovis)
{
	...
	viewleaf = model->brush.PointInLeaf ? model->brush.PointInLeaf(model, r_refdef.view.origin) : nullptr;
...
{
	dp_model_t *model = ent->model;
	if (model == nullptr || model->surfmesh.isanimated)
		return;
...
static void R_Q1BSP_RecursiveGetLightInfo_BSP(r_q1bsp_getlightinfo_t *info, bool skipsurfaces)
{
	// nodestack
	...
	float v2[3][3];
	bool insidebox;
	bool frontsidecasting = r_shadow_frontsidecasting.integer != 0;
	bool svbspactive = info->svbsp_active;
	bool svbspinsertoccluder = info->svbsp_insertoccluder;
	...
	while (nodestackpos)
	{
		...
		else
		{
			...
			if (r_shadow_frontsidecasting.integer && info->pvs != nullptr && !CHECKPVSBIT(info->pvs, leaf->clusterindex))
				continue;
			...
			if (svbspactive)
			{
				...
				for (portal = leaf->portals;portal;portal = portal->next)
				{
					...
					if (SVBSP_AddPolygon(&r_svbsp, portal->numpoints, points[0], false, nullptr, nullptr, 0) & 2)
						break;
				}
				if (leaf->portals && portal == nullptr)
				...
				for (leafsurfaceindex = 0;leafsurfaceindex < numleafsurfaces;leafsurfaceindex++)
				{
					...
					for (triangleindex = 0, t = surface->num_firstshadowmeshtriangle, e = info->model->brush.shadowmesh->element3i + t * 3;triangleindex < surface->num_triangles;triangleindex++, t++, e += 3)
					{
						...
						if (insidebox || TriangleBBoxOverlapsBox(v2[0], v2[1], v2[2], info->lightmins, info->lightmaxs))
							SVBSP_AddPolygon(&r_svbsp, 3, v2[0], true, nullptr, nullptr, 0);
					}
				}
			}
			else
			{
				for (leafsurfaceindex = 0;leafsurfaceindex < numleafsurfaces;leafsurfaceindex++)
				{
					...
					for (triangleindex = 0, t = surface->num_firstshadowmeshtriangle, e = info->model->brush.shadowmesh->element3i + t * 3;triangleindex < surface->num_triangles;triangleindex++, t++, e += 3)
					{
						...
						if (!insidebox && !TriangleBBoxOverlapsBox(v2[0], v2[1], v2[2], info->lightmins, info->lightmaxs))
							continue;
						if (svbspactive && !(SVBSP_AddPolygon(&r_svbsp, 3, v2[0], false, nullptr, nullptr, 0) & 2))
							continue;
	...
	while (nodestackpos)
	{
		...
		if (node->type == BIH_UNORDERED)
		{
			for (nodeleafindex = 0;nodeleafindex < BIH_MAXUNORDEREDCHILDREN && node->children[nodeleafindex] >= 0;nodeleafindex++)
			{
				...
				if (info->svbsp_active && !(SVBSP_AddPolygon(&r_svbsp, 3, v2[0], false, nullptr, nullptr, 0) & 2))
					continue;
...
static void R_Q1BSP_CallRecursiveGetLightInfo(r_q1bsp_getlightinfo_t *info, bool use_svbsp)
{
	...
	if (r_shadow_usebihculling.integer > 0 && (r_shadow_usebihculling.integer == 2 || info->model->brush.num_leafs == 1) && info->model->render_bih.leafs != nullptr)
	{
	...
	r_svbsp.nodes = nullptr;
	if (unlikely(developer_extra.integer != 0) && use_svbsp)
	{
...
void R_Q1BSP_GetLightInfo(entity_render_t *ent, vec3_t relativelightorigin, float lightradius, vec3_t outmins, vec3_t outmaxs, int *outleaflist, unsigned char *outleafpvs, int *outnumleafspointer, int *outsurfacelist, unsigned char *outsurfacepvs, int *outnumsurfacespointer, unsigned char *outshadowtrispvs, unsigned char *outlighttrispvs, unsigned char *visitingleafpvs, int numfrustumplanes, const mplane_t *frustumplanes)
{
	...
	if (r_shadow_frontsidecasting.integer && r_shadow_compilingrtlight && r_shadow_realtime_world_compileportalculling.integer && info.model->brush.data_portals)
	{
		...
		Portal_Visibility(info.model, info.relativelightorigin, info.outleaflist, info.outleafpvs, &info.outnumleafs, info.outsurfacelist, info.outsurfacepvs, &info.outnumsurfaces, nullptr, 0, true, info.lightmins, info.lightmaxs, info.outmins, info.outmaxs, info.outshadowtrispvs, info.outlighttrispvs, info.visitingleafpvs);
	}
	else if (r_shadow_frontsidecasting.integer && r_shadow_realtime_dlight_portalculling.integer && info.model->brush.data_portals)
	{
		...
		Portal_Visibility(info.model, info.relativelightorigin, info.outleaflist, info.outleafpvs, &info.outnumleafs, info.outsurfacelist, info.outsurfacepvs, &info.outnumsurfaces, nullptr, 0, r_shadow_realtime_dlight_portalculling.integer >= 2, info.lightmins, info.lightmaxs, info.outmins, info.outmaxs, info.outshadowtrispvs, info.outlighttrispvs, info.visitingleafpvs);
	}
	else
	{
...
void R_Q1BSP_CompileShadowVolume(entity_render_t *ent, vec3_t relativelightorigin, vec3_t relativelightdirection, float lightradius, int numsurfaces, const int *surfacelist)
{
	...
	if (!model->brush.shadowmesh->neighbor3i)
		return;
	r_shadow_compilingrtlight->static_meshchain_shadow_zfail = Mod_ShadowMesh_Begin(r_main_mempool, 32768, 32768, nullptr, nullptr, nullptr, false, false, true);
...
void R_Q1BSP_CompileShadowMap(entity_render_t *ent, vec3_t relativelightorigin, vec3_t relativelightdirection, float lightradius, int numsurfaces, const int *surfacelist)
{
	...
	if (!model->brush.shadowmesh)
		return;
	r_shadow_compilingrtlight->static_meshchain_shadow_shadowmap = Mod_ShadowMesh_Begin(r_main_mempool, 32768, 32768, nullptr, nullptr, nullptr, false, false, true);
	R_Shadow_PrepareShadowSides(model->brush.shadowmesh->numtriangles);
	for (surfacelistindex = 0;surfacelistindex < numsurfaces;surfacelistindex++)
	{
		surface = model->data_surfaces + surfacelist[surfacelistindex];
		sidemasks |= R_Shadow_ChooseSidesFromBox(surface->num_firstshadowmeshtriangle, surface->num_triangles, model->brush.shadowmesh->vertex3f, model->brush.shadowmesh->element3i, &r_shadow_compilingrtlight->matrix_worldtolight, relativelightorigin, relativelightdirection, r_shadow_compilingrtlight->cullmins, r_shadow_compilingrtlight->cullmaxs, surface->mins, surface->maxs, surface->texture->basematerialflags & MATERIALFLAG_NOSHADOW ? nullptr : sidetotals);
	}
...
//Made by [515]
static void R_ReplaceWorldTexture ()
{
	dp_model_t		*m;
	texture_t	*t;
	int			i;
...
//Made by [515]
static void R_ListWorldTextures ()
{
	dp_model_t		*m;
	texture_t	*t;
	int			i;
