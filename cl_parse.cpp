...
static bool QW_CL_CheckOrDownloadFile(const char *filename);
static void QW_CL_RequestNextDownload();
static void QW_CL_NextUpload();
static void QW_CL_StopUpload();
...
void CL_KeepaliveMessage (bool readmessages)
{
...
static void CL_SetupWorldModel()
{
...
static void QW_CL_RequestNextDownload()
{
	...
	case dl_model:
		...
		for (i = 1;i < MAX_MODELS && cl.model_name[i][0];i++)
			Mod_FindName(cl.model_name[i], cl.model_name[i][0] == '*' ? cl.model_name[1] : nullptr);
		// precache any models used by the client (this also marks them used)
		cl.model_bolt = Mod_ForName("progs/bolt.mdl", false, false, nullptr);
		cl.model_bolt2 = Mod_ForName("progs/bolt2.mdl", false, false, nullptr);
		cl.model_bolt3 = Mod_ForName("progs/bolt3.mdl", false, false, nullptr);
		cl.model_beam = Mod_ForName("progs/beam.mdl", false, false, nullptr);


		// now we try to load everything that is new

		// world model
		cl.model_precache[1] = Mod_ForName(cl.model_name[1], false, false, nullptr);
		if (cl.model_precache[1]->Draw == nullptr)
			Con_Printf("Map %s could not be found or downloaded\n", cl.model_name[1]);

		// normal models
		for (i = 2;i < MAX_MODELS && cl.model_name[i][0];i++)
			if ((cl.model_precache[i] = Mod_ForName(cl.model_name[i], false, false, cl.model_name[i][0] == '*' ? cl.model_name[1] : nullptr))->Draw == nullptr)
		...
		CL_SetInfo("pmodel", va(vabuf, sizeof(vabuf), "%i", FS_CRCFile("progs/player.mdl", nullptr)), true, true, true, true);
		CL_SetInfo("emodel", va(vabuf, sizeof(vabuf), "%i", FS_CRCFile("progs/eyes.mdl", nullptr)), true, true, true, true);
...
static void QW_CL_ParseDownload()
{
	const int size		=	static_cast<int16>(MSG_ReadShort(&cl_message));
	const int percent	=	MSG_ReadByte(&cl_message);
	...
	if (cl_message.readcount + static_cast<uint16>(size) > cl_message.cursize)
	...
	if (!cls.qw_downloadmemory || cls.qw_downloadmemorymaxsize < cls.qw_downloadmemorycursize + size)
	{
		...
		uint8* old = cls.qw_downloadmemory;
		cls.qw_downloadmemory = cls.permanentmempool->alloc<uint8>(cls.qw_downloadmemorymaxsize);
...
static void QW_CL_ParseModelList()
{
	...
	for (;;)
	{
		char* str = MSG_ReadString(&cl_message, cl_readstring, sizeof(cl_readstring));
		if (!str[0])
			break;
		nummodels++;
		
		if (unlikely(nummodels == MAX_MODELS))
			Host_Error("Server sent too many model precaches");
			
		if (unlikely(strlen(str) >= MAX_QPATH))
			Host_Error("Server sent a precache name of %i characters (max %i)", 
			static_cast<int>(strlen(str)), static_cast<int>(MAX_QPATH - 1));
			
		strlcpy(cl.model_name[nummodels], str, sizeof (cl.model_name[nummodels]));
	}

	const int n = MSG_ReadByte(&cl_message);
	if (n)
	{
		MSG_WriteByte(&cls.netcon->message, qw_clc_stringcmd);
		MSG_WriteString(&cls.netcon->message, va(vabuf, sizeof(vabuf), "modellist %i %i", static_cast<int>(cl.qw_servercount), n));
		return;
	}
...
static void QW_CL_ParseSoundList()
{
	...
	for (;;)
	{
		...
		if (!str[0])
			break;
		numsounds++;
		if (unlikely(numsounds == MAX_SOUNDS))
...
void QW_CL_NextUpload()
{
	if (!cls.qw_uploaddata)
		return;

	int r = cls.qw_uploadsize - cls.qw_uploadpos;
	if (r > 768)
		r = 768;
	const int size	=	min(1, cls.qw_uploadsize);
	const int percent =	(cls.qw_uploadpos + r) * 100 / size;
...
void QW_CL_StopUpload()
{
	if (cls.qw_uploaddata)
		Mem_Free(cls.qw_uploaddata);
	cls.qw_uploaddata = nullptr;
	cls.qw_uploadsize = 0;
	cls.qw_uploadpos = 0;
}
...
static void QW_CL_ProcessUserInfo(int slot)
{
	...
	InfoString_GetValue(cl.scores[slot].qw_userinfo, "topcolor", temp, sizeof(temp));
	const int topcolor = atoi(temp);
	
	InfoString_GetValue(cl.scores[slot].qw_userinfo, "bottomcolor", temp, sizeof(temp));
	const int bottomcolor = atoi(temp);
...
static void QW_CL_UpdateUserInfo()
{
	const int slot = MSG_ReadByte(&cl_message);
	if (unlikely(slot >= cl.maxclients))
	{
...
static void QW_CL_ParseNails()
{
	size_t numnails = static_cast<size_t>(MSG_ReadByte(&cl_message));
	
	unsigned char bits[6];
	
	for ( size_t i = 0; i < numnails; i++ )
	{
		...
		vec_t * RESTRICT const v = cl.qw_nails[ cl.qw_num_nails++ ];
		
		v[0] = ( ( bits[0] + ((bits[1]&15)<<8) ) <<1) - 4096.0f;
		v[1] = ( ( (bits[1]>>4) + (bits[2]<<4) ) <<1) - 4096.0f;
		v[2] = ( ( bits[3] + ((bits[4]&15)<<8) ) <<1) - 4096.0f;
		v[3] = -360.0f*(bits[4]>>4)/16.0f;
		v[4] = 360.0f * bits[5] / 256.0f;
		v[5] = .0f;
	}
}

static void CL_UpdateItemsAndWeapon()
{
...
static void CL_BeginDownloads(bool aborteddownload)
{
	char vabuf[1024];
	...
	if (cl.loadmodel_current < cl.loadmodel_total)
	{
		...
		for (;cl.loadmodel_current < cl.loadmodel_total;cl.loadmodel_current++)
		{
			...
			else
			{
				...
				cl.model_precache[cl.loadmodel_current] = Mod_ForName(cl.model_name[cl.loadmodel_current], false, false, cl.model_name[cl.loadmodel_current][0] == '*' ? cl.model_name[1] : nullptr);
			}
			SCR_PopLoadingScreen(false);
	...
	if (cl.downloadmodel_current < cl.loadmodel_total)
	{
		...
		for (;cl.downloadmodel_current < cl.loadmodel_total;cl.downloadmodel_current++)
		{
			...
			cl.model_precache[cl.downloadmodel_current] = Mod_ForName(cl.model_name[cl.downloadmodel_current], false, true, cl.model_name[cl.downloadmodel_current][0] == '*' ? cl.model_name[1] : nullptr);
			if (cl.downloadmodel_current == 1)
			{
				// we now have the worldmodel so we can set up the game world
...
static void CL_ParseDownload()
{
	...
	int start	=	MSG_ReadLong(&cl_message);
	int size	=	static_cast<uint16>(MSG_ReadShort(&cl_message));

	// record the start/size information to ack in the next input packet
	for (size_t i = 0;i < CL_MAX_DOWNLOADACKS;i++)
	{
	...
	if (unlikely(start + size > cls.qw_downloadmemorymaxsize))
		Host_Error("corrupt download message\n");
...
static void CL_DownloadBegin_f()
{
	const int size = atoi(Cmd_Argv(1));

	if (unlikely(size < 0 || size > 1<<30 || FS_CheckNastyPath(Cmd_Argv(2), false)))
	{
...
static void CL_DownloadFinished_f()
{
	if (unlikely(Cmd_Argc() < 3))
	{
...
void CL_ValidateState(entity_state_t *s)
{
	...
	if (unlikely(s->modelindex >= MAX_MODELS))
	...
	if (unlikely(!(s->flags & RENDER_COLORMAPPED) && s->colormap > cl.maxclients))
		Con_DPrintf("CL_ValidateState: colormap (%i) > cl.maxclients (%i)\n", s->colormap, cl.maxclients);

	if (developer_extra.integer)
	{
		model = CL_GetModelByIndex(s->modelindex);
		if (unlikely(model && model->type && s->frame >= model->numframes))
			Con_DPrintf("CL_ValidateState: no such frame %i in \"%s\" (which has %i frames)\n", s->frame, model->name, model->numframes);
		if (unlikely(model && model->type && s->skin > 0 && s->skin >= model->numskins && !(s->lightpflags & PFLAGS_FULLDYNAMIC)))
...
static void CL_ParseBaseline (entity_t *ent, int large)
{
	...
	if (large)
	{
		ent->state_baseline.modelindex = static_cast<uint16>( MSG_ReadShort(&cl_message));
		ent->state_baseline.frame = static_cast<uint16>(MSG_ReadShort(&cl_message));
	}
...
==================
*/
static void CL_ParseClientdata ()
{
	...
	int bits = static_cast<uint16>(MSG_ReadShort(&cl_message));
	if (bits & SU_EXTEND1)
...
static void CL_ParseStatic (int large)
{
	...
	if (unlikely(ent->state_baseline.modelindex == 0))
	{
...
static void CL_ParseStaticSound (int large)
{
	...
	if (large || cls.protocol == PROTOCOL_NEHAHRABJP2)
		sound_num = static_cast<uint16>(MSG_ReadShort(&cl_message));
	else
		sound_num = MSG_ReadByte(&cl_message);

	if (unlikely(sound_num < 0 || sound_num >= MAX_SOUNDS))
	{
		Con_Printf("CL_ParseStaticSound: sound_num(%i) >= MAX_SOUNDS (%i)\n", sound_num, MAX_SOUNDS);
		return;
	}

	const int vol = MSG_ReadByte(&cl_message);
	const int atten = MSG_ReadByte(&cl_message);

	S_StaticSound (cl.sound_precache[sound_num], org, vol/255.0f, atten);
}

static void CL_ParseEffect ()
{
	vec3_t		org;

	MSG_ReadVector(&cl_message, org, cls.protocol);
	
	const int modelindex 	= MSG_ReadByte(&cl_message);
	const int startframe 	= MSG_ReadByte(&cl_message);
	const int framecount 	= MSG_ReadByte(&cl_message);
	const int framerate 	= MSG_ReadByte(&cl_message);

	CL_Effect(org, modelindex, startframe, framecount, framerate);
}

static void CL_ParseEffect2 ()
{
	vec3_t		org;

	MSG_ReadVector(&cl_message, org, cls.protocol);
	
	const int modelindex = static_cast<uint16>(MSG_ReadShort(&cl_message));
	const int startframe = static_cast<uint16>( MSG_ReadShort(&cl_message));
...
static void CL_ParseBeam (dp_model_t *m, int lightning)
{
	...
	int ent = static_cast<uint16>(MSG_ReadShort(&cl_message));
...
static void CL_ParseTempEntity()
{
	int type;
	vec3_t pos, pos2;
	vec3_t vel1, vel2;
	...
	if (cls.protocol == PROTOCOL_QUAKEWORLD)
	{
		...
		case QW_TE_WIZSPIKE:
			...
			CL_ParticleEffect(EFFECT_TE_WIZSPIKE, 1, pos, pos, vec3_origin, vec3_origin, nullptr, 0);
		...
		case QW_TE_KNIGHTSPIKE:
			...
			MSG_ReadVector(&cl_message, pos, cls.protocol);
			CL_FindNonSolidLocation(pos, pos, 4);
			CL_ParticleEffect(EFFECT_TE_KNIGHTSPIKE, 1, pos, pos, vec3_origin, vec3_origin, nullptr, 0);
		...
		case QW_TE_SPIKE:
			...
			MSG_ReadVector(&cl_message, pos, cls.protocol);
			CL_FindNonSolidLocation(pos, pos, 4);
			CL_ParticleEffect(EFFECT_TE_SPIKE, 1, pos, pos, vec3_origin, vec3_origin, nullptr, 0);
			if (rand() % 5)
		...
		case QW_TE_SUPERSPIKE:
			...
			MSG_ReadVector(&cl_message, pos, cls.protocol);
			CL_FindNonSolidLocation(pos, pos, 4);
			CL_ParticleEffect(EFFECT_TE_SUPERSPIKE, 1, pos, pos, vec3_origin, vec3_origin, nullptr, 0);
			if (rand() % 5)
				S_StartSound(-1, 0, cl.sfx_tink1, pos, 1, 1);
			else
			{
		...
		case QW_TE_EXPLOSION:
			...
			CL_ParticleEffect(EFFECT_TE_EXPLOSION, 1, pos, pos, vec3_origin, vec3_origin, nullptr, 0);
		...
		case QW_TE_TAREXPLOSION:
			...
			MSG_ReadVector(&cl_message, pos, cls.protocol);
			CL_FindNonSolidLocation(pos, pos, 10);
			CL_ParticleEffect(EFFECT_TE_TAREXPLOSION, 1, pos, pos, vec3_origin, vec3_origin, nullptr, 0);
			S_StartSound(-1, 0, cl.sfx_r_exp3, pos, 1, 1);
			break;
		...
		case QW_TE_GUNSHOT:
			...
			CL_ParticleEffect(EFFECT_TE_GUNSHOT, radius, pos, pos2, vec3_origin, vec3_origin, nullptr, 0);
		...
		case QW_TE_BLOOD:
			...
			MSG_ReadVector(&cl_message, pos, cls.protocol);
			CL_FindNonSolidLocation(pos, pos, 4);
			CL_ParticleEffect(EFFECT_TE_BLOOD, count, pos, pos, vec3_origin, vec3_origin, nullptr, 0);
			break;

		case QW_TE_LIGHTNINGBLOOD:
			MSG_ReadVector(&cl_message, pos, cls.protocol);
			CL_FindNonSolidLocation(pos, pos, 4);
			CL_ParticleEffect(EFFECT_TE_BLOOD, 2.5, pos, pos, vec3_origin, vec3_origin, nullptr, 0);
			break;
	...
	else
	{
		...
		case TE_SPIKEQUAD:
			...
			MSG_ReadVector(&cl_message, pos, cls.protocol);
			CL_FindNonSolidLocation(pos, pos, 4);
			CL_ParticleEffect(EFFECT_TE_SPIKEQUAD, 1, pos, pos, vec3_origin, vec3_origin, nullptr, 0);
			if (rand() % 5)
				S_StartSound(-1, 0, cl.sfx_tink1, pos, 1, 1);
			else
			{
		...
		case TE_SUPERSPIKEQUAD:
			...
			MSG_ReadVector(&cl_message, pos, cls.protocol);
			CL_FindNonSolidLocation(pos, pos, 4);
			CL_ParticleEffect(EFFECT_TE_SUPERSPIKEQUAD, 1, pos, pos, vec3_origin, vec3_origin, nullptr, 0);
			if (rand() % 5)
				S_StartSound(-1, 0, cl.sfx_tink1, pos, 1, 1);
			else
			{
		...
		case TE_BLOOD:
			...
			count = MSG_ReadByte(&cl_message);
			CL_ParticleEffect(EFFECT_TE_BLOOD, count, pos, pos, dir, dir, nullptr, 0);
			break;
		case TE_SPARK:
			// spark shower
			MSG_ReadVector(&cl_message, pos, cls.protocol);
			...
			dir[2] = MSG_ReadChar(&cl_message);
			count = MSG_ReadByte(&cl_message);
			CL_ParticleEffect(EFFECT_TE_SPARK, count, pos, pos, dir, dir, nullptr, 0);
			break;
		case TE_PLASMABURN:
			MSG_ReadVector(&cl_message, pos, cls.protocol);
			CL_FindNonSolidLocation(pos, pos, 4);
			CL_ParticleEffect(EFFECT_TE_PLASMABURN, 1, pos, pos, vec3_origin, vec3_origin, nullptr, 0);
			break;
		...
		case TE_BLOODSHOWER:
			...
			vel2[1] = velspeed;
			vel2[2] = velspeed;
			CL_ParticleEffect(EFFECT_TE_BLOOD, count, pos, pos2, vel1, vel2, nullptr, 0);
			break;
		...
		case TE_GUNSHOT:
			...
			MSG_ReadVector(&cl_message, pos, cls.protocol);
			CL_FindNonSolidLocation(pos, pos, 4);
			CL_ParticleEffect(EFFECT_TE_GUNSHOT, 1, pos, pos, vec3_origin, vec3_origin, nullptr, 0);
			if(cl_sound_ric_gunshot.integer & RIC_GUNSHOT)
			{
				if (rand() % 5)
					S_StartSound(-1, 0, cl.sfx_tink1, pos, 1, 1);
		...
		case TE_GUNSHOTQUAD:
			...
			MSG_ReadVector(&cl_message, pos, cls.protocol);
			CL_FindNonSolidLocation(pos, pos, 4);
			CL_ParticleEffect(EFFECT_TE_GUNSHOTQUAD, 1, pos, pos, vec3_origin, vec3_origin, nullptr, 0);
		...
		case TE_EXPLOSIONQUAD:
			...
			MSG_ReadVector(&cl_message, pos, cls.protocol);
			CL_FindNonSolidLocation(pos, pos, 10);
			CL_ParticleEffect(EFFECT_TE_EXPLOSIONQUAD, 1, pos, pos, vec3_origin, vec3_origin, nullptr, 0);
			S_StartSound(-1, 0, cl.sfx_r_exp3, pos, 1, 1);
			break;
		...
		case TE_SMALLFLASH:
			MSG_ReadVector(&cl_message, pos, cls.protocol);
			CL_FindNonSolidLocation(pos, pos, 10);
			CL_ParticleEffect(EFFECT_TE_SMALLFLASH, 1, pos, pos, vec3_origin, vec3_origin, nullptr, 0);
			break;
		...
		case TE_CUSTOMFLASH:
			...
			color[2] = MSG_ReadByte(&cl_message) * (2.0f / 255.0f);
			Matrix4x4_CreateTranslate(&tempmatrix, pos[0], pos[1], pos[2]);
			CL_AllocLightFlash(nullptr, &tempmatrix, radius, color[0], color[1], color[2], radius / velspeed, velspeed, 0, -1, true, 1, 0.25, 1, 1, 1, LIGHTFLAG_NORMALMODE | LIGHTFLAG_REALTIMEMODE);
			break;
		...
		case TE_FLAMEJET:
			...
			count = MSG_ReadByte(&cl_message);
			CL_ParticleEffect(EFFECT_TE_FLAMEJET, count, pos, pos, dir, dir, nullptr, 0);
			break;
	...
	// LordHavoc: for compatibility with the Nehahra movie...
		case TE_LIGHTNING4NEH:
			CL_ParseBeam(Mod_ForName(MSG_ReadString(&cl_message, cl_readstring, sizeof(cl_readstring)), true, false, nullptr), false);
			break;
		...
		case TE_TEI_G3:
			...
			MSG_ReadVector(&cl_message, dir, cls.protocol);
			CL_ParticleTrail(EFFECT_TE_TEI_G3, 1, pos, pos2, dir, dir, nullptr, 0, true, true, nullptr, nullptr, 1);
			break;
		...
		case TE_TEI_SMOKE:
			...
			count = MSG_ReadByte(&cl_message);
			CL_FindNonSolidLocation(pos, pos, 4);
			CL_ParticleEffect(EFFECT_TE_TEI_SMOKE, count, pos, pos, vec3_origin, vec3_origin, nullptr, 0);
			break;

		case TE_TEI_BIGEXPLOSION:
			MSG_ReadVector(&cl_message, pos, cls.protocol);
			CL_FindNonSolidLocation(pos, pos, 10);
			CL_ParticleEffect(EFFECT_TE_TEI_BIGEXPLOSION, 1, pos, pos, vec3_origin, vec3_origin, nullptr, 0);
			S_StartSound(-1, 0, cl.sfx_r_exp3, pos, 1, 1);
			break;
		...
		case TE_TEI_PLASMAHIT:
			...
			CL_ParticleEffect(EFFECT_TE_TEI_PLASMAHIT, count, pos, pos, vec3_origin, vec3_origin, nullptr, 0);
			break;
...
static void CL_ParseTrailParticles()
{
	int entityindex;
	int effectindex;
	vec3_t start, end;
	...
	MSG_ReadVector(&cl_message, start, cls.protocol);
	MSG_ReadVector(&cl_message, end, cls.protocol);
	CL_ParticleTrail(effectindex, 1, start, end, vec3_origin, vec3_origin, entityindex > 0 ? cl.entities + entityindex : nullptr, 0, true, true, nullptr, nullptr, 1);
}

static void CL_ParsePointParticles()
{
	...
	CL_ParticleEffect(effectindex, count, origin, origin, velocity, velocity, nullptr, 0);
}

static void CL_ParsePointParticles1()
{
	int effectindex;
	vec3_t origin;
	effectindex = (unsigned short)MSG_ReadShort(&cl_message);
	MSG_ReadVector(&cl_message, origin, cls.protocol);
	CL_ParticleEffect(effectindex, 1, origin, origin, vec3_origin, vec3_origin, nullptr, 0);
}
...
static bool cl_iplog_loaded = false;
static int cl_iplog_numitems = 0;
static int cl_iplog_maxitems = 0;
static cl_iplog_item_t *cl_iplog_items;

static void CL_IPLog_Load();
static void CL_IPLog_Add(const char *address, const char *name, bool checkexisting, bool addtofile)
{
	int i;
...
static bool CL_ExaminePrintString(const char *text)
{
	int len;
	const char *t;
	...
	if (cl.parsingtextmode == CL_PARSETEXTMODE_PING)
	{
		...
		bool expected = cl.parsingtextexpectingpingforscores != 0;
		cl.parsingtextexpectingpingforscores = 0;
		cl.parsingtextmode = CL_PARSETEXTMODE_NONE;
		t = text;
		while (*t == ' ')
...
//
	...
	if (cls.protocol == PROTOCOL_QUAKEWORLD)
	{
		...
		while (1)
		{
			...
			{
				// LordHavoc: fix for bizarre problem in MSVC that I do not understand (if I assign the string pointer directly it ends up storing a nullptr pointer)
				temp = "<unknown>";
			...
			case svc_precache:
				...
				else
				{
					...
					if (i < 32768)
					{
						if (i >= 1 && i < MAX_MODELS)
						{
							dp_model_t *model = Mod_ForName(s, false, false, s[0] == '*' ? cl.model_name[1] : nullptr);
							if (!model)
