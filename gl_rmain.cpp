...
#if RMAIN_MAY_ASSUME
	#define	r_assume(x)		__builtin_assume(x)
...
static bool r_loadnormalmap;
static bool r_loadgloss;
bool r_loadfog;
static bool r_loaddds;
static bool r_savedds;
static bool r_gpuskeletal;

//
// screen size info
//
r_refdef_t r_refdef;
#include "renderer/main/rmain_cvars.hpp"

extern bool v_flipped_state;
...
static void R_BuildBlankTextures()
{
	...
	data[0] = 255; // normal Z
	data[3] = 255; // height
	r_texture_blanknormalmap = R_LoadTexture2D(r_main_texturepool, "blankbump", 1, 1, data, TEXTYPE_BGRA, TEXF_PERSISTENT, -1, nullptr);
	data[0] = 255;
	data[1] = 255;
	data[2] = 255;
	data[3] = 255;
	r_texture_white = R_LoadTexture2D(r_main_texturepool, "blankwhite", 1, 1, data, TEXTYPE_BGRA, TEXF_PERSISTENT, -1, nullptr);
	data[0] = 128;
	data[1] = 128;
	data[2] = 128;
	data[3] = 255;
	r_texture_grey128 = R_LoadTexture2D(r_main_texturepool, "blankgrey128", 1, 1, data, TEXTYPE_BGRA, TEXF_PERSISTENT, -1, nullptr);
	data[0] = 0;
	data[1] = 0;
	data[2] = 0;
	data[3] = 255;
	r_texture_black = R_LoadTexture2D(r_main_texturepool, "blankblack", 1, 1, data, TEXTYPE_BGRA, TEXF_PERSISTENT, -1, nullptr);
}

static void R_BuildNoTexture()
{
	int x, y;
	...
	r_texture_notexture = R_LoadTexture2D(r_main_texturepool, "notexture", 16, 16, &pix[0][0][0], TEXTYPE_BGRA, TEXF_MIPMAP | TEXF_PERSISTENT, -1, nullptr);
}

static void R_BuildWhiteCube()
{
	unsigned char data[6*1*1*4];
	memset(data, 255, sizeof(data));
	r_texture_whitecube = R_LoadTextureCubeMap(r_main_texturepool, "whitecube", 1, data, TEXTYPE_BGRA, TEXF_CLAMP | TEXF_PERSISTENT, -1, nullptr);
}

static void R_BuildNormalizationCube()
{
	int x, y, side;
	vec3_t v;
	vec_t s, t, intensity;
#define NORMSIZE 64
	...
	for (side = 0;side < 6;side++)
	{
		for (y = 0;y < NORMSIZE;y++)
		{
			for (x = 0;x < NORMSIZE;x++)
			{
				...
				intensity = 127.0f / math::sqrtf(DotProduct(v, v));
	...
	r_texture_normalizationcube = R_LoadTextureCubeMap(r_main_texturepool, "normalcube", NORMSIZE, data, TEXTYPE_BGRA, TEXF_CLAMP | TEXF_PERSISTENT, -1, nullptr);
	Mem_Free(data);
}

static void R_BuildFogTexture()
{
	int x, b;
#define FOGWIDTH 256
	...
	if (r_texture_fogattenuation)
		...
		r_texture_fogattenuation = R_LoadTexture2D(r_main_texturepool, "fogattenuation", FOGWIDTH, 1, &data1[0][0], TEXTYPE_BGRA, TEXF_FORCELINEAR | TEXF_CLAMP | TEXF_PERSISTENT, -1, nullptr);
	
}

static void R_BuildFogHeightTexture()
{
	...
	r_texture_fogheighttexture = R_LoadTexture2D(r_main_texturepool, "fogheighttable", size, size, r_refdef.fog_height_table2d, TEXTYPE_BGRA, TEXF_ALPHA | TEXF_CLAMP, -1, nullptr);
}
...
char *glslshaderstring = nullptr;
char *hlslshaderstring = nullptr;
...
extern bool r_shadow_shadowmapsampler;
extern int r_shadow_shadowmappcf;
bool R_CompileShader_CheckStaticParms()
{
...
static char *R_GetShaderText(const char *filename, bool printfromdisknotice, bool builtinonly)
{
	char *shaderstring;
	...
	if (!strcmp(filename, "glsl/default.glsl"))
	{
		...
		if (!glslshaderstring)
		{
			glslshaderstring = (char *)FS_LoadFile(filename, r_main_mempool, false, nullptr);
			if (glslshaderstring)
	...
	if (!strcmp(filename, "hlsl/default.hlsl"))
	{
		...
		if (!hlslshaderstring)
		{
			hlslshaderstring = (char *)FS_LoadFile(filename, r_main_mempool, false, nullptr);
			if (hlslshaderstring)
				Con_DPrintf("Loading shaders from file %s...\n", filename);
			else
	...
	if (builtinonly)
		return nullptr;
	shaderstring = (char *)FS_LoadFile(filename, r_main_mempool, false, nullptr);
	if (shaderstring)
	{
...
static void R_SetupShader_SetPermutationGLSL(unsigned int mode, unsigned int permutation)
{
	...
	if (r_glsl_permutation->loc_ModelViewProjectionMatrix >= 0)
		qglUniformMatrix4fv(r_glsl_permutation->loc_ModelViewProjectionMatrix, 1, false, gl_modelviewprojection16f);
		
	if (r_glsl_permutation->loc_ModelViewMatrix >= 0) 
		qglUniformMatrix4fv(r_glsl_permutation->loc_ModelViewMatrix, 1, false, gl_modelview16f);
...
static void R_HLSL_CacheShader(r_hlsl_permutation_t *p, const char *cachename, const char *vertstring, const char *fragstring)
{
	...
	char vabuf[1024];
	bool debugshader = gl_paranoid.integer != 0;
		...
		if (Sys_LoadLibrary(dllnames_d3dx9, &d3dx9_dll, d3dx9_dllfuncs))
		{
			...
			if (vertstring && vertstring[0])
			{
				if (debugshader)
				{
					...
					vsresult = qD3DXCompileShaderFromFileA(va(vabuf, sizeof(vabuf), "%s/%s_vs.fx", fs_gamedir, cachename), nullptr, nullptr, "main", vsversion, shaderflags, &vsbuffer, &vslog, &vsconstanttable);
				}
				else
					vsresult = qD3DXCompileShader(vertstring, (unsigned int)strlen(vertstring), nullptr, nullptr, "main", vsversion, shaderflags, &vsbuffer, &vslog, &vsconstanttable);
				if (vsbuffer)
				{
			...
			if (fragstring && fragstring[0])
			{
				if (debugshader)
				{
					...
					psresult = qD3DXCompileShaderFromFileA(va(vabuf, sizeof(vabuf), "%s/%s_ps.fx", fs_gamedir, cachename), nullptr, nullptr, "main", psversion, shaderflags, &psbuffer, &pslog, &psconstanttable);
				}
				else
					psresult = qD3DXCompileShader(fragstring, (unsigned int)strlen(fragstring), nullptr, nullptr, "main", psversion, shaderflags, &psbuffer, &pslog, &psconstanttable);
				if (psbuffer)
				{
...
static void R_HLSL_CompilePermutation(r_hlsl_permutation_t *p, unsigned int mode, unsigned int permutation)
{
	...
	if (p->compiled)
		return;
	p->compiled = true;
	p->vertexshader = nullptr;
	p->pixelshader = nullptr;
...
void R_GLSL_Restart_f()
{
	unsigned int i, limit;
	if (glslshaderstring)
		Mem_Free(glslshaderstring);
	glslshaderstring = nullptr;
	if (hlslshaderstring)
		Mem_Free(hlslshaderstring);
	hlslshaderstring = nullptr;
...
static void R_GLSL_DumpShader_f()
{
...
void R_SetupShader_Generic(rtexture_t *first, rtexture_t *second, int texturemode, int rgbscale, bool usegamma, bool notrippy, bool suppresstexalpha)
{
...
void R_SetupShader_Generic_NoTexture(bool usegamma, bool notrippy)
{
	R_SetupShader_Generic(nullptr, nullptr, GL_MODULATE, 1, usegamma, notrippy, false);
}

void R_SetupShader_DepthOrShadow(bool notrippy, bool depthrgb, bool skeletal)
{
	unsigned int permutation = 0;
	#if !NO_SILLY_CVARS
		if (r_trippy.integer && !notrippy)
...
#ifndef USE_GLES2 /* FIXME: GLES3 only */
		if (r_glsl_permutation->ubiloc_Skeletal_Transform12_UniformBlock >= 0 && rsurface.batchskeletaltransform3x4buffer) 
			qglBindBufferRange(GL_UNIFORM_BUFFER, r_glsl_permutation->ubibind_Skeletal_Transform12_UniformBlock, rsurface.batchskeletaltransform3x4buffer->bufferobject, rsurface.batchskeletaltransform3x4offset, rsurface.batchskeletaltransform3x4size);
#endif
		break;
	case RENDERPATH_GL13:
	case RENDERPATH_GLES1:
...
extern bool r_shadow_usingdeferredprepass;
extern rtexture_t *r_shadow_attenuationgradienttexture;
extern rtexture_t *r_shadow_attenuation2dtexture;
extern rtexture_t *r_shadow_attenuation3dtexture;
extern bool r_shadow_usingshadowmap2d;
extern bool r_shadow_usingshadowmaportho;
extern float r_shadow_shadowmap_texturescale[2];
extern float r_shadow_shadowmap_parameters[4];
extern bool r_shadow_shadowmapvsdct;
...
void R_SetupShader_Surface(const vec3_t lightcolorbase, bool modellighting, float ambientscale, float diffusescale, float specularscale, rsurfacepass_t rsurfacepass, int texturenumsurfaces, const msurface_t **texturesurfacelist, void *surfacewaterplane, bool notrippy)
{
	...
	case RENDERPATH_GLES2:
		if (!vid.useinterleavedarrays)
		{
			...
			R_Mesh_TexCoordPointer(5, 2, GL_FLOAT, sizeof(float[2]), nullptr, nullptr, 0);
...
void R_SkinFrame_PrepareForPurge()
{
...
{
	for (size_t i = 0; i < SKINFRAME_HASH; i++)
	{
		for (skinframe_t *s = r_skinframe.hash[i];s;s = s->next)
		{
			if (s->loadsequence && s->loadsequence != r_skinframe.loadsequence)
			{
				...
				R_PurgeTexture(s->stain );s->stain  = nullptr;
				R_PurgeTexture(s->merged);s->merged = nullptr;
				R_PurgeTexture(s->base  );s->base   = nullptr;
				R_PurgeTexture(s->pants );s->pants  = nullptr;
				R_PurgeTexture(s->shirt );s->shirt  = nullptr;
				R_PurgeTexture(s->nmap  );s->nmap   = nullptr;
				R_PurgeTexture(s->gloss );s->gloss  = nullptr;
				R_PurgeTexture(s->glow  );s->glow   = nullptr;
				R_PurgeTexture(s->fog   );s->fog    = nullptr;
				R_PurgeTexture(s->reflect);s->reflect = nullptr;
				s->loadsequence = 0;
			}
		}
	}
...
skinframe_t *R_SkinFrame_Find(const char *name, int textureflags, int comparewidth, int compareheight, int comparecrc, bool add)
{
	skinframe_t *item;
	int hashindex;
	char basename[MAX_QPATH];
	...
	if (!item) {
		...
		item->base = dyntexture; // either nullptr or dyntexture handle
	...
	else if (textureflags & TEXF_FORCE_RELOAD)
	{
		...
		if (item->merged == item->base)
			item->merged = nullptr;
		// FIXME: maybe pass a pointer to the pointer to R_PurgeTexture and reset it to nullptr inside? [11/29/2007 Black]
		R_PurgeTexture(item->stain );item->stain  = nullptr;
		R_PurgeTexture(item->merged);item->merged = nullptr;
		R_PurgeTexture(item->base  );item->base   = nullptr;
		R_PurgeTexture(item->pants );item->pants  = nullptr;
		R_PurgeTexture(item->shirt );item->shirt  = nullptr;
		R_PurgeTexture(item->nmap  );item->nmap   = nullptr;
		R_PurgeTexture(item->gloss );item->gloss  = nullptr;
		R_PurgeTexture(item->glow  );item->glow   = nullptr;
		R_PurgeTexture(item->fog   );item->fog    = nullptr;
	R_PurgeTexture(item->reflect);item->reflect = nullptr;
		item->loadsequence = 0;
	}
	else if( item->base == nullptr )
	{
		rtexture_t *dyntexture;
		// check whether its a dynamic texture
		// this only needs to be done because Purge doesnt delete skinframes - only sets the texture pointers to nullptr and we need to restore it before returing.. [11/29/2007 Black]
		dyntexture = CL_GetDynTexture( basename );
		item->base = dyntexture; // either nullptr or dyntexture handle
	}
...
skinframe_t *R_SkinFrame_LoadExternal(const char *name, int textureflags, bool complain)
{
	int j;
	unsigned char *pixels;
	unsigned char *bumppixels;
	unsigned char *basepixels = nullptr;
	int basepixels_width = 0;
	int basepixels_height = 0;
	skinframe_t *skinframe;
	rtexture_t *ddsbase = nullptr;
	...
	skinframe->stain = nullptr;
	skinframe->merged = nullptr;
	skinframe->base = nullptr;
	skinframe->pants = nullptr;
	skinframe->shirt = nullptr;
	skinframe->nmap = nullptr;
	skinframe->gloss = nullptr;
	skinframe->glow = nullptr;
	skinframe->fog = nullptr;
	skinframe->reflect = nullptr;
	...
	if (ddsbase)
	{
		...
		if (r_loadfog && skinframe->hasalpha)
			skinframe->fog = R_LoadTextureDDSFile(r_main_texturepool, va(vabuf, sizeof(vabuf), "dds/%s_mask.dds", skinframe->basename), false, textureflags | TEXF_ALPHA, nullptr, nullptr, miplevel, true);
	}
	else
	{
		basepixels_width = image_width;
		basepixels_height = image_height;
		skinframe->base = R_LoadTexture2D (r_main_texturepool, skinframe->basename, basepixels_width, basepixels_height, basepixels, vid.sRGB3D ? TEXTYPE_SRGB_BGRA : TEXTYPE_BGRA, textureflags & (gl_texturecompression_color.integer && gl_texturecompression.integer ? ~0 : ~TEXF_COMPRESS), miplevel, nullptr);
		if (textureflags & TEXF_ALPHA)
		{
			...
			if (r_loadfog && skinframe->hasalpha)
			{
				...
				skinframe->fog = R_LoadTexture2D (r_main_texturepool, va(vabuf, sizeof(vabuf), "%s_mask", skinframe->basename), image_width, image_height, pixels, TEXTYPE_BGRA, textureflags & (gl_texturecompression_color.integer && gl_texturecompression.integer ? ~0 : ~TEXF_COMPRESS), miplevel, nullptr);
				Mem_Free(pixels);
			}
		}
	...
	if (r_loaddds)
	{
		...
		if (r_loadnormalmap)
			skinframe->nmap = R_LoadTextureDDSFile(r_main_texturepool, va(vabuf, sizeof(vabuf), "dds/%s_norm.dds", skinframe->basename), false, (TEXF_ALPHA | textureflags) & (r_mipnormalmaps.integer ? ~0 : ~TEXF_MIPMAP), nullptr, nullptr, mymiplevel, true);
		skinframe->glow = R_LoadTextureDDSFile(r_main_texturepool, va(vabuf, sizeof(vabuf), "dds/%s_glow.dds", skinframe->basename), vid.sRGB3D, textureflags, nullptr, nullptr, mymiplevel, true);
		if (r_loadgloss)
			skinframe->gloss = R_LoadTextureDDSFile(r_main_texturepool, va(vabuf, sizeof(vabuf), "dds/%s_gloss.dds", skinframe->basename), vid.sRGB3D, textureflags, nullptr, nullptr, mymiplevel, true);
		skinframe->pants = R_LoadTextureDDSFile(r_main_texturepool, va(vabuf, sizeof(vabuf), "dds/%s_pants.dds", skinframe->basename), vid.sRGB3D, textureflags, nullptr, nullptr, mymiplevel, true);
		skinframe->shirt = R_LoadTextureDDSFile(r_main_texturepool, va(vabuf, sizeof(vabuf), "dds/%s_shirt.dds", skinframe->basename), vid.sRGB3D, textureflags, nullptr, nullptr, mymiplevel, true);
		skinframe->reflect = R_LoadTextureDDSFile(r_main_texturepool, va(vabuf, sizeof(vabuf), "dds/%s_reflect.dds", skinframe->basename), vid.sRGB3D, textureflags, nullptr, nullptr, mymiplevel, true);
	}

	// _norm is the name used by tenebrae and has been adopted as standard
	if (r_loadnormalmap && skinframe->nmap == nullptr)
	{
		mymiplevel = savemiplevel;
		if ((pixels = loadimagepixelsbgra(va(vabuf, sizeof(vabuf), "%s_norm", skinframe->basename), false, false, false, &mymiplevel)) != nullptr)
		{
			skinframe->nmap = R_LoadTexture2D (r_main_texturepool, va(vabuf, sizeof(vabuf), "%s_nmap", skinframe->basename), image_width, image_height, pixels, TEXTYPE_BGRA, (TEXF_ALPHA | textureflags) & (r_mipnormalmaps.integer ? ~0 : ~TEXF_MIPMAP) & (gl_texturecompression_normal.integer && gl_texturecompression.integer ? ~0 : ~TEXF_COMPRESS), mymiplevel, nullptr);
			Mem_Free(pixels);
			pixels = nullptr;
		}
		else if (r_shadow_bumpscale_bumpmap.value > 0 && (bumppixels = loadimagepixelsbgra(va(vabuf, sizeof(vabuf), "%s_bump", skinframe->basename), false, false, false, &mymiplevel)) != nullptr)
		{
			pixels = (unsigned char *)Mem_Alloc(tempmempool, image_width * image_height * 4);
	...
	mymiplevel = savemiplevel;
	if (skinframe->glow == nullptr && ((pixels = loadimagepixelsbgra(va(vabuf, sizeof(vabuf), "%s_glow",  skinframe->basename), false, false, false, &mymiplevel)) || (pixels = loadimagepixelsbgra(va(vabuf, sizeof(vabuf), "%s_luma", skinframe->basename), false, false, false, &mymiplevel))))
	{
		skinframe->glow = R_LoadTexture2D (r_main_texturepool, va(vabuf, sizeof(vabuf), "%s_glow", skinframe->basename), image_width, image_height, pixels, vid.sRGB3D ? TEXTYPE_SRGB_BGRA : TEXTYPE_BGRA, textureflags & (gl_texturecompression_glow.integer && gl_texturecompression.integer ? ~0 : ~TEXF_COMPRESS), mymiplevel, nullptr);
#ifndef USE_GLES2
...
#endif
		Mem_Free(pixels);pixels = nullptr;
	}

	mymiplevel = savemiplevel;
	if (skinframe->gloss == nullptr && r_loadgloss && (pixels = loadimagepixelsbgra(va(vabuf, sizeof(vabuf), "%s_gloss", skinframe->basename), false, false, false, &mymiplevel)))
	{
		skinframe->gloss = R_LoadTexture2D (r_main_texturepool, va(vabuf, sizeof(vabuf), "%s_gloss", skinframe->basename), image_width, image_height, pixels, vid.sRGB3D ? TEXTYPE_SRGB_BGRA : TEXTYPE_BGRA, (TEXF_ALPHA | textureflags) & (gl_texturecompression_gloss.integer && gl_texturecompression.integer ? ~0 : ~TEXF_COMPRESS), mymiplevel, nullptr);
#ifndef USE_GLES2
	...
	mymiplevel = savemiplevel;
	if (skinframe->pants == nullptr && (pixels = loadimagepixelsbgra(va(vabuf, sizeof(vabuf), "%s_pants", skinframe->basename), false, false, false, &mymiplevel)))
	{
		skinframe->pants = R_LoadTexture2D (r_main_texturepool, va(vabuf, sizeof(vabuf), "%s_pants", skinframe->basename), image_width, image_height, pixels, vid.sRGB3D ? TEXTYPE_SRGB_BGRA : TEXTYPE_BGRA, textureflags & (gl_texturecompression_color.integer && gl_texturecompression.integer ? ~0 : ~TEXF_COMPRESS), mymiplevel, nullptr);
#ifndef USE_GLES2
	...
	mymiplevel = savemiplevel;
	if (skinframe->shirt == nullptr && (pixels = loadimagepixelsbgra(va(vabuf, sizeof(vabuf), "%s_shirt", skinframe->basename), false, false, false, &mymiplevel)))
	{
		skinframe->shirt = R_LoadTexture2D (r_main_texturepool, va(vabuf, sizeof(vabuf), "%s_shirt", skinframe->basename), image_width, image_height, pixels, vid.sRGB3D ? TEXTYPE_SRGB_BGRA : TEXTYPE_BGRA, textureflags & (gl_texturecompression_color.integer && gl_texturecompression.integer ? ~0 : ~TEXF_COMPRESS), mymiplevel, nullptr);
#ifndef USE_GLES2
	...
	mymiplevel = savemiplevel;
	if (skinframe->reflect == nullptr && (pixels = loadimagepixelsbgra(va(vabuf, sizeof(vabuf), "%s_reflect", skinframe->basename), false, false, false, &mymiplevel)))
	{
		skinframe->reflect = R_LoadTexture2D (r_main_texturepool, va(vabuf, sizeof(vabuf), "%s_reflect", skinframe->basename), image_width, image_height, pixels, vid.sRGB3D ? TEXTYPE_SRGB_BGRA : TEXTYPE_BGRA, textureflags & (gl_texturecompression_reflectmask.integer && gl_texturecompression.integer ? ~0 : ~TEXF_COMPRESS), mymiplevel, nullptr);
#ifndef USE_GLES2
...
skinframe_t *R_SkinFrame_LoadInternalBGRA(const char *name, int textureflags, const unsigned char *skindata, int width, int height, bool sRGB)
{
	int i;
	...
	if (!skindata)
		return nullptr;
	#if DEVELOPER_CVARS
	if (unlikely(developer_loading.integer))
	...
	if (r_loadnormalmap && r_shadow_bumpscale_basetexture.value > 0)
	{
		...
		skinframe->nmap = R_LoadTexture2D(r_main_texturepool, va(vabuf, sizeof(vabuf), "%s_nmap", skinframe->basename), width, height, temp2, TEXTYPE_BGRA, (textureflags | TEXF_ALPHA) & (r_mipnormalmaps.integer ? ~0 : ~TEXF_MIPMAP), -1, nullptr);
		Mem_Free(temp1);
	}
	skinframe->base = skinframe->merged = R_LoadTexture2D(r_main_texturepool, skinframe->basename, width, height, skindata, sRGB ? TEXTYPE_SRGB_BGRA : TEXTYPE_BGRA, textureflags, -1, nullptr);
	if (textureflags & TEXF_ALPHA)
	{
		...
		if (r_loadfog && skinframe->hasalpha)
		{
			...
			skinframe->fog = R_LoadTexture2D(r_main_texturepool, va(vabuf, sizeof(vabuf), "%s_fog", skinframe->basename), width, height, fogpixels, TEXTYPE_BGRA, textureflags, -1, nullptr);
			Mem_Free(fogpixels);
		}
	}
...
static void R_SkinFrame_GenerateTexturesFromQPixels(skinframe_t *skinframe, bool colormapped)
{
	int width;
	int height;
	...
	if (skinframe->qgeneratenmap)
	{
		...
		skinframe->nmap = R_LoadTexture2D(r_main_texturepool, va(vabuf, sizeof(vabuf), "%s_nmap", skinframe->basename), width, height, temp2, TEXTYPE_BGRA, (skinframe->textureflags | TEXF_ALPHA) & (r_mipnormalmaps.integer ? ~0 : ~TEXF_MIPMAP), -1, nullptr);
		Mem_Free(temp1);
	}
...
static rtexture_t *R_LoadCubemap(const char *basename)
{
	...
	for (j = 0;j < 3 && !cubemappixels;j++)
	{
		...
		for (i = 0;i < 6;i++)
		{
			...
			// load it
			if ((image_buffer = loadimagepixelsbgra(name, false, false, false, nullptr)))
			{
				// an image loaded, make sure width and height are equal
				if (likely(image_width == image_height && (!cubemappixels || image_width == cubemapsize)))
				{
	...
	if (likely(cubemappixels != nullptr))
	{
		#if DEVELOPER_CVARS
		if (unlikely(developer_loading.integer))
			Con_Printf("loading cubemap \"%s\"\n", basename);
		#endif
		cubemaptexture = R_LoadTextureCubeMap(r_main_texturepool, basename, cubemapsize, cubemappixels, vid.sRGB3D ? TEXTYPE_SRGB_BGRA : TEXTYPE_BGRA, (gl_texturecompression_lightcubemaps.integer && gl_texturecompression.integer ? TEXF_COMPRESS : 0) | TEXF_FORCELINEAR | TEXF_CLAMP, -1, nullptr);
...
rtexture_t *R_GetCubemap(const char *basename)
{
	...
	for (i = 0;i < r_texture_numcubemaps;i++)
		if (r_texture_cubemaps[i] != nullptr)
...
static void R_Main_FreeViewCache()
{
...
static void R_Main_ResizeViewCache(RenderState* renderState)
{
	int numentities = renderState->scene.numentities;
	int numclusters = renderState->scene.worldmodel
	? renderState->scene.worldmodel->brush.num_pvsclusters : 1;
	int numclusterbytes =
	renderState->scene.worldmodel
	? renderState->scene.worldmodel->brush.num_pvsclusterbytes : 1;
	int numleafs = renderState->scene.worldmodel
	?
	renderState->scene.worldmodel->brush.num_leafs : 1;
	int numsurfaces =
	renderState->scene.worldmodel
	? renderState->scene.worldmodel->num_surfaces : 1;
	if (renderState->viewcache.maxentities < numentities)
	{
		renderState->viewcache.maxentities = numentities;
		if (renderState->viewcache.entityvisible)
			Mem_Free(renderState->viewcache.entityvisible);
		renderState->viewcache.entityvisible = (unsigned char *)Mem_Alloc(r_main_mempool, renderState->viewcache.maxentities);
	}
	if (renderState->viewcache.world_numclusters != numclusters)
	{
		renderState->viewcache.world_numclusters = numclusters;
		renderState->viewcache.world_numclusterbytes = numclusterbytes;
		if (renderState->viewcache.world_pvsbits)
			Mem_Free(renderState->viewcache.world_pvsbits);
		renderState->viewcache.world_pvsbits = (unsigned char *)Mem_Alloc(r_main_mempool, renderState->viewcache.world_numclusterbytes);
	}
	if (renderState->viewcache.world_numleafs != numleafs)
	{
		renderState->viewcache.world_numleafs = numleafs;
		if (renderState->viewcache.world_leafvisible)
			Mem_Free(renderState->viewcache.world_leafvisible);
		renderState->viewcache.world_leafvisible = (unsigned char *)Mem_Alloc(r_main_mempool, renderState->viewcache.world_numleafs);
	}
	if (renderState->viewcache.world_numsurfaces != numsurfaces)
	{
		renderState->viewcache.world_numsurfaces = numsurfaces;
		if (renderState->viewcache.world_surfacevisible)
			Mem_Free(renderState->viewcache.world_surfacevisible);
		renderState->viewcache.world_surfacevisible = (unsigned char *)Mem_Alloc(r_main_mempool, renderState->viewcache.world_numsurfaces);
	}
}

extern rtexture_t *loadingscreentexture;
static void gl_main_start()
{
	loadingscreentexture = nullptr;
	r_texture_blanknormalmap = nullptr;
	r_texture_white = nullptr;
	r_texture_grey128 = nullptr;
	r_texture_black = nullptr;
	r_texture_whitecube = nullptr;
	r_texture_normalizationcube = nullptr;
	r_texture_fogattenuation = nullptr;
	r_texture_fogheighttexture = nullptr;
	r_texture_gammaramps = nullptr;
	...
	r_texture_fogheighttexture = nullptr;
	r_texture_gammaramps = nullptr;
	//r_texture_fogintensity = nullptr;
...
static void gl_main_newmap()
{
	...
	if (cl.worldmodel)
	{
		...
		if ((entities = (char *)FS_LoadFile(entname, tempmempool, true, nullptr)))
		{
...
{
	int i;
	r_main_mempool = Mem_AllocPool("Renderer", 0, nullptr);
...
#endif
	R_RegisterModule("GL_Main", gl_main_start, gl_main_shutdown, gl_main_newmap, nullptr, nullptr);
}
...
void GL_Init ()
{
	...
	GL_Clear(GL_COLOR_BUFFER_BIT, nullptr, 1.0f, 128);
}
#endif
...
static void R_FrameData_Resize(bool mustgrow)
{
	size_t wantedsize;
...
static void R_BufferData_Resize(r_bufferdata_type_t type, bool mustgrow, size_t minsize)
{
	...
	if (!mem || mem->size != size || mustgrow)
	{
		...
		if (type == R_BUFFERDATA_VERTEX)
			mem->buffer = R_Mesh_CreateMeshBuffer(nullptr, mem->size, "dynamicbuffervertex", false, false, true, false);
		else if (type == R_BUFFERDATA_INDEX16)
			mem->buffer = R_Mesh_CreateMeshBuffer(nullptr, mem->size, "dynamicbufferindex16", true, false, true, true);
		else if (type == R_BUFFERDATA_INDEX32)
			mem->buffer = R_Mesh_CreateMeshBuffer(nullptr, mem->size, "dynamicbufferindex32", true, false, true, false);
		else if (type == R_BUFFERDATA_UNIFORM)
			mem->buffer = R_Mesh_CreateMeshBuffer(nullptr, mem->size, "dynamicbufferuniform", false, true, true, false);
		mem->purge = r_bufferdata_buffer[r_bufferdata_cycle][type];
		r_bufferdata_buffer[r_bufferdata_cycle][type] = mem;
	}
}

void R_BufferData_NewFrame()
{
	int type;
	r_bufferdata_buffer_t **p, *mem;
...
r_meshbuffer_t *R_BufferData_Store(size_t datasize, const void *data, r_bufferdata_type_t type, int *returnbufferoffset)
{
	...
	*returnbufferoffset = 0;
	...
	if (offset == 0)
		R_Mesh_UpdateMeshBuffer(mem->buffer, nullptr, mem->size, false, 0);
...
void R_AnimCache_Free()
{
}

void R_AnimCache_ClearCache(RenderState* renderState)
{
	const size_t entcount = renderState->scene.numentities;
	
	for (size_t i = 0; i < entcount; i++ )
	{
		entity_render_t *ent = renderState->scene.entities[i];
		ent->animcache_vertex3f = nullptr;
		ent->animcache_vertex3f_vertexbuffer = nullptr;
		ent->animcache_vertex3f_bufferoffset = 0;
		ent->animcache_normal3f = nullptr;
		ent->animcache_normal3f_vertexbuffer = nullptr;
		ent->animcache_normal3f_bufferoffset = 0;
		ent->animcache_svector3f = nullptr;
		ent->animcache_svector3f_vertexbuffer = nullptr;
		ent->animcache_svector3f_bufferoffset = 0;
		ent->animcache_tvector3f = nullptr;
		ent->animcache_tvector3f_vertexbuffer = nullptr;
		ent->animcache_tvector3f_bufferoffset = 0;
		ent->animcache_vertexmesh = nullptr;
		ent->animcache_vertexmesh_vertexbuffer = nullptr;
		ent->animcache_vertexmesh_bufferoffset = 0;
		ent->animcache_skeletaltransform3x4 = nullptr;
		ent->animcache_skeletaltransform3x4buffer = nullptr;
...
bool R_AnimCache_GetEntity(entity_render_t *ent, bool wantnormals, bool wanttangents)
{
	...
	if (r_gpuskeletal && model->num_bones > 0 && model->surfmesh.data_skeletalindex4ub)
	{
		...
		Mod_Skeletal_BuildTransforms(model, ent->frameblend, ent->skeleton, nullptr, ent->animcache_skeletaltransform3x4); 
	...
	else if (ent->animcache_vertex3f)
	{
		...
		if (wantnormals || wanttangents)
		{
			...
			model->AnimateVertices(model, ent->frameblend, ent->skeleton, nullptr, wantnormals ? ent->animcache_normal3f : nullptr, wanttangents ? ent->animcache_svector3f : nullptr, wanttangents ? ent->animcache_tvector3f : nullptr);
...
void R_AnimCache_CacheVisibleEntities()
{
//	int i;
	bool wantnormals = true;
	bool wanttangents = !r_showsurfaces.integer;
	...
	{
		const size_t sceneEntities = r_refdef.scene.numentities;
		for (size_t i = 0; i < sceneEntities;i++)
		{
...
static void R_View_UpdateEntityLighting ()
{
	int i;
	entity_render_t *ent;
	vec3_t tempdiffusenormal, avg;
	vec_t f, fa, fd, fdd;
	bool skipunseen = r_shadows.integer != 1; //|| R_Shadow_ShadowMappingEnabled();
...
static bool R_CanSeeBox(int numsamples, vec_t enlarge, vec3_t eye, vec3_t entboxmins, vec3_t entboxmaxs)
{
	int i;
	...
	boxmins[0] = (enlarge+1.0f) * entboxmins[0] - enlarge * entboxmaxs[0];
	boxmaxs[0] = (enlarge+1.0f) * entboxmaxs[0] - enlarge * entboxmins[0];
	boxmins[1] = (enlarge+1.0f) * entboxmins[1] - enlarge * entboxmaxs[1];
	boxmaxs[1] = (enlarge+1.0f) * entboxmaxs[1] - enlarge * entboxmins[1];
	boxmins[2] = (enlarge+1.0f) * entboxmins[2] - enlarge * entboxmaxs[2];
	boxmaxs[2] = (enlarge+1.0f) * entboxmaxs[2] - enlarge * entboxmins[2];
...
static int R_DrawBrushModelsSky ()
{
	int sky = false;
	
	{
		...
		for (size_t i = 0; i < sceneEntities; i++)
		{
			if (!r_refdef.viewcache.entityvisible[i])
				continue;
			entity_render_t *RESTRICT const entity = entities[i];
			if (!entity->model || !entity->model->DrawSky)
				continue;
			entity->model->DrawSky(entity);
			sky = true;
		}
	}
	return sky;
...
static void R_DrawModels()
{
	...
	for (i = 0;i < r_refdef.scene.numentities;i++)
	{
		...
		if (ent->model && ent->model->Draw != nullptr)
			ent->model->Draw(ent);
		else
			R_DrawNoModel(ent);
	}
...
static void R_DrawModelsDepth()
{
	...
	for (i = 0;i < r_refdef.scene.numentities;i++)
	{
		if (!r_refdef.viewcache.entityvisible[i])
			continue;
		ent = r_refdef.scene.entities[i];
		if (ent->model && ent->model->DrawDepth != nullptr)
			ent->model->DrawDepth(ent);
	}
}

static void R_DrawModelsDebug()
{
	int i;
	entity_render_t *ent;
	...
	for (i = 0;i < r_refdef.scene.numentities;i++)
	{
		if (!r_refdef.viewcache.entityvisible[i])
			continue;
		ent = r_refdef.scene.entities[i];
		if (ent->model && ent->model->DrawDebug != nullptr)
			ent->model->DrawDebug(ent);
	}
}

static void R_DrawModelsAddWaterPlanes()
{
	int i;
	entity_render_t *ent;
	...
	for (i = 0;i < r_refdef.scene.numentities;i++)
	{
		if (!r_refdef.viewcache.entityvisible[i])
			continue;
		ent = r_refdef.scene.entities[i];
		if (ent->model && ent->model->DrawAddWaterPlanes != nullptr)
...
void R_HDR_UpdateIrisAdaptation(const vec3_t point)
{
	if (r_hdr_irisadaptation.integer)
	{
		...
		brightness *= 1.0f / c;
		brightness += .00001f; // make sure it's never zero
...
static void R_View_SetFrustum(RenderState* renderState, const int *scissor)
{
	...
	if(scissor != nullptr)
	{
		...
		fpx =  1.0 - 2.0 * (scissor[0]              - renderState->view.viewport.x) / (double) (renderState->view.viewport.width);
		fnx =  1.0 - 2.0 * (scissor[0] + scissor[2] - renderState->view.viewport.x) / (double) (renderState->view.viewport.width);
		...
		switch(vid.renderpath)
		{
			...
			case RENDERPATH_GLES2:
				// non-flipped y coordinates
				fny = -1.0 + 2.0 * (scissor[1]              - renderState->view.viewport.y) / (double) (renderState->view.viewport.height);
				fpy = -1.0 + 2.0 * (scissor[1] + scissor[3] - renderState->view.viewport.y) / (double) (renderState->view.viewport.height);
				break;
		}
	}

	// we can't trust r_refdef.view.forward and friends in reflected scenes
	Matrix4x4_ToVectors(&renderState->view.matrix, forward, left, up, origin);

	if (renderState->view.useperspective)
	{
		// calculate frustum corners, which are used to calculate deformed frustum planes for shadow caster culling
		VectorMAMAM(1024, forward, fnx * 1024.0 * renderState->view.frustum_x, left, fny * 1024.0 * renderState->view.frustum_y, up, renderState->view.frustumcorner[0]);
		VectorMAMAM(1024, forward, fpx * 1024.0 * renderState->view.frustum_x, left, fny * 1024.0 * renderState->view.frustum_y, up, renderState->view.frustumcorner[1]);
		VectorMAMAM(1024, forward, fnx * 1024.0 * renderState->view.frustum_x, left, fpy * 1024.0 * renderState->view.frustum_y, up, renderState->view.frustumcorner[2]);
		VectorMAMAM(1024, forward, fpx * 1024.0 * renderState->view.frustum_x, left, fpy * 1024.0 * renderState->view.frustum_y, up, renderState->view.frustumcorner[3]);

		// then the normals from the corners relative to origin
		CrossProduct(renderState->view.frustumcorner[2], renderState->view.frustumcorner[0], renderState->view.frustum[0].normal);
		CrossProduct(renderState->view.frustumcorner[1], renderState->view.frustumcorner[3], renderState->view.frustum[1].normal);
		CrossProduct(renderState->view.frustumcorner[0], renderState->view.frustumcorner[1], renderState->view.frustum[2].normal);
		CrossProduct(renderState->view.frustumcorner[3], renderState->view.frustumcorner[2], renderState->view.frustum[3].normal);
		...
		{
			VectorNegate(renderState->view.frustum[0].normal, renderState->view.frustum[0].normal);
			VectorNegate(renderState->view.frustum[1].normal, renderState->view.frustum[1].normal);
			VectorNegate(renderState->view.frustum[2].normal, renderState->view.frustum[2].normal);
			VectorNegate(renderState->view.frustum[3].normal, renderState->view.frustum[3].normal);
		}
		...
		VectorNormalize(renderState->view.frustum[0].normal);
		VectorNormalize(renderState->view.frustum[1].normal);
		VectorNormalize(renderState->view.frustum[2].normal);
		VectorNormalize(renderState->view.frustum[3].normal);

		// make the corners absolute
		VectorAdd(renderState->view.frustumcorner[0], renderState->view.origin, renderState->view.frustumcorner[0]);
		VectorAdd(renderState->view.frustumcorner[1], renderState->view.origin, renderState->view.frustumcorner[1]);
		VectorAdd(renderState->view.frustumcorner[2], renderState->view.origin, renderState->view.frustumcorner[2]);
		VectorAdd(renderState->view.frustumcorner[3], renderState->view.origin, renderState->view.frustumcorner[3]);

		// one more normal
		VectorCopy(forward, renderState->view.frustum[4].normal);

		renderState->view.frustum[0].dist = DotProduct (renderState->view.origin, renderState->view.frustum[0].normal);
		renderState->view.frustum[1].dist = DotProduct (renderState->view.origin, renderState->view.frustum[1].normal);
		renderState->view.frustum[2].dist = DotProduct (renderState->view.origin, renderState->view.frustum[2].normal);
		renderState->view.frustum[3].dist = DotProduct (renderState->view.origin, renderState->view.frustum[3].normal);
		renderState->view.frustum[4].dist = DotProduct (renderState->view.origin, renderState->view.frustum[4].normal) + renderState->nearclip;
	}
	else
	{
		VectorScale(left, -renderState->view.ortho_x, renderState->view.frustum[0].normal);
		VectorScale(left,  renderState->view.ortho_x, renderState->view.frustum[1].normal);
		VectorScale(up, -renderState->view.ortho_y, renderState->view.frustum[2].normal);
		VectorScale(up,  renderState->view.ortho_y, renderState->view.frustum[3].normal);
		VectorCopy(forward, renderState->view.frustum[4].normal);
		renderState->view.frustum[0].dist = DotProduct (renderState->view.origin, renderState->view.frustum[0].normal) + renderState->view.ortho_x;
		renderState->view.frustum[1].dist = DotProduct (renderState->view.origin, renderState->view.frustum[1].normal) + renderState->view.ortho_x;
		renderState->view.frustum[2].dist = DotProduct (renderState->view.origin, renderState->view.frustum[2].normal) + renderState->view.ortho_y;
		renderState->view.frustum[3].dist = DotProduct (renderState->view.origin, renderState->view.frustum[3].normal) + renderState->view.ortho_y;
		renderState->view.frustum[4].dist = DotProduct (renderState->view.origin, renderState->view.frustum[4].normal) + renderState->nearclip;
	}
	renderState->view.numfrustumplanes = 5;

	if (renderState->view.useclipplane)
	{
		renderState->view.numfrustumplanes = 6;
		renderState->view.frustum[5] = renderState->view.clipplane;
	}

	for (i = 0;i < renderState->view.numfrustumplanes;i++)
		PlaneClassify(renderState->view.frustum + i);
}

static void R_View_UpdateWithScissor(RenderState* renderState, const int *myscissor)
{
	R_Main_ResizeViewCache(renderState);
	R_View_SetFrustum(renderState, myscissor);
	R_View_WorldVisibility(r_refdef.view.useclipplane);
	R_View_UpdateEntityVisible();
	R_View_UpdateEntityLighting();
}

static void R_View_Update(RenderState* renderState)
{
	R_Main_ResizeViewCache(renderState);
	R_View_SetFrustum(renderState, nullptr);
	R_View_WorldVisibility(r_refdef.view.useclipplane);
	R_View_UpdateEntityVisible();
	R_View_UpdateEntityLighting();
}
...
{
	float scale = r_viewscale.value * sqrtf(viewscalefpsadjusted);
	scale = bound(0.03125f, scale, 1.0f);
	*outwidth = (int)ceil(width * scale);
	*outheight = (int)ceil(height * scale);
}

void R_SetupView(bool allowwaterclippingplane, int fbo, rtexture_t *depthtexture, rtexture_t *colortexture)
{
	const float *customclipplane = nullptr;
	float plane[4];
...
void R_ResetViewRendering2D_Common(int fbo, rtexture_t *depthtexture, rtexture_t *colortexture, float x2, float y2)
{
	...
	R_Viewport_InitOrtho(&viewport, reinterpret_cast<const matrix4x4_t*>(&identitymatrix), 
	r_refdef.view.x, vid.height - r_refdef.view.height - r_refdef.view.y, r_refdef.view.width, 
	r_refdef.view.height, 0, 0, x2, y2, -10, 100, nullptr);
	R_Mesh_SetRenderTargets(fbo, depthtexture, colortexture, nullptr, nullptr, nullptr);
...
================
*/
void R_RenderView_UpdateViewVectors()
{
...
void R_RenderScene(RenderState* renderState, int fbo, rtexture_t *depthtexture, rtexture_t *colortexture);
void R_RenderWaterPlanes(int fbo, rtexture_t *depthtexture, rtexture_t *colortexture);

static void R_Water_StartFrame(RenderState* renderState)
{
	int i;
	int waterwidth, waterheight, texturewidth, textureheight, camerawidth, cameraheight;
	r_waterstate_waterplane_t *p;
	bool usewaterfbo = (r_viewfbo.integer >= 1 || r_water_fbo.integer >= 1) && vid.support.ext_framebuffer_object && vid.support.arb_texture_non_power_of_two && vid.samples < 2;
	...
	if (r_fb.water.texturewidth)
	{
		...
		r_fb.water.waterwidth = (int)bound(1, renderState->view.width * r_water_resolutionmultiplier.value, renderState->view.width);
		r_fb.water.waterheight = (int)bound(1, renderState->view.height * r_water_resolutionmultiplier.value, renderState->view.height);
...
static void R_Water_ProcessPlanes(int fbo, rtexture_t *depthtexture, rtexture_t *colortexture)
{
	...
	for (planeindex = 0, p = r_fb.water.waterplanes;planeindex < r_fb.water.numwaterplanes;planeindex++, p++)
	{
		...
		{
			if (!p->texture_refraction)
				p->texture_refraction = R_LoadTexture2D(r_main_texturepool, va(vabuf, sizeof(vabuf), "waterplane%i_refraction", planeindex), r_fb.water.texturewidth, r_fb.water.textureheight, nullptr, r_fb.textype, TEXF_RENDERTARGET | TEXF_FORCELINEAR | TEXF_CLAMP, -1, nullptr);
			if (unlikely(!p->texture_refraction))
				goto error;
			if (usewaterfbo)
			{
				if (r_fb.water.depthtexture == nullptr)
				...
				if (p->fbo_refraction == 0)
					p->fbo_refraction = R_Mesh_CreateFramebufferObject(r_fb.water.depthtexture, p->texture_refraction, nullptr, nullptr, nullptr);
			}
		}
		else if (p->materialflags & MATERIALFLAG_CAMERA)
		{
			if (!p->texture_camera)
				p->texture_camera = R_LoadTexture2D(r_main_texturepool, va(vabuf, sizeof(vabuf), "waterplane%i_camera", planeindex), r_fb.water.camerawidth, r_fb.water.cameraheight, nullptr, r_fb.textype, TEXF_RENDERTARGET | TEXF_FORCELINEAR, -1, nullptr);
			if (unlikely(!p->texture_camera))
				goto error;
			if (usewaterfbo)
			{
				if (r_fb.water.depthtexture == nullptr)
					r_fb.water.depthtexture = R_LoadTextureRenderBuffer(r_main_texturepool, "waterviewdepth", r_fb.water.texturewidth, r_fb.water.textureheight, TEXTYPE_DEPTHBUFFER24STENCIL8);
				if (p->fbo_camera == 0)
					p->fbo_camera = R_Mesh_CreateFramebufferObject(r_fb.water.depthtexture, p->texture_camera, nullptr, nullptr, nullptr);
			}
		}
		...
		{
			if (!p->texture_reflection)
				p->texture_reflection = R_LoadTexture2D(r_main_texturepool, va(vabuf, sizeof(vabuf), "waterplane%i_reflection", planeindex), r_fb.water.texturewidth, r_fb.water.textureheight, nullptr, r_fb.textype, TEXF_RENDERTARGET | TEXF_FORCELINEAR | TEXF_CLAMP, -1, nullptr);
			if (unlikely(!p->texture_reflection))
				goto error;
			if (usewaterfbo)
			{
				if (r_fb.water.depthtexture == nullptr)
				...
				if (p->fbo_reflection == 0)
					p->fbo_reflection = R_Mesh_CreateFramebufferObject(r_fb.water.depthtexture, p->texture_reflection, nullptr, nullptr, nullptr);
			}
		}
	}
	...
	for (planeindex = 0, p = r_fb.water.waterplanes;planeindex < r_fb.water.numwaterplanes;planeindex++, p++)
	{
		if (p->materialflags & (MATERIALFLAG_WATERSHADER | MATERIALFLAG_REFLECTION))
		{
			...
			R_RenderScene(&r_refdef, p->fbo_reflection, r_fb.water.depthtexture, p->texture_reflection);
		...
		if (p->materialflags & (MATERIALFLAG_WATERSHADER | MATERIALFLAG_REFRACTION))
		{
			...
			if(r_water_scissormode.integer & 1)
				GL_Scissor(myscissor[0], myscissor[1], myscissor[2], myscissor[3]);
			R_RenderScene(&r_refdef, p->fbo_refraction, r_fb.water.depthtexture, p->texture_refraction);
...
static void R_Bloom_StartFrame(RenderState* renderState)
{
	int i;
	int bloomtexturewidth, bloomtextureheight, screentexturewidth, screentextureheight;
	int viewwidth, viewheight;
	bool useviewfbo = r_viewfbo.integer >= 1 && vid.support.ext_framebuffer_object && vid.support.arb_texture_non_power_of_two && vid.samples < 2;
	...
	case RENDERPATH_GL20:
		...
		{
			if (r_viewfbo.integer == 2)
				textype = TEXTYPE_COLORBUFFER16F;
			if (r_viewfbo.integer == 3)
				textype = TEXTYPE_COLORBUFFER32F;
		}
		break;
	case RENDERPATH_GL11:
	case RENDERPATH_GL13:
	...
	R_GetScaledViewSize(
		renderState->view.width,
		renderState->view.height,
		&viewwidth,
		&viewheight
	);
	...
	if ((r_bloom.integer || (!R_Stereo_Active() &&
	(r_motionblur.value > 0 || r_damageblur.value > 0)))
	&& ((r_bloom_resolution.integer < 4 || r_bloom_blur.value < 1
	|| r_bloom_blur.value >= 512) || renderState->view.width > (int)vid.maxtexturesize_2d
	|| renderState->view.height > (int)vid.maxtexturesize_2d))
	{
	 ...
	 || useviewfbo != (r_fb.fbo != 0))
	{
		...
		if (r_fb.screentexturewidth && r_fb.screentextureheight)
		{
			if (r_motionblur.value > 0 || r_damageblur.value > 0)
				r_fb.ghosttexture = R_LoadTexture2D(r_main_texturepool, "framebuffermotionblur", r_fb.screentexturewidth, r_fb.screentextureheight, nullptr, r_fb.textype, TEXF_RENDERTARGET | TEXF_FORCELINEAR | TEXF_CLAMP, -1, nullptr);
			r_fb.ghosttexture_valid = false;
			r_fb.colortexture = R_LoadTexture2D(r_main_texturepool, "framebuffercolor", r_fb.screentexturewidth, r_fb.screentextureheight, nullptr, r_fb.textype, TEXF_RENDERTARGET | TEXF_FORCELINEAR | TEXF_CLAMP, -1, nullptr);
			if (useviewfbo)
			{
				...
				r_fb.fbo = R_Mesh_CreateFramebufferObject(r_fb.depthtexture, r_fb.colortexture, nullptr, nullptr, nullptr);
				R_Mesh_SetRenderTargets(r_fb.fbo, r_fb.depthtexture, r_fb.colortexture, nullptr, nullptr, nullptr);
			}
		}
		...
		if (r_fb.bloomtexturewidth && r_fb.bloomtextureheight)
		{
			for (i = 0;i < (int)(sizeof(r_fb.bloomtexture)/sizeof(r_fb.bloomtexture[i]));i++)
			{
				r_fb.bloomtexture[i] = R_LoadTexture2D(r_main_texturepool, "framebufferbloom", r_fb.bloomtexturewidth, r_fb.bloomtextureheight, nullptr, r_fb.textype, TEXF_RENDERTARGET | TEXF_FORCELINEAR | TEXF_CLAMP, -1, nullptr);
				if (useviewfbo)
					r_fb.bloomfbo[i] = R_Mesh_CreateFramebufferObject(nullptr, r_fb.bloomtexture[i], nullptr, nullptr, nullptr);
			}
		}
	}

	// bloom texture is a different resolution
	r_fb.bloomwidth = bound(1, r_bloom_resolution.integer, renderState->view.width);
	r_fb.bloomheight = r_fb.bloomwidth * renderState->view.height / renderState->view.width;
	r_fb.bloomheight = bound(1, r_fb.bloomheight, renderState->view.height);
	...
	if(r_fb.fbo) 
	{
		for (i = 1;i < 8;i += 2)
		{
			r_fb.screentexcoord2f[i] += 1.0f - (float)(viewheight + renderState->view.y) / (float)r_fb.screentextureheight;
		}
	}

	// set up a texcoord array for the reduced resolution bloom image
	// (which will be additive blended over the screen image)
	r_fb.bloomtexcoord2f[0] = .0f;
	...
	r_fb.bloomtexcoord2f[5] = .0f;
	r_fb.bloomtexcoord2f[6] = .0f;
	r_fb.bloomtexcoord2f[7] = .0f;
	...
	R_Viewport_InitOrtho(&r_fb.bloomviewport, reinterpret_cast<const matrix4x4_t*>(&identitymatrix), 0, 0, r_fb.bloomwidth, r_fb.bloomheight, 0, 0, 1, 1, -10, 100, nullptr);

	if (r_fb.fbo)
		renderState->view.clear = true;
}

static void R_Bloom_MakeTexture()
{
	int x, range, dir;
	...
	CHECKGLERROR
	r_fb.bloomindex = 0;
	R_Mesh_SetRenderTargets(r_fb.bloomfbo[r_fb.bloomindex], nullptr, r_fb.bloomtexture[r_fb.bloomindex], nullptr, nullptr, nullptr);
	...
	case RENDERPATH_SOFT:
	#endif
		R_Mesh_PrepareVertices_Generic_Arrays(4, r_screenvertex3f, nullptr, r_fb.screentexcoord2f);
		break;
	...
	case RENDERPATH_D3D10:
	case RENDERPATH_D3D11:
		R_Mesh_PrepareVertices_Generic_Arrays(4, r_d3dscreenvertex3f, nullptr, r_fb.screentexcoord2f);
		break;
	#endif
	}
	// TODO: do boxfilter scale-down in shader?
	R_SetupShader_Generic(r_fb.colortexture, nullptr, GL_MODULATE, 1, false, true, true);
	...
	for (x = 1;x < min(r_bloom_colorexponent.value, 32);)
	{
		...
		if (!r_fb.bloomfbo[r_fb.bloomindex])
		{
			...
			GL_Color(r, r, r, 1.0f); // apply fix factor
		}
		else
		{
			if(x <= 2)
			...
			GL_Color(1.0f, 1.0f, 1.0f, 1.0f); // no fix factor supported here
		}
		R_Mesh_PrepareVertices_Generic_Arrays(4, r_screenvertex3f, nullptr, r_fb.bloomtexcoord2f);
		R_SetupShader_Generic(intex, nullptr, GL_MODULATE, 1, false, true, false);
		R_Mesh_Draw(0, 4, 0, 2, polygonelement3i, nullptr, 0, polygonelement3s, nullptr, 0);
		r_refdef.stats[r_stat_bloom_drawpixels] += r_fb.bloomwidth * r_fb.bloomheight;
	...
	for (dir = 0;dir < 2;dir++)
	{
		...
		for (x = -range;x <= range;x++)
		{
			...
			GL_Color(r, r, r, 1.0f);
			R_Mesh_PrepareVertices_Generic_Arrays(4, r_screenvertex3f, nullptr, r_fb.offsettexcoord2f);
			R_Mesh_Draw(0, 4, 0, 2, polygonelement3i, nullptr, 0, polygonelement3s, nullptr, 0);
			r_refdef.stats[r_stat_bloom_drawpixels] += r_fb.bloomwidth * r_fb.bloomheight;
	...
	case RENDERPATH_GLES2:
		...
		if (r_fb.colortexture)
		{
			...
			if(!R_Stereo_Active() && (r_motionblur.value > 0 || r_damageblur.value > 0) && r_fb.ghosttexture)
			{
				...
				if (cl.motionbluralpha > 0 && !r_refdef.envmap && r_fb.ghosttexture_valid)
				{
					...
					R_SetupShader_Generic(r_fb.ghosttexture, nullptr, GL_MODULATE, 1, false, true, true);
					R_Mesh_Draw(0, 4, 0, 2, polygonelement3i, nullptr, 0, polygonelement3s, nullptr, 0);
		...
		else
		{
			...
			if (r_refdef.viewblend[3] >= (1.0f / 256.0f))
			{
				...
				R_ResetViewRendering2D(0, nullptr, nullptr);
...
#define sscanf sscanf_s
		...
		R_ResetViewRendering2D(0, nullptr, nullptr); // here we render to the real framebuffer!
		GL_Color(1, 1, 1, 1);
		GL_BlendFunc(GL_ONE, GL_ZERO);
		...
		case RENDERPATH_GL20:
		case RENDERPATH_GLES2:
			R_Mesh_PrepareVertices_Mesh_Arrays(4, r_screenvertex3f, nullptr, nullptr, nullptr, nullptr, r_fb.screentexcoord2f, r_fb.bloomtexcoord2f);
...
#ifdef SUPPORTD3D
			// D3D has upside down Y coords, the easiest way to flip this is to flip the screen vertices rather than the texcoords, so we just use a different array for that...
			R_Mesh_PrepareVertices_Mesh_Arrays(4, r_d3dscreenvertex3f, nullptr, nullptr, nullptr, nullptr, r_fb.screentexcoord2f, r_fb.bloomtexcoord2f);
...
#define RAMPWIDTH 256
				...
				{
					r_texture_gammaramps = R_LoadTexture2D(r_main_texturepool, "gammaramps", RAMPWIDTH, 1, &rampbgr[0][0], TEXTYPE_BGRA, TEXF_FORCELINEAR | TEXF_CLAMP | TEXF_PERSISTENT, -1, nullptr);
				}
			}
		}
		else
...
{
	// of course, we could also add a bool that provides a lock state and a ReleaseScenePointer function..
	if( scenetype == r_currentscenetype ) {
...
static void R_SortEntities(RenderState* renderState)
{
	// below or equal 2 ents, sorting never gains anything
	if(renderState->scene.numentities <= 2)
		return;
	// sort
	qsort(renderState->scene.entities,
			renderState->scene.numentities,
	sizeof(*(renderState->scene.entities)),
	R_SortEntities_Compare);
}
...
void R_RenderView()
{
	...
	matrix4x4_t originalmatrix = renderState->view.matrix;
	...
	rsurface.entity = nullptr;
	// used only by R_GetCurrentTexture and
	//RSurf_ActiveWorldEntity/RSurf_ActiveModelEntity

	if(R_CompileShader_CheckStaticParms())
		R_GLSL_Restart_f();

	if (unlikely(r_drawentities.integer == 0))
		renderState->scene.numentities = 0;
	else if (r_sortentities.integer)
		R_SortEntities(renderState);

	R_AnimCache_ClearCache(renderState);

	/* adjust for stereo display */
	if(R_Stereo_Active())
	{
		Matrix4x4_CreateFromQuakeEntity(&offsetmatrix, 0,
		r_stereo_separation.value * (0.5f - r_stereo_side), 0, 0,
		r_stereo_angle.value * (0.5f - r_stereo_side), 0, 1);

		Matrix4x4_Concat(&renderState->view.matrix, &originalmatrix, &offsetmatrix);
	}

	if (renderState->view.isoverlay)
	{
		// TODO: FIXME: move this into its own backend function maybe? [2/5/2008 Andreas]
		R_Mesh_SetRenderTargets(0, nullptr, nullptr, nullptr, nullptr, nullptr);
		GL_Clear(GL_DEPTH_BUFFER_BIT, nullptr, 1.0f, 0);
		#if ENABLE_TIMEREPORT
		R_TimeReport("depthclear");
		#endif
		renderState->view.showdebug = false;

		r_fb.water.enabled = false;
		r_fb.water.numwaterplanes = 0;

		R_RenderScene(renderState, 0, nullptr, nullptr);

		renderState->view.matrix = originalmatrix;

		CHECKGLERROR
		return;
	}

	if (unlikely(renderState->scene.entities == 0)
	|| unlikely(renderState->view.width * renderState->view.height == 0 )
	|| !r_renderview.integer || cl_videoplaying)
	{
		renderState->view.matrix = originalmatrix;
		return;
	}

	renderState->view.colorscale = r_hdr_scenebrightness.value * r_hdr_irisadaptation_value.value;

	if(vid_sRGB.integer && vid_sRGB_fallback.integer && !vid.sRGB3D)
		// in sRGB fallback, behave similar to true sRGB: convert this
		// value from linear to sRGB
		r_refdef.view.colorscale = Image_sRGBFloatFromLinearFloat(renderState->view.colorscale);

	R_RenderView_UpdateViewVectors();

	R_Shadow_UpdateWorldLightSelection();

	R_Bloom_StartFrame(renderState);

	// apply bloom brightness offset
	if(r_fb.bloomtexture[0])
		r_refdef.view.colorscale *= r_bloom_scenebrightness.value;

	R_Water_StartFrame(renderState);
	...
	if (renderState->view.clear || renderState->fogenabled || fbo)
	{
		R_ClearScreen(renderState->fogenabled);
		#if ENABLE_TIMEREPORT
		if (r_timereport_active)
	...
	renderState->view.showdebug = true;

	R_View_Update(renderState);
	#if ENABLE_TIMEREPORT
	if (r_timereport_active)
	...
	#endif
	r_fb.water.numwaterplanes = 0;
	if (likely(r_fb.water.enabled))
		R_RenderWaterPlanes(fbo, depthtexture, colortexture);

	R_RenderScene(renderState, fbo, depthtexture, colortexture);
	r_fb.water.numwaterplanes = 0;
...
static void R_DrawLocs();
static void R_DrawEntityBBoxes();
static void R_DrawModelDecals();
...
void R_RenderScene (RenderState* renderState, int fbo, rtexture_t *depthtexture, rtexture_t *colortexture)
{
	...
	#endif
	renderState->stats[r_stat_renders]++;
	...
	if (cl.csqc_vidvars.drawworld)
	{
		...
		if (renderState->scene.worldmodel && renderState->scene.worldmodel->DrawSky)
		{
			renderState->scene.worldmodel->DrawSky(renderState->scene.worldentity);
			#if ENABLE_TIMEREPORT
			if (r_timereport_active)
	...
	if (r_depthfirst.integer >= 1 && cl.csqc_vidvars.drawworld
	&& renderState->scene.worldmodel && renderState->scene.worldmodel->DrawDepth)
	{
		renderState->scene.worldmodel->DrawDepth(renderState->scene.worldentity);
		#if ENABLE_TIMEREPORT
		if (r_timereport_active)
	...
	if (r_shadows.integer >= 2 && shadowmapping
	&& renderState->lightmapintensity > 0)
	{
		R_ResetViewRendering3D(fbo, depthtexture, colortexture);
	...
	if (cl.csqc_vidvars.drawworld && renderState->scene.worldmodel
	&& renderState->scene.worldmodel->Draw)
	{
		renderState->scene.worldmodel->Draw(renderState->scene.worldentity);
		#if ENABLE_TIMEREPORT
		if (r_timereport_active)
			R_TimeReport("world");
		#endif
	...
	(r_shadows.integer > 0 && !shadowmapping))
	&& !r_shadows_drawafterrtlighting.integer
	&& renderState->lightmapintensity > 0)
	{
		R_ResetViewRendering3D(fbo, depthtexture, colortexture);
	...
	if (unlikely(renderState->view.showdebug)
	&& renderState->scene.worldmodel
	&& renderState->scene.worldmodel->DrawDebug
	&&
	(r_showtris.value > 0 || r_shownormals.value != 0
	|| r_showcollisionbrushes.value > 0 || r_showoverdraw.value > 0))
	{
		renderState->scene.worldmodel->DrawDebug(renderState->scene.worldentity);
		#if ENABLE_TIMEREPORT
		if (r_timereport_active)
...
static void R_DrawBBoxMesh(vec3_t mins, vec3_t maxs, float cr, float cg, float cb, float ca)
{
	...
	R_Mesh_PrepareVertices_Generic_Arrays(8, vertex3f, color4f, nullptr);
	R_Mesh_ResetTextureState();
...
static void R_DrawEntityBBoxes_Callback(const entity_render_t *ent, const rtlight_t *rtlight, int numsurfaces, int *surfacelist)
{
	...
	GL_CullFace(GL_NONE);
	R_SetupShader_Generic_NoTexture(false, false);
	const size_t numSurfaces = numsurfaces;
	for ( size_t i = 0; i < numSurfaces; i++ )
	{
		float color[4];
		int solidity;
		
		prvm_edict_t *edict = PRVM_EDICT_NUM(surfacelist[i]);
		solidity = (int)PRVM_serveredictfloat(edict, solid);
	
		switch (solidity)
		{
			case SOLID_NOT:      
				Vector4Set(color, 1.0f, 1.0f, 1.0f, .05f);
				break;
			case SOLID_TRIGGER:  
				Vector4Set(color, 1.0f, .0f, 1.0f, .1f);
				break;
			case SOLID_BBOX:     
				Vector4Set(color, .0f, 1.0f, .0f, .1f);
				break;
			case SOLID_SLIDEBOX: 
				Vector4Set(color, 1.0f, .0f, .0f, .1f);
				break;
			case SOLID_BSP:      
				Vector4Set(color, .0f, .0f, 1.0f, .05f);
				break;
			case SOLID_CORPSE:   
				Vector4Set(color, 1.0f, .5f, .0f, .05f);
				break;
			default:             
				Vector4Set(color, .0f, .0f, .0f, .5f);
				break;
		}
		color[3] *= r_showbboxes.value;
		color[3] = bound(.0f, color[3], 1.0f);
...
static void R_DrawEntityBBoxes()
{
	...
	for (i = 0;i < prog->num_edicts;i++)
	{
		...
		R_MeshQueue_AddTransparent(TRANSPARENTSORT_DISTANCE, center, R_DrawEntityBBoxes_Callback, (entity_render_t *)nullptr, i, (rtlight_t *)nullptr);
	}
}
...
static void R_DrawNoModel_TransparentCallback(const entity_render_t *ent, const rtlight_t *rtlight, int numsurfaces, int *surfacelist)
{
	...
	RSurf_ActiveCustomEntity(&ent->matrix, &ent->inversematrix, ent->flags, ent->shadertime, ent->colormod[0], ent->colormod[1], ent->colormod[2], ent->alpha, 6, nomodelvertex3f, nullptr, nullptr, nullptr, nullptr, nomodelcolor4f, 8, nomodelelement3i, nomodelelement3s, false, false);
	...
	GL_DepthRange(.0f, (rsurface.ent_flags & RENDER_VIEWMODEL) ? 0.0625f : 1.0f);
	...
	R_SetupShader_Generic_NoTexture(false, false);
	R_Mesh_PrepareVertices_Generic_Arrays(6, nomodelvertex3f, color4f, nullptr);
	R_Mesh_Draw(0, 6, 0, 8, nomodelelement3i, nullptr, 0, nomodelelement3s, nullptr, 0);
}
...
void R_DrawNoModel(entity_render_t *ent)
{
	...
	if ((ent->flags & RENDER_ADDITIVE) || (ent->alpha < 1))
		...
		R_DrawNoModel_TransparentCallback(ent, rsurface.rtlight, 0, nullptr);
}
...
void R_CalcSprite_Vertex3f(float *RESTRICT vertex3f, const vec3_t origin, const vec3_t left, const vec3_t up, float scalex1, float scalex2, float scaley1, float scaley2)
{
	...
	for (planenum = 0, plane = planes;planenum < numplanes;planenum++, plane++)
	{
		...
		for (planenum2 = 0, plane2 = planes;planenum2 < numplanes && tempnumpoints >= 3;planenum2++, plane2++)
		{
			if (planenum2 == planenum)
				continue;
			PolygonD_Divide(tempnumpoints, temppoints[w], plane2->normal[0], plane2->normal[1], plane2->normal[2], plane2->dist, R_MESH_PLANE_DIST_EPSILON, 0, nullptr, nullptr, 256, temppoints[!w], &tempnumpoints, nullptr);
			w = !w;
		}
		if (tempnumpoints < 3)
			continue;
...
static void R_Texture_AddLayer(texture_t *t, bool depthmask, int blendfunc1, int blendfunc2, texturelayertype_t type, rtexture_t *texture, const matrix4x4_t *matrix, float r, float g, float b, float a)
{
...
static bool R_TestQ3WaveFunc(q3wavefunc_t func, const float *parms)
{
...
texture_t *R_GetCurrentTexture(texture_t *t)
{
	...
	dp_model_t *model = ent->model; // when calling this, ent must not be nullptr
	...
	else if ((rsurface.modeltexcoordlightmap2f == nullptr || (rsurface.ent_flags & (RENDER_DYNAMICMODELLIGHT | RENDER_CUSTOMIZEDMODELLIGHT))) && !(t->currentmaterialflags & MATERIALFLAG_FULLBRIGHT))
	{
	...
	t->pantstexture = t->colormapping ? t->currentskinframe->pants : nullptr;
	t->shirttexture = t->colormapping ? t->currentskinframe->shirt : nullptr;
	...
	if (t->backgroundnumskinframes)
	{
		...
		// make sure that if glow is going to be used, both textures are not nullptr
	...
	else
	{
		...
		t->backgroundnmaptexture = r_texture_blanknormalmap;
		t->backgroundglosstexture = r_texture_black;
		t->backgroundglowtexture = nullptr;
	}
	...
	if (gl_lightmaps.integer)
	{
		...
		if (gl_lightmaps.integer < 2)
			t->nmaptexture = r_texture_blanknormalmap;
		t->glosstexture = r_texture_black;
		t->glowtexture = nullptr;
		t->fogtexture = nullptr;
		t->reflectmasktexture = nullptr;
		t->backgroundbasetexture = nullptr;
		if (gl_lightmaps.integer < 2)
			t->backgroundnmaptexture = r_texture_blanknormalmap;
		t->backgroundglosstexture = r_texture_black;
		t->backgroundglowtexture = nullptr;
	...
	if (t->currentmaterialflags & MATERIALFLAG_WALL)
	{
		...
		if (t->glowtexture != nullptr && !gl_lightmaps.integer)
...
void RSurf_ActiveWorldEntity()
{
	...
	rsurface.entityskeletaltransform3x4 = nullptr;
	rsurface.entityskeletaltransform3x4buffer = nullptr;
	...
	rsurface.batchvertex3f  = nullptr;
	rsurface.batchvertex3f_vertexbuffer = nullptr;
	rsurface.batchvertex3f_bufferoffset = 0;
	rsurface.batchsvector3f = nullptr;
	rsurface.batchsvector3f_vertexbuffer = nullptr;
	rsurface.batchsvector3f_bufferoffset = 0;
	rsurface.batchtvector3f = nullptr;
	rsurface.batchtvector3f_vertexbuffer = nullptr;
	rsurface.batchtvector3f_bufferoffset = 0;
	rsurface.batchnormal3f  = nullptr;
	rsurface.batchnormal3f_vertexbuffer = nullptr;
	rsurface.batchnormal3f_bufferoffset = 0;
	rsurface.batchlightmapcolor4f = nullptr;
	rsurface.batchlightmapcolor4f_vertexbuffer = nullptr;
	rsurface.batchlightmapcolor4f_bufferoffset = 0;
	rsurface.batchtexcoordtexture2f = nullptr;
	rsurface.batchtexcoordtexture2f_vertexbuffer = nullptr;
	rsurface.batchtexcoordtexture2f_bufferoffset = 0;
	rsurface.batchtexcoordlightmap2f = nullptr;
	rsurface.batchtexcoordlightmap2f_vertexbuffer = nullptr;
	rsurface.batchtexcoordlightmap2f_bufferoffset = 0;
	rsurface.batchskeletalindex4ub = nullptr;
	rsurface.batchskeletalindex4ub_vertexbuffer = nullptr;
	rsurface.batchskeletalindex4ub_bufferoffset = 0;
	rsurface.batchskeletalweight4ub = nullptr;
	rsurface.batchskeletalweight4ub_vertexbuffer = nullptr;
	rsurface.batchskeletalweight4ub_bufferoffset = 0;
	rsurface.batchvertexmesh = nullptr;
	rsurface.batchvertexmesh_vertexbuffer = nullptr;
	rsurface.batchvertexmesh_bufferoffset = 0;
	rsurface.batchelement3i = nullptr;
	rsurface.batchelement3i_indexbuffer = nullptr;
	rsurface.batchelement3i_bufferoffset = 0;
	rsurface.batchelement3s = nullptr;
	rsurface.batchelement3s_indexbuffer = nullptr;
	rsurface.batchelement3s_bufferoffset = 0;
	rsurface.passcolor4f = nullptr;
	rsurface.passcolor4f_vertexbuffer = nullptr;
	rsurface.passcolor4f_bufferoffset = 0;
	rsurface.forcecurrenttextureupdate = false;
}

void RSurf_ActiveModelEntity(const entity_render_t *ent, bool wantnormals, bool wanttangents, bool prepass)
{
	dp_model_t *model = ent->model;
	...
	if (model->surfmesh.isanimated && model->AnimateVertices && !rsurface.entityskeletaltransform3x4)
	{
		if (ent->animcache_vertex3f)
		{
			...
			rsurface.modelsvector3f = wanttangents ? ent->animcache_svector3f : nullptr;
			rsurface.modelsvector3f_vertexbuffer = wanttangents ? ent->animcache_svector3f_vertexbuffer : nullptr;
			rsurface.modelsvector3f_bufferoffset = wanttangents ? ent->animcache_svector3f_bufferoffset : 0;
			rsurface.modeltvector3f = wanttangents ? ent->animcache_tvector3f : nullptr;
			rsurface.modeltvector3f_vertexbuffer = wanttangents ? ent->animcache_tvector3f_vertexbuffer : nullptr;
			rsurface.modeltvector3f_bufferoffset = wanttangents ? ent->animcache_tvector3f_bufferoffset : 0;
			rsurface.modelnormal3f = wantnormals ? ent->animcache_normal3f : nullptr;
			rsurface.modelnormal3f_vertexbuffer = wantnormals ? ent->animcache_normal3f_vertexbuffer : nullptr;
		...
		else if (wanttangents)
		{
			...
			rsurface.modelvertexmesh = nullptr;
			rsurface.modelvertexmesh_vertexbuffer = nullptr;
			rsurface.modelvertexmesh_bufferoffset = 0;
			rsurface.modelvertex3f_vertexbuffer = nullptr;
		...
		else if (wantnormals)
		{
			...
			r_refdef.stats[r_stat_batch_entityanimate_triangles] += model->surfmesh.num_triangles;
			rsurface.modelvertex3f = (float *)R_FrameData_Alloc(model->surfmesh.num_vertices * sizeof(float[3]));
			rsurface.modelsvector3f = nullptr;
			rsurface.modeltvector3f = nullptr;
			rsurface.modelnormal3f = (float *)R_FrameData_Alloc(model->surfmesh.num_vertices * sizeof(float[3]));
			model->AnimateVertices(model, rsurface.frameblend, rsurface.skeleton, rsurface.modelvertex3f, rsurface.modelnormal3f, nullptr, nullptr);
			rsurface.modelvertexmesh = nullptr;
			rsurface.modelvertexmesh_vertexbuffer = nullptr;
			rsurface.modelvertexmesh_bufferoffset = 0;
			rsurface.modelvertex3f_vertexbuffer = nullptr;
		...
		else
		{
			...
			rsurface.modelsvector3f = nullptr;
			rsurface.modeltvector3f = nullptr;
			rsurface.modelnormal3f = nullptr;
			model->AnimateVertices(model, rsurface.frameblend, rsurface.skeleton, rsurface.modelvertex3f, nullptr, nullptr, nullptr);
			rsurface.modelvertexmesh = nullptr;
			rsurface.modelvertexmesh_vertexbuffer = nullptr;
			rsurface.modelvertexmesh_bufferoffset = 0;
			rsurface.modelvertex3f_vertexbuffer = nullptr;
...
void RSurf_ActiveCustomEntity(const matrix4x4_t *matrix, const matrix4x4_t *inversematrix, int entflags, double shadertime, float r, float g, float b, float a, int numvertices, const float *vertex3f, const float *texcoord2f, const float *normal3f, const float *svector3f, const float *tvector3f, const float *color4f, int numtriangles, const int *element3i, const unsigned short *element3s, bool wantnormals, bool wanttangents)
{
	...
	rsurface.modeltexcoordlightmap2f  = nullptr;
	rsurface.modeltexcoordlightmap2f_vertexbuffer = 0;
	rsurface.modeltexcoordlightmap2f_bufferoffset = 0;
	rsurface.modelskeletalindex4ub = nullptr;
	rsurface.modelskeletalindex4ub_vertexbuffer = nullptr;
	rsurface.modelskeletalindex4ub_bufferoffset = 0;
	rsurface.modelskeletalweight4ub = nullptr;
	rsurface.modelskeletalweight4ub_vertexbuffer = nullptr;
	rsurface.modelskeletalweight4ub_bufferoffset = 0;
	rsurface.modelelement3i = (int *)element3i;
	rsurface.modelelement3i_indexbuffer = nullptr;
	rsurface.modelelement3i_bufferoffset = 0;
	rsurface.modelelement3s = (unsigned short *)element3s;
	rsurface.modelelement3s_indexbuffer = nullptr;
	rsurface.modelelement3s_bufferoffset = 0;
	rsurface.modellightmapoffsets = nullptr;
	rsurface.modelsurfaces = nullptr;
	rsurface.batchgeneratedvertex = false;
	rsurface.batchfirstvertex = 0;
	rsurface.batchnumvertices = 0;
	rsurface.batchfirsttriangle = 0;
...
static inline void RSurf_RenumberElements(const int *RESTRICT const inelement3i, int *RESTRICT const outelement3i, const size_t numelements, const int adjust)
{
...
void RSurf_PrepareVerticesForBatch(int batchneed, int texturenumsurfaces, const msurface_t **texturesurfacelist)
{
	...
	rsurface.batchmultidrawsurfacelist = nullptr;
	...
	// need actual vertex positions and normals
	{
		...
		rsurface.batchelement3s_indexbuffer = nullptr;
		rsurface.batchelement3s_bufferoffset = 0;
		rsurface.batchskeletaltransform3x4buffer = nullptr;
	...
	if (rsurface.batchskeletaltransform3x4)
	{
		...
		rsurface.batchskeletaltransform3x4 = nullptr;
...
static int RSurf_FindWaterPlaneForSurface(const msurface_t *surface)
{
	...
	for (planeindex = 0, p = r_fb.water.waterplanes;planeindex < r_fb.water.numwaterplanes;planeindex++, p++)
	{
		...
		for (vertexindex = 0, v = rsurface.batchvertex3f + rsurface.batchfirstvertex * 3;vertexindex < rsurface.batchnumvertices;vertexindex++, v += 3)
		{
			...
			d += math::fabsf(PlaneDiff(vert, &p->plane));
		}
...
static void RSurf_DrawBatch_GL11_MakeFullbrightLightmapColorArray()
{
	int i;
...
static void RSurf_DrawBatch_GL11_ApplyFogToFinishedVertexColors()
{
	...
	if (!rsurface.passcolor4f)
		return;
	const float *c = rsurface.passcolor4f + rsurface.batchfirstvertex * 4;
	rsurface.passcolor4f = (float *)R_FrameData_Alloc(rsurface.batchnumvertices * sizeof(float[4]));
	rsurface.passcolor4f_vertexbuffer = 0;
	rsurface.passcolor4f_bufferoffset = 0;
	for (i = 0, v = rsurface.batchvertex3f + rsurface.batchfirstvertex * 3, c2 = rsurface.passcolor4f + rsurface.batchfirstvertex * 4;i < rsurface.batchnumvertices;i++, v += 3, c += 4, c2 += 4)
	{
		{
			const float f = RSurf_FogVertex(v);
			c2[0] = c[0] * f + r_refdef.fogcolor[0] * (1.0f - f);
			c2[1] = c[1] * f + r_refdef.fogcolor[1] * (1.0f - f);
			c2[2] = c[2] * f + r_refdef.fogcolor[2] * (1.0f - f);
		}
		c2[3] = c[3];
	}
}
...
static void RSurf_DrawBatch_GL11_ApplyAmbient()
{
	int i;
	const float *c;
	float *c2;
...
static void RSurf_DrawBatch_GL11_Lightmap(float r, float g, float b, float a, bool applycolor, bool applyfog)
{
	// TODO: optimize
	rsurface.passcolor4f = nullptr;
	rsurface.passcolor4f_vertexbuffer = 0;
...
static void RSurf_DrawBatch_GL11_Unlit(float r, float g, float b, float a, bool applycolor, bool applyfog)
{
...
static void RSurf_DrawBatch_GL11_VertexColor(float r, float g, float b, float a, bool applycolor, bool applyfog)
{
	// TODO: optimize
...
static void RSurf_DrawBatch_GL11_ClampColor()
{
	int i;
	const float *c1;
	float *c2;
...
static void RSurf_DrawBatch_GL11_ApplyFakeLight()
{
	int i;
	float f;
	const float *v;
	...
	for (i = 0, v = rsurface.batchvertex3f + rsurface.batchfirstvertex * 3, n = rsurface.batchnormal3f + rsurface.batchfirstvertex * 3, c = rsurface.passcolor4f + rsurface.batchfirstvertex * 4;i < rsurface.batchnumvertices;i++, v += 3, n += 3, c += 4)
	{
		...
		f = max(.0f, f);
		f = f * 0.85f + 0.15f; // work around so stuff won't get black
		f *= r_refdef.lightmapintensity;
		Vector4Set(c, f, f, f, 1);
	}
}

static void RSurf_DrawBatch_GL11_FakeLight(float r, float g, float b, float a, bool applycolor, bool applyfog)
{
...
static void RSurf_DrawBatch_GL11_ApplyVertexShade(float *r, float *g, float *b, float *a, bool *applycolor)
{
	int i;
	float f;
	float alpha;
...
static void RSurf_DrawBatch_GL11_VertexShade(float r, float g, float b, float a, bool applycolor, bool applyfog)
{
...
static void RSurf_DrawBatch_GL11_MakeFogColor(float r, float g, float b, float a)
{
	...
	{
		{
			const float f = 1.0f - RSurf_FogVertex(v);
			c[3] = f * a;
		}
		c[0] = r;
		c[1] = g;
...
void RSurf_SetupDepthAndCulling()
{
...
extern rtexture_t *r_shadow_prepasslightingdiffusetexture;
extern rtexture_t *r_shadow_prepasslightingspeculartexture;
static void R_DrawTextureSurfaceList_GL20(int texturenumsurfaces, const msurface_t **texturesurfacelist, bool writedepth, bool prepass)
{
	...
	if (prepass)
	{
		...
		GL_DepthMask(true);
		R_SetupShader_Surface(vec3_origin, (rsurface.texture->currentmaterialflags & MATERIALFLAG_MODELLIGHT) != 0, 1, 1, rsurface.texture->specularscale, RSURFPASS_DEFERREDGEOMETRY, texturenumsurfaces, texturesurfacelist, nullptr, false);
		RSurf_DrawBatch();
		return;
	}
	...
	if ((rsurface.texture->currentmaterialflags & (MATERIALFLAG_WATERSHADER | MATERIALFLAG_REFRACTION | MATERIALFLAG_CAMERA | MATERIALFLAG_REFLECTION)))
	{
		...
		for (start = 0;start < texturenumsurfaces;start = end)
		{
			...
			if ((rsurface.texture->currentmaterialflags & (MATERIALFLAG_WATERSHADER | MATERIALFLAG_REFRACTION | MATERIALFLAG_CAMERA)))
			{
				...
				GL_DepthMask(false);
				R_SetupShader_Surface(vec3_origin, (rsurface.texture->currentmaterialflags & MATERIALFLAG_MODELLIGHT) != 0, 1, 1, rsurface.texture->specularscale, RSURFPASS_BASE, end-start, texturesurfacelist + start, nullptr, false);
				RSurf_DrawBatch();
			}
	...
	GL_DepthMask(writedepth && !(rsurface.texture->currentmaterialflags & MATERIALFLAG_BLENDED));
	R_SetupShader_Surface(vec3_origin, (rsurface.texture->currentmaterialflags & MATERIALFLAG_MODELLIGHT) != 0, 1, 1, rsurface.texture->specularscale, RSURFPASS_BASE, texturenumsurfaces, texturesurfacelist, nullptr, (rsurface.texture->currentmaterialflags & MATERIALFLAG_SKY) != 0);
	RSurf_DrawBatch();
}

static void R_DrawTextureSurfaceList_GL13(int texturenumsurfaces, const msurface_t **texturesurfacelist, bool writedepth)
{
	...
	for (layerindex = 0, layer = rsurface.texture->currentlayers;layerindex < rsurface.texture->currentnumlayers;layerindex++, layer++)
	{
		...
		R_Mesh_ColorPointer(4, GL_FLOAT, sizeof(float[4]), nullptr, 0, 0);
		...
		case TEXTURELAYERTYPE_TEXTURE:
			...
			R_Mesh_TexCoordPointer(0, 2, GL_FLOAT, sizeof(float[2]), rsurface.batchtexcoordtexture2f, rsurface.batchtexcoordtexture2f_vertexbuffer, rsurface.batchtexcoordtexture2f_bufferoffset);
			R_Mesh_TexBind(1, 0);
			R_Mesh_TexCoordPointer(1, 2, GL_FLOAT, sizeof(float[2]), nullptr, 0, 0);
		...
		case TEXTURELAYERTYPE_FOG:
			...
			{
				R_Mesh_TexBind(0, 0);
				R_Mesh_TexCoordPointer(0, 2, GL_FLOAT, sizeof(float[2]), nullptr, 0, 0);
			}
			R_Mesh_TexBind(1, 0);
			R_Mesh_TexCoordPointer(1, 2, GL_FLOAT, sizeof(float[2]), nullptr, 0, 0);
...
static void R_DrawTextureSurfaceList_GL11(int texturenumsurfaces, const msurface_t **texturesurfacelist, bool writedepth)
{
...
static void R_DrawTextureSurfaceList_ShowSurfaces(int texturenumsurfaces, const msurface_t **texturesurfacelist, bool writedepth)
{
	int vi;
	int j;
	...
	if (r_showsurfaces.integer == 3)
	{
		...
		R_Mesh_PrepareVertices_Generic_Arrays(rsurface.batchnumvertices, rsurface.batchvertex3f, rsurface.passcolor4f, nullptr);
		R_SetupShader_Generic_NoTexture(false, false);
		RSurf_DrawBatch();
	}
	...
	else if (r_showsurfaces.integer == 2)
	{
		...
		R_Mesh_PrepareVertices_Generic_Unlock();
		R_Mesh_Draw(0, rsurface.batchnumtriangles*3, 0, rsurface.batchnumtriangles, nullptr, nullptr, 0, nullptr, nullptr, 0);
	}
	else
	{
...
static void R_DrawWorldTextureSurfaceList(int texturenumsurfaces, const msurface_t **texturesurfacelist, bool writedepth, bool prepass)
{
	CHECKGLERROR
	RSurf_SetupDepthAndCulling();
	if (r_showsurfaces.integer)
...
static void R_DrawModelTextureSurfaceList(int texturenumsurfaces, const msurface_t **texturesurfacelist, bool writedepth, bool prepass)
{
	CHECKGLERROR
	RSurf_SetupDepthAndCulling();
	if (r_showsurfaces.integer)
...
static void R_DrawSurface_TransparentCallback(const entity_render_t *ent, const rtlight_t *rtlight, int numsurfaces, int *surfacelist)
{
	...
	if (r_transparentdepthmasking.integer)
	{
		...
		for (i = 0;i < numsurfaces;i = j)
		{
			...
			rsurface.lightmaptexture = nullptr;
			rsurface.deluxemaptexture = nullptr;
	...
	for (i = 0;i < numsurfaces;i = j)
	{
		...
		else
		{
			...
			rsurface.uselightmaptexture = surface->lightmaptexture != nullptr;
			for (;j < endsurface;j++)
			{
				surface = rsurface.modelsurfaces + surfacelist[j];
...
static void R_ProcessWorldTextureSurfaceList(int texturenumsurfaces, const msurface_t **texturesurfacelist, bool writedepth, bool depthonly, bool prepass)
{
	CHECKGLERROR
	if (depthonly)
...
static void R_QueueWorldSurfaceList(int numsurfaces, const msurface_t **surfacelist, int flagsmask, bool writedepth, bool depthonly, bool prepass)
{
	int i, j;
	texture_t *texture;
	R_FrameData_SetMark();
	...
	for (i = 0;i < numsurfaces;i = j)
	{
		...
		else
		{
			...
			rsurface.uselightmaptexture = surfacelist[i]->lightmaptexture != nullptr;
			// simply scan ahead until we find a different texture or lightmap state
...
static void R_ProcessModelTextureSurfaceList(int texturenumsurfaces, const msurface_t **texturesurfacelist, bool writedepth, bool depthonly, bool prepass)
{
	CHECKGLERROR
	if (depthonly)
		R_DrawTextureSurfaceList_DepthOnly(texturenumsurfaces, texturesurfacelist);
...
static void R_QueueModelSurfaceList(entity_render_t *ent, int numsurfaces, const msurface_t **surfacelist, int flagsmask, bool writedepth, bool depthonly, bool prepass)
{
	int i, j;
	texture_t *texture;
	R_FrameData_SetMark();
...
static void R_DrawLoc_Callback(const entity_render_t *ent, const rtlight_t *rtlight, int numsurfaces, int *surfacelist)
{
	...
	R_Mesh_PrepareVertices_Generic_Arrays(6*4, vertex3f, nullptr, nullptr);
	R_SetupShader_Generic_NoTexture(false, false);
	R_Mesh_Draw(0, 6*4, 0, 6*2, nullptr, nullptr, 0, locboxelements, nullptr, 0);
}
...
void R_DrawLocs()
{
	...
	for (loc = cl.locnodes, index = 0;loc;loc = loc->next, index++)
	{
		...
		R_MeshQueue_AddTransparent(TRANSPARENTSORT_DISTANCE, center, R_DrawLoc_Callback, (entity_render_t *)loc, loc == nearestloc ? -1 : index, nullptr);
	}
}
...
static void R_DecalSystem_SpawnTriangle(decalsystem_t *decalsystem, const float *v0, const float *v1, const float *v2, const float *t0, const float *t1, const float *t2, const float *c0, const float *c1, const float *c2, int triangleindex, int surfaceindex, int decalsequence)
{
	...
	if (decalsystem->maxdecals <= decalsystem->numdecals)
	{
		...
		decalsystem->element3s = (useshortelements ? ((unsigned short *)(decalsystem->element3i + decalsystem->maxdecals*3)) : nullptr);
...
static void R_DecalSystem_SplatTriangle(decalsystem_t *decalsystem, float r, float g, float b, float a, float s1, float t1, float s2, float t2, int decalsequence, bool dynamic, float (*planes)[4], matrix4x4_t *projection, int triangleindex, int surfaceindex)
{
	int cornerindex;
	int index;
	float v[9][3];
	...
	for (cornerindex = 0;cornerindex < numpoints;cornerindex++)
	{
		...
		f = a * (1.0f-math::fabsf(temp[0])) * cl_decals_newsystem_intensitymultiplier.value;
		f = bound(.0f, f, 1.0f);
		c[cornerindex][0] = r * f;
		c[cornerindex][1] = g * f;
		c[cornerindex][2] = b * f;
...
static void R_DecalSystem_SplatEntity(entity_render_t *ent, const vec3_t worldorigin, const vec3_t worldnormal, float r, float g, float b, float a, float s1, float t1, float s2, float t2, float worldsize, int decalsequence)
{
	...
	AnglesFromVectors(angles, localnormal, nullptr, false);
...
static void R_DecalSystem_ApplySplatEntitiesQueue()
{
	int i;
	r_decalsystem_splatqueue_t *queue;
...
static void R_DrawModelDecals_Entity(entity_render_t *ent)
{
	...
	float *t2f;
	const int *e;
	const unsigned char *surfacevisible = ent == r_refdef.scene.worldentity ? r_refdef.viewcache.world_surfacevisible : nullptr;
	int numtris = 0;
	...
	for (i = 0, decal = decalsystem->decals;i < numdecals;i++, decal++)
	{
		...
		if (decal->lived >= cl_decals_time.value)
			alpha = 1.0f - faderate * (decal->lived - cl_decals_time.value);
		else
			alpha = 1.0f;
	...
	if (numtris > 0)
	{
		...
		RSurf_ActiveCustomEntity(&rsurface.matrix, &rsurface.inversematrix, rsurface.ent_flags, ent->shadertime, 1, 1, 1, 1, numdecals*3, decalsystem->vertex3f, decalsystem->texcoord2f, nullptr, nullptr, nullptr, decalsystem->color4f, numtris, decalsystem->element3i, decalsystem->element3s, false, false);
		...
		R_SetupShader_Generic(decalskinframe->base, nullptr, GL_MODULATE, 1, false, false, false);
		R_Mesh_Draw(0, numtris * 3, 0, numtris, decalsystem->element3i, nullptr, 0, decalsystem->element3s, nullptr, 0);
	}
}
		...
		for (i = 0, j = model->firstmodelsurface, surface = model->data_surfaces + j;i < model->nummodelsurfaces;i++, j++, surface++)
		{
			...
			if ((rsurface.texture->currentmaterialflags & flagsmask) && surface->num_triangles)
			{
				...
				if (!rsurface.texture->currentlayers->depthmask)
					GL_Color(c, .0f, .0f, 1.0f);
				else if (ent == r_refdef.scene.worldentity)
					GL_Color(c, c, c, 1.0f);
				else
					GL_Color(.0f, c, .0f, 1.0f);
				R_Mesh_PrepareVertices_Generic_Arrays(rsurface.batchnumvertices, rsurface.batchvertex3f, nullptr, nullptr);
				RSurf_DrawBatch();
			}
		}
		rsurface.texture = nullptr;
	}
		...
		for (bihleafindex = 0, bihleaf = bih->leafs;bihleafindex < bih->numleafs;bihleafindex++, bihleaf++)
		{
			...
			case BIH_BRUSH:
				...
				if (brush->colbrushf && brush->colbrushf->numtriangles)
				{
					...
					R_Mesh_PrepareVertices_Generic_Arrays(brush->colbrushf->numpoints, brush->colbrushf->points->v, nullptr, nullptr);
					R_Mesh_Draw(0, brush->colbrushf->numpoints, 0, brush->colbrushf->numtriangles, brush->colbrushf->elements, nullptr, 0, nullptr, nullptr, 0);
				}
				break;
			case BIH_COLLISIONTRIANGLE:
				...
				GL_Color((bihleafindex & 31) * (1.0f / 32.0f) * r_refdef.view.colorscale, ((bihleafindex >> 5) & 31) * (1.0f / 32.0f) * r_refdef.view.colorscale, ((bihleafindex >> 10) & 31) * (1.0f / 32.0f) * r_refdef.view.colorscale, r_showcollisionbrushes.value);
				R_Mesh_PrepareVertices_Generic_Arrays(3, vertex3f[0], nullptr, nullptr);
				R_Mesh_Draw(0, 3, 0, 1, polygonelement3i, nullptr, 0, polygonelement3s, nullptr, 0);
				break;
	...
	if (r_showtris.integer && qglPolygonMode)
	{
		...
		for (i = 0, j = model->firstmodelsurface, surface = model->data_surfaces + j;i < model->nummodelsurfaces;i++, j++, surface++)
		{
			...
			if ((rsurface.texture->currentmaterialflags & flagsmask) && surface->num_triangles)
			{
				...
				if (!rsurface.texture->currentlayers->depthmask)
					GL_Color(r_refdef.view.colorscale, .0f, .0f, r_showtris.value);
				else if (ent == r_refdef.scene.worldentity)
					GL_Color(r_refdef.view.colorscale, r_refdef.view.colorscale, r_refdef.view.colorscale, r_showtris.value);
				else
					GL_Color(.0f, r_refdef.view.colorscale, .0f, r_showtris.value);
				R_Mesh_PrepareVertices_Generic_Arrays(rsurface.batchnumvertices, rsurface.batchvertex3f, nullptr, nullptr);
				RSurf_DrawBatch();
			}
		}
			...
			if ((rsurface.texture->currentmaterialflags & flagsmask) && surface->num_triangles)
			{
				...
				if (r_shownormals.value < 0 && rsurface.batchnormal3f)
				{
					for (k = 0, l = rsurface.batchfirstvertex;k < rsurface.batchnumvertices;k++, l++)
					{
						...
						GL_Color(.0f, .0f, r_refdef.view.colorscale, 1.0f);
						qglVertex3f(v[0], v[1], v[2]);
						VectorMA(v, -r_shownormals.value, rsurface.batchnormal3f + l * 3, v);
						GL_Color(r_refdef.view.colorscale, r_refdef.view.colorscale, r_refdef.view.colorscale, 1.0f);
						qglVertex3f(v[0], v[1], v[2]);
					}
				}
				if (r_shownormals.value > 0 && rsurface.batchsvector3f)
				{
					...
					{
						VectorCopy(rsurface.batchvertex3f + l * 3, v);
						GL_Color(r_refdef.view.colorscale, .0f, .0f, 1.0f);
						qglVertex3f(v[0], v[1], v[2]);
				...
				if (r_shownormals.value > 0 && rsurface.batchtvector3f)
				{
					...
					{
						VectorCopy(rsurface.batchvertex3f + l * 3, v);
						GL_Color(.0f, r_refdef.view.colorscale, .0f, 1.0f);
						qglVertex3f(v[0], v[1], v[2]);
...
const msurface_t **r_surfacelist = nullptr;
void R_DrawWorldSurfaces(bool skysurfaces, bool writedepth, bool depthonly, bool debug, bool prepass)
{
...
void R_DrawModelSurfaces(entity_render_t *ent, bool skysurfaces, bool writedepth, bool depthonly, bool debug, bool prepass)
{
	int i, j, endj, flagsmask;
	dp_model_t *model = ent->model;
	msurface_t *surfaces;
...
static texture_t R_DrawCustom_texture;
void R_DrawCustomSurface(skinframe_t *skinframe, const matrix4x4_t *texmatrix, int materialflags, int firstvertex, int numvertices, int firsttriangle, int numtriangles, bool writedepth, bool prepass)
{
	...
	R_DrawCustom_texture.update_lastrenderframe = -1; // regenerate this texture
	R_DrawCustom_texture.basematerialflags = materialflags | MATERIALFLAG_CUSTOMSURFACE | MATERIALFLAG_WALL;
	R_DrawCustom_texture.basealpha = 1.0f;
	R_DrawCustom_texture.currentskinframe = skinframe;
	R_DrawCustom_texture.currenttexmatrix = *texmatrix; // requires MATERIALFLAG_CUSTOMSURFACE
	R_DrawCustom_texture.offsetmapping = OFFSETMAPPING_OFF;
	R_DrawCustom_texture.offsetscale = 1;
	R_DrawCustom_texture.specularscalemod = 1;
	R_DrawCustom_texture.specularpowermod = 1;
	R_DrawCustom_texture.transparentsort = TRANSPARENTSORT_DISTANCE;
	// WHEN ADDING DEFAULTS HERE, REMEMBER TO PUT DEFAULTS IN ALL LOADERS
	// JUST GREP FOR "specularscalemod = 1".

	surface.texture = &R_DrawCustom_texture;
...
void R_DrawCustomSurface_Texture(texture_t *texture, const matrix4x4_t *texmatrix, int materialflags, int firstvertex, int numvertices, int firsttriangle, int numtriangles, bool writedepth, bool prepass)
{
	static msurface_t surface;
	const msurface_t *surfacelist = &surface;
