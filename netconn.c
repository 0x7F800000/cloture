...
static bool serverlist_querysleep = true;
static bool serverlist_paused = false;
...
netconn_t *netconn_list = nullptr;
mempool_t *netconn_mempool = nullptr;
void *netconn_mutex = nullptr;
...
serverlist_entry_t *serverlist_cache = nullptr;

bool serverlist_consoleoutput;
...
static bool _ServerList_Entry_Compare( serverlist_entry_t *A, serverlist_entry_t *B )
{
...
static bool _ServerList_CompareInt( int A, serverlist_maskop_t op, int B )
{
...
static bool _ServerList_CompareStr( const char *A, serverlist_maskop_t op, const char *B )
{
	int i;
...
static bool _ServerList_Entry_Mask( serverlist_mask_t *mask, serverlist_info_t *info )
{
...
static void ServerList_ViewList_Insert( serverlist_entry_t *entry )
{
	...
	if(LHNETADDRESS_FromString(&addr, entry->info.cname, 26000))
	{
		...
		if(Crypto_RetrieveHostKey(&addr, 0, nullptr, 0, idfp, sizeof(idfp), nullptr))
		{
...
void ServerList_RebuildViewList()
{
	int i;
...
void ServerList_QueryList(bool resetcache, bool querydp, bool queryqw, bool consoleoutput)
{
...
bool NetConn_CanSend(netconn_t *conn)
{
...
int NetConn_SendUnreliableMessage(netconn_t *conn, sizebuf_t *data, protocolversion_t protocol, int rate, int burstsize, bool quakesignon_suppressreliables)
{
	int totallen = 0;
...
bool NetConn_HaveClientPorts()
{
	return !!cl_numsockets;
}

bool NetConn_HaveServerPorts()
{
	return !!sv_numsockets;
}

void NetConn_CloseClientPorts()
{
...
void NetConn_OpenClientPorts()
{
	...
	NetConn_OpenClientPort(nullptr, LHNETADDRESSTYPE_LOOP, 2);
...
static bool NetConn_OpenServerPort(const char *addressstring, lhnetaddresstype_t addresstype, int defaultport, int range)
{
	lhnetaddress_t address;
	lhnetsocket_t *s;
	int port;
...
void NetConn_OpenServerPorts(int opennetports)
{
	...
	if (cls.state != ca_dedicated)
		NetConn_OpenServerPort(nullptr, LHNETADDRESSTYPE_LOOP, 1, 1);
	if (opennetports)
	{
#ifdef SUPPORTIPV6
		bool ip4success = NetConn_OpenServerPort(net_address.string, LHNETADDRESSTYPE_INET4, port, 100);
...
static int NetConn_ClientParsePacket_ServerList_ProcessReply(const char *addressstring)
{
	int n;
	int pingtime;
	serverlist_entry_t *entry = nullptr;
...
static bool NetConn_ClientParsePacket_ServerList_PrepareQuery( int protocol, const char *ipstring, bool isfavorite ) {
	int n;
...
static void NetConn_ClientParsePacket_ServerList_ParseDPList(lhnetaddress_t *senderaddress, const unsigned char *data, int length, bool isextended)
{
	masterreplycount++;
	if (serverlist_consoleoutput)
...
static int NetConn_ClientParsePacket(lhnetsocket_t *mysocket, unsigned char *data, int length, lhnetaddress_t *peeraddress)
{
	...
	if (length >= 5 && data[0] == 255 && data[1] == 255 && data[2] == 255 && data[3] == 255)
	{
		...
		if (length >= 15 && !memcmp(string, "statusResponse\x0A", 15))
		{
			...
			if ((s = InfoString_GetValue(string, "gamename"     , infostringvalue, sizeof(infostringvalue))) != nullptr) strlcpy(info->game, s, sizeof (info->game));
			if ((s = InfoString_GetValue(string, "modname"      , infostringvalue, sizeof(infostringvalue))) != nullptr) strlcpy(info->mod , s, sizeof (info->mod ));
			if ((s = InfoString_GetValue(string, "mapname"      , infostringvalue, sizeof(infostringvalue))) != nullptr) strlcpy(info->map , s, sizeof (info->map ));
			if ((s = InfoString_GetValue(string, "hostname"     , infostringvalue, sizeof(infostringvalue))) != nullptr) strlcpy(info->name, s, sizeof (info->name));
			if ((s = InfoString_GetValue(string, "protocol"     , infostringvalue, sizeof(infostringvalue))) != nullptr) info->protocol = atoi(s);
			if ((s = InfoString_GetValue(string, "clients"      , infostringvalue, sizeof(infostringvalue))) != nullptr) info->numplayers = atoi(s);
			if ((s = InfoString_GetValue(string, "bots"         , infostringvalue, sizeof(infostringvalue))) != nullptr) info->numbots = atoi(s);
			if ((s = InfoString_GetValue(string, "sv_maxclients", infostringvalue, sizeof(infostringvalue))) != nullptr) info->maxplayers = atoi(s);
			if ((s = InfoString_GetValue(string, "gameversion"  , infostringvalue, sizeof(infostringvalue))) != nullptr) info->gameversion = atoi(s);
			if ((s = InfoString_GetValue(string, "qcstatus"     , infostringvalue, sizeof(infostringvalue))) != nullptr) strlcpy(info->qcstatus, s, sizeof(info->qcstatus));
			if (p                                                                                         != nullptr) strlcpy(info->players, p, sizeof(info->players));
		...
		if (length > 2 && !memcmp(string, "n\\", 2))
		{
			...
			if ((s = InfoString_GetValue(string, "*gamedir"     , infostringvalue, sizeof(infostringvalue))) != nullptr) strlcpy(info->mod , s, sizeof (info->mod ));else info->mod[0]  = 0;
			if ((s = InfoString_GetValue(string, "map"          , infostringvalue, sizeof(infostringvalue))) != nullptr) strlcpy(info->map , s, sizeof (info->map ));else info->map[0]  = 0;
			if ((s = InfoString_GetValue(string, "hostname"     , infostringvalue, sizeof(infostringvalue))) != nullptr) strlcpy(info->name, s, sizeof (info->name));else info->name[0] = 0;
			info->protocol = 0;
			info->numplayers = 0; // updated below
			info->numhumans = 0; // updated below
			if ((s = InfoString_GetValue(string, "maxclients"   , infostringvalue, sizeof(infostringvalue))) != nullptr) info->maxplayers = atoi(s);else info->maxplayers  = 0;
			if ((s = InfoString_GetValue(string, "gameversion"  , infostringvalue, sizeof(infostringvalue))) != nullptr) info->gameversion = atoi(s);else info->gameversion = 0;
...
static bool NetConn_BuildStatusResponse(const char* challenge, char* out_msg, size_t out_size, bool fullstatus)
{
...
static bool NetConn_PreventFlood(lhnetaddress_t *peeraddress, server_floodaddress_t *floodlist, size_t floodlength, double floodtime, bool renew)
{
...
typedef bool (*rcon_matchfunc_t) (lhnetaddress_t *peeraddress, const char *password, const char *hash, const char *s, int slen);

static bool hmac_mdfour_time_matching(lhnetaddress_t *peeraddress, const char *password, const char *hash, const char *s, int slen)
{
	char mdfourbuf[16];
	long t1, t2;

	t1 = (long) time(nullptr);
	t2 = strtol(s, nullptr, 0);
...
static bool hmac_mdfour_challenge_matching(lhnetaddress_t *peeraddress, const char *password, const char *hash, const char *s, int slen)
{
	char mdfourbuf[16];
	int i;
...
static bool plaintext_matching(lhnetaddress_t *peeraddress, const char *password, const char *hash, const char *s, int slen)
{
...
check:
	for(text = s; text != endpos; ++text)
		if((signed char) *text > 0 && ((signed char) *text < (signed char) ' ' || *text == ';'))
			return nullptr; // block possible exploits against the parser/alias expansion
...
static void RCon_Execute(lhnetsocket_t *mysocket, lhnetaddress_t *peeraddress, const char *addressstring2, const char *userlevel, const char *s, const char *endpos, bool proquakeprotocol)
{
	if(userlevel)
	{
...
static int NetConn_ServerParsePacket(lhnetsocket_t *mysocket, unsigned char *data, int length, lhnetaddress_t *peeraddress)
{
	...
	bool islocal = (LHNETADDRESS_GetAddressType(peeraddress) == LHNETADDRESSTYPE_LOOP);
	char senddata[NET_HEADERSIZE+NET_MAXMESSAGE+CRYPTO_HEADERSIZE];
	...
	{
		// received a command string - strip off the packaging and put it
		// into our string buffer with nullptr termination
		data += 4;
		length -= 4;
		length = min(length, (int)sizeof(stringbuf) - 1);
		memcpy(stringbuf, data, length);
		...
		if (length >= 5 && !memcmp(string, "rcon ", 5))
		{
			...
			{
				const char *userlevel = RCon_Authenticate(peeraddress, password, s, endpos, plaintext_matching, nullptr, 0);
				RCon_Execute(mysocket, peeraddress, addressstring2, userlevel, s, endpos, false);
			}
			return true;
		}
	...
	if (length >= 5 && (i = BuffBigLong(data)) && (i & (~NETFLAG_LENGTH_MASK)) == (int)NETFLAG_CTL && (i & NETFLAG_LENGTH_MASK) == length && (sv.protocol == PROTOCOL_QUAKE || sv.protocol == PROTOCOL_QUAKEDP || sv.protocol == PROTOCOL_NEHAHRAMOVIE || sv.protocol == PROTOCOL_NEHAHRABJP || sv.protocol == PROTOCOL_NEHAHRABJP2 || sv.protocol == PROTOCOL_NEHAHRABJP3 || sv.protocol == PROTOCOL_DARKPLACES1 || sv.protocol == PROTOCOL_DARKPLACES2 || sv.protocol == PROTOCOL_DARKPLACES3) && !ENCRYPTION_REQUIRED)
	{
		...
		case CCREQ_CONNECT:
			...
			{
				netconn_t *conn;
				if (!client->active && (client->netconnection = conn = NetConn_Open(mysocket, peeraddress)) != nullptr)
				{
		...
		case CCREQ_RCON:
			...
			if (sv.active && !rcon_secure.integer)
			{
				...
				userlevel = RCon_Authenticate(peeraddress, password, s, endpos, plaintext_matching, nullptr, 0);
...
#ifdef CONFIG_MENU
void NetConn_QueryMasters(bool querydp, bool queryqw)
{
	int i, j;
	int masternum;
...
void NetConn_Init()
{
	...
	netconn_mempool = Mem_AllocPool("network connections", 0, nullptr);
