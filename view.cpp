...
static vec_t lowpass(vec_t value, vec_t frac, vec_t *store)
{
	...
	return (*store = *store * (1.0f - frac) + value * frac);
}
		...
		else
		{
			...
			if (!cldead)
			{
				...
				if (cl_bob2.value && cl_bob2cycle.value)
				{
					...
					{
						if(cl.bob2_smooth > 0.0f)
							cl.bob2_smooth -= bound(0.0f, cl_bob2smooth.value, 1.0f);
						else
							cl.bob2_smooth = 0.0f;
					}
				...
				if (cl_bobfall.value && cl_bobfallcycle.value)
				{
					if (!clonground)
					{
						...
						if (clvelocity[2] < -cl_bobfallminspeed.value)
							cl.bobfall_swing = 1.0f;
						else
							cl.bobfall_swing = 0.0f; // TODO really?
					}
					else
					{
...
void V_FadeViewFlashs(void)
{
	...
	if (cl.cshifts[CSHIFT_DAMAGE].percent <= 0.0f)
		cl.cshifts[CSHIFT_DAMAGE].percent = 0.0f;
	// drop the bonus value
	cl.cshifts[CSHIFT_BONUS].percent -= (cl.time - cl.oldtime)*cl.cshifts[CSHIFT_BONUS].alphafade;
	if (cl.cshifts[CSHIFT_BONUS].percent <= 0.0f)
		cl.cshifts[CSHIFT_BONUS].percent = 0.0f;
}
...
void V_CalcViewBlend(void)
{
	float a2;
	int j;
	r_refdef.viewblend[0] = .0f;
	r_refdef.viewblend[1] = .0f;
	r_refdef.viewblend[2] = .0f;
	r_refdef.viewblend[3] = .0f;
	r_refdef.frustumscale_x = 1.0f;
	r_refdef.frustumscale_y = 1.0f;
	if (cls.state == ca_connected && cls.signon == SIGNONS)
	{
		...
		if (supercontents & SUPERCONTENTS_LIQUIDSMASK)
		{
			...
			{
				cl.cshifts[CSHIFT_CONTENTS].destcolor[0] = 255.0f;
				cl.cshifts[CSHIFT_CONTENTS].destcolor[1] = 80.0f;
				cl.cshifts[CSHIFT_CONTENTS].destcolor[2] = 0.0f;
			}
			else if (supercontents & SUPERCONTENTS_SLIME)
			{
				cl.cshifts[CSHIFT_CONTENTS].destcolor[0] = 0.0f;
				cl.cshifts[CSHIFT_CONTENTS].destcolor[1] = 25.0f;
				cl.cshifts[CSHIFT_CONTENTS].destcolor[2] = 5.0f;
			}
			else
			{
				cl.cshifts[CSHIFT_CONTENTS].destcolor[0] = 130.0f;
				cl.cshifts[CSHIFT_CONTENTS].destcolor[1] = 80.0f;
				cl.cshifts[CSHIFT_CONTENTS].destcolor[2] = 50.0f;
			}
			cl.cshifts[CSHIFT_CONTENTS].percent = 150.0f * 0.5f;
		}
		else
		{
			cl.cshifts[CSHIFT_CONTENTS].destcolor[0] = 0.0f;
			cl.cshifts[CSHIFT_CONTENTS].destcolor[1] = 0.0f;
			cl.cshifts[CSHIFT_CONTENTS].destcolor[2] = 0.0f;
			cl.cshifts[CSHIFT_CONTENTS].percent = 0.0f;
		}
		...
		if (gamemode != GAME_TRANSFUSION)
		{
			...
			{
				cl.cshifts[CSHIFT_POWERUP].destcolor[0] = 0.0f;
				cl.cshifts[CSHIFT_POWERUP].destcolor[1] = 0.0f;
				cl.cshifts[CSHIFT_POWERUP].destcolor[2] = 255.0f;
				cl.cshifts[CSHIFT_POWERUP].percent = 30.0f;
			}
			else if (cl.stats[STAT_ITEMS] & IT_SUIT)
			{
				cl.cshifts[CSHIFT_POWERUP].destcolor[0] = 0.0f;
				cl.cshifts[CSHIFT_POWERUP].destcolor[1] = 255.0f;
				cl.cshifts[CSHIFT_POWERUP].destcolor[2] = 0.0f;
				cl.cshifts[CSHIFT_POWERUP].percent = 20.0f;
			}
			else if (cl.stats[STAT_ITEMS] & IT_INVISIBILITY)
			{
				cl.cshifts[CSHIFT_POWERUP].destcolor[0] = 100.0f;
				cl.cshifts[CSHIFT_POWERUP].destcolor[1] = 100.0f;
				cl.cshifts[CSHIFT_POWERUP].destcolor[2] = 100.0f;
				cl.cshifts[CSHIFT_POWERUP].percent = 100.0f;
			}
			else if (cl.stats[STAT_ITEMS] & IT_INVULNERABILITY)
			{
				cl.cshifts[CSHIFT_POWERUP].destcolor[0] = 255.0f;
				cl.cshifts[CSHIFT_POWERUP].destcolor[1] = 255.0f;
				cl.cshifts[CSHIFT_POWERUP].destcolor[2] = 0.0f;
				cl.cshifts[CSHIFT_POWERUP].percent = 30.0f;
			}
			else
				cl.cshifts[CSHIFT_POWERUP].percent = 0.0f;
		}
		...
		for (j = 0;j < NUM_CSHIFTS;j++)
		{
			...
			if (a2 > 0)
			{
				...
				r_refdef.viewblend[3] = (1.0f - (1.0f - r_refdef.viewblend[3]) * (1.0f - a2)); // correct alpha multiply...  took a while to find it on the web
			}
		}
		// saturate color (to avoid blending in black)
		if (r_refdef.viewblend[3])
		{
			a2 = 1.0f / r_refdef.viewblend[3];
		...
		if (cl.stats[STAT_HEALTH] <= 0 && cl.stats[STAT_HEALTH] != -666 && 
			cl.stats[STAT_HEALTH] != -2342 && cl_deathfade.value > 0.0f)
		{
		...
		if(cl.deathfade > 0.0f)
		{
			float deathfadevec[3] = {0.3f, 0.0f, 0.0f};
			const float a = r_refdef.viewblend[3] + cl.deathfade - r_refdef.viewblend[3]*cl.deathfade;
			if(a > 0)
