...
int SV_PointSuperContents(const vec3_t point)
{
	...
	// list of entities to test for collisions
	int numtouchedicts = SV_EntitiesInBox(point, point, MAX_EDICTS, touchedicts);
	...
	for (i = 0;i < numtouchedicts;i++)
	{
		prvm_edict_t *touch = touchedicts[i];
...
int SV_EntitiesInBox(const vec3_t mins, const vec3_t maxs, int maxedicts, prvm_edict_t **resultedicts)
{
	...
	if (maxedicts < 1 || resultedicts == nullptr)
		return 0;
...
static void RotateBBox(const vec3_t mins, const vec3_t maxs, const vec3_t angles, vec3_t rotatedmins, vec3_t rotatedmaxs)
{
	...
	Matrix4x4_Transform(&m, v, u);
	
	VectorCopy(u, rotatedmins); 
	VectorCopy(u, rotatedmaxs);
	...
	if(rotatedmins[0] > u[0]) 
		rotatedmins[0] = u[0]; 
		
	if(rotatedmins[1] > u[1]) 
		rotatedmins[1] = u[1]; 
		
	if(rotatedmins[2] > u[2]) 
		rotatedmins[2] = u[2];
		
	if(rotatedmaxs[0] < u[0]) 
		rotatedmaxs[0] = u[0]; 
		
	if(rotatedmaxs[1] < u[1]) 
		rotatedmaxs[1] = u[1]; 
		
	if(rotatedmaxs[2] < u[2]) 
		rotatedmaxs[2] = u[2];
...
void SV_LinkEdict (prvm_edict_t *ent)
{
	...
	int modelindex = static_cast<int>(PRVM_serveredictfloat(ent, modelindex));
...
static int SV_CheckContentsTransition(prvm_edict_t *ent, const int nContents)
{
	...
	{ // Changed Contents
		...
		{ // Valid Function; Execute
			...
			PRVM_serverglobalfloat(time) = sv.time;
			// Execute VM Function
			prog->ExecuteProgram(prog, PRVM_serveredictfunction(ent, contentstransition), "contentstransition: nullptr function");
		}
	}
...
//
	...
	{
		if (unlikely(PRVM_IS_NAN(PRVM_serveredictvector(ent, velocity)[i])))
		{
			Con_Printf("Got a NaN velocity on entity #%i (%s)\n", PRVM_NUM_FOR_EDICT(ent), PRVM_GetString(prog, PRVM_serveredictstring(ent, classname)));
			PRVM_serveredictvector(ent, velocity)[i] = 0;
		}
		if (unlikely(PRVM_IS_NAN(PRVM_serveredictvector(ent, origin)[i])))
		{
...
==================
*/
constexpr vec_t STOP_EPSILON = static_cast<vec_t>(0.1);
...
If stepnormal is not nullptr, the plane normal of any vertical wall hit will be stored
============
*/
...
static int SV_FlyMove (prvm_edict_t *ent, float time, bool applygravity, float *stepnormal, int hitsupercontentsmask, float stepheight)
{
	prvm_prog_t *prog = SVVM_prog;
	float d;
	...
	for (size_t bumpcount = 0; bumpcount < MAX_CLIP_PLANES; bumpcount++)
	{
...
{
	prvm_prog_t *prog = SVVM_prog;
	float ent_gravity = PRVM_serveredictfloat(ent, gravity);
	if (!ent_gravity)
		ent_gravity = 1.0f;
...
static bool SV_PushEntity (trace_t *trace, prvm_edict_t *ent, vec3_t push, bool dolink)
{
	...
	int solid = (int)PRVM_serveredictfloat(ent, solid);
...
static void SV_PushMove (prvm_edict_t *pusher, float movetime)
{
	...
	if (unlikely(index < 1 || index >= MAX_MODELS))
	{
...
static bool SV_CheckWater (prvm_edict_t *ent)
{
	...
	nNativeContents = Mod_Q1BSP_NativeContentsFromSuperContents(nullptr, cont);
...
static void SV_WallFriction (prvm_edict_t *ent, float *stepnormal)
{
	...
	AngleVectors (v_angle, forward, nullptr, nullptr);
	
	const float d = DotProduct (stepnormal, forward) + 0.5f;
	if (d >= 0)
		return;
	
	// cut the tangential velocity
	const float i = DotProduct (stepnormal, PRVM_serveredictvector(ent, velocity));
...
static void SV_WalkMove (prvm_edict_t *ent)
{
	...
	oldonground = static_cast<int>(PRVM_serveredictfloat(ent, flags)) & FL_ONGROUND;

	VectorCopy (PRVM_serveredictvector(ent, origin), start_origin);
	VectorCopy (PRVM_serveredictvector(ent, velocity), start_velocity);

	clip = SV_FlyMove (ent, sv.frametime, applygravity, nullptr, hitsupercontentsmask, sv_gameplayfix_stepmultipletimes.integer ? sv_stepheight.value : 0);
	...
	originalmove_flags = static_cast<int>(PRVM_serveredictfloat(ent, flags));
...
static void SV_CheckWaterTransition (prvm_edict_t *ent)
{
	...
	int cont;
	VectorCopy(PRVM_serveredictvector(ent, origin), entorigin);
	cont = Mod_Q1BSP_NativeContentsFromSuperContents(nullptr, SV_PointSuperContents(entorigin));
	if (!PRVM_serveredictfloat(ent, watertype))
	{
		// just spawned here
...
// if onground, return without moving
	if (static_cast<int>(PRVM_serveredictfloat(ent, flags)) & FL_ONGROUND)
	{
...
// move angles
	...
	// move origin
		...
		switch(static_cast<int>(PRVM_serveredictfloat(ent, movetype)))
		{
	...
	if (!(flags & (FL_FLY | FL_SWIM)))
	{
		if (flags & FL_ONGROUND)
		{
			...
			if (PRVM_serveredictvector(ent, velocity)[2] >= (1.0 / 32.0) && sv_gameplayfix_upwardvelocityclearsongroundflag.integer)
			{
				PRVM_serveredictfloat(ent, flags) -= FL_ONGROUND;
				SV_CheckVelocity(ent);
				SV_FlyMove(ent, sv.frametime, true, nullptr, SV_GenericHitSuperContentsMask(ent), 0);
				SV_LinkEdict(ent);
				SV_LinkEdict_TouchAreaGrid(ent);
		...
		else
		{
			...
			if (hitsound && (int)PRVM_serveredictfloat(ent, flags) & FL_ONGROUND)
			{
				...
				{ // Valid Function; Execute
					...
					PRVM_serverglobalfloat(time) = sv.time;
					// Execute VM Function
					prog->ExecuteProgram(prog, PRVM_serveredictfunction(ent, movetypesteplandevent), "movetypesteplandevent: nullptr function");
				}
				else
