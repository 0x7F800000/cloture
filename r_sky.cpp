...
__align(16) struct suffixinfo_s
{
	const char *suffix;
	bool flipx, flipy, flipdiagonal;
};

using suffixinfo_t = suffixinfo_s;
...
static void R_UnloadSkyBox()
{
	...
	for (size_t i = 0; i < 6; i++)
	{
		if (skyboxskinframe[i])
			c++;	// TODO: make a R_SkinFrame_Purge for single skins...
		skyboxskinframe[i] = nullptr;
	}
	if (unlikely(developer_loading.integer != 0) && c != 0)
		Con_Printf("unloading skybox\n");
}

static int R_LoadSkyBox()
{
	int i, j, success;
	...
	for (j=0; j<3; j++)
	{
		...
		for (i=0; i<6; i++)
		{
			if (dpsnprintf(name, sizeof(name), "%s_%s", skyname, suffix[j][i].suffix) < 0 || !(image_buffer = loadimagepixelsbgra(name, false, false, false, nullptr)))
			{
				if (dpsnprintf(name, sizeof(name), "%s%s", skyname, suffix[j][i].suffix) < 0 || !(image_buffer = loadimagepixelsbgra(name, false, false, false, nullptr)))
				{
					if (dpsnprintf(name, sizeof(name), "env/%s%s", skyname, suffix[j][i].suffix) < 0 || !(image_buffer = loadimagepixelsbgra(name, false, false, false, nullptr)))
					{
						if (dpsnprintf(name, sizeof(name), "gfx/env/%s%s", skyname, suffix[j][i].suffix) < 0 || !(image_buffer = loadimagepixelsbgra(name, false, false, false, nullptr)))
							continue;
					}
				}
			}
...
static void R_SkyBox()
{
	RSurf_ActiveCustomEntity(&skymatrix, &skyinversematrix, 0, 0, 1, 1, 1, 1, 6*4, skyboxvertex3f, skyboxtexcoord2f, nullptr, nullptr, nullptr, nullptr, 6*2, skyboxelement3i, skyboxelement3s, false, false);
	for (size_t i = 0; i < 6; i++)
	{
		if(skyboxskinframe[i])
		{
			R_DrawCustomSurface(skyboxskinframe[i], reinterpret_cast<const matrix4x4_t*>(&identitymatrix),
			MATERIALFLAG_SKY | MATERIALFLAG_FULLBRIGHT | MATERIALFLAG_NOCULLFACE | MATERIALFLAG_NODEPTHTEST, i*4, 4, i*2, 2, false, false);
		}
	}
}
...
static void skyspherecalc()
{
	...
	float *vertex3f		= skysphere_vertex3f;
	float *texcoord2f	= skysphere_texcoord2f;
	...
	for (j = 0;j <= skygridy;j++)
	{
		...
		for (i = 0;i <= skygridx;i++)
		{
			...
			length = 3.0f / sqrtf(v[0]*v[0]+v[1]*v[1]+(v[2]*v[2]*9));
	...
	{
		unsigned short *RESTRICT e = skysphere_element3s;
		for (j = 0; j < skygridy;j++)
		{
			for (i = 0; i < skygridx; i++)
			{
...
static void R_SkySphere()
{
	...
	RSurf_ActiveCustomEntity(&skymatrix, &skyinversematrix, 0, 0, 1, 1, 1, 1, skysphere_numverts, skysphere_vertex3f, skysphere_texcoord2f, nullptr, nullptr, nullptr, nullptr, skysphere_numtriangles, skysphere_element3i, skysphere_element3s, false, false);
...
static void r_sky_start()
{
	skytexturepool = R_AllocTexturePool();
	R_LoadSkyBox();
}

static void r_sky_shutdown()
{
	R_UnloadSkyBox();
	R_FreeTexturePool(&skytexturepool);
}

static void r_sky_newmap()	{}
...
void R_Sky_Init()
{
	...
	skyname[0] = 0;
	R_RegisterModule("R_Sky", r_sky_start, r_sky_shutdown, r_sky_newmap, nullptr, nullptr);
}
