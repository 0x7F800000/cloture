...
/*
	...
	struct packedString : __markAsCtfe()
	{
		...
		{
			using pack = templates::variadic::paramPack<T, packedVals...>;
			...
			static constexpr size_t packedStrlen()
			{
				...
				while(true)
				{
					T indexed = pack::arr[ toPackedIndex(i)];
			...
			{
				T val = pack::arr[ toPackedIndex(index)];
	...
	#define mConstString16(str, i) \
		...
		#define mConstString16(str, i) 								\
			...
			cloture::util::ctfe::packString<unsigned __int128>(		\
	...
	cloture::util::ctfe::packedString<unsigned __int128, \
