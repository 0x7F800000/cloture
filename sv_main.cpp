...
void SV_Init ()
{
	...
	Cmd_AddCommand_WithClientCommand("sv_startdownload", nullptr, SV_StartDownload_f, "begins sending a file to the client (network protocol use only)");
	Cmd_AddCommand_WithClientCommand("download", nullptr, SV_Download_f, "downloads a specified file from the server");
	...
	sv_mempool = Mem_AllocPool("server", 0, nullptr);
}
...
void SV_StartParticle (vec3_t org, vec3_t dir, int color, int count)
{
	...
	for( size_t i = 0; i < 3; i++ )
		MSG_WriteChar (&sv.datagram, (int)bound(-128, dir[i]*16.0f, 127));
...
void SV_SendServerinfo (client_t *client)
{
	...
	if (sv.csqc_progname[0])
	{
		...
		if(client->sv_demo_file != nullptr)
		{
			int i;
...
static bool SV_PrepareEntityForSending (prvm_edict_t *ent, entity_state_t *cs, int enumber)
{
	prvm_prog_t *prog = SVVM_prog;
	int i;
	...
	*cs = defaultstate;
	...
	{
		i = (int)(v[0] * 32.0f);
		cs->colormod[0] = bound(0, i, 255);
		i = (int)(v[1] * 32.0f);
		cs->colormod[1] = bound(0, i, 255);
		i = (int)(v[2] * 32.0f);
		cs->colormod[2] = bound(0, i, 255);
	}
	...
	{
		i = (int)(v[0] * 32.0f);
		cs->glowmod[0] = bound(0, i, 255);
		i = (int)(v[1] * 32.0f);
		cs->glowmod[1] = bound(0, i, 255);
		i = (int)(v[2] * 32.0f);
		cs->glowmod[2] = bound(0, i, 255);
	}
...
static void SV_PrepareEntitiesForSending()
{
	prvm_prog_t *prog = SVVM_prog;
	int e;
	prvm_edict_t *ent;
	...
	sv.sendentitiesindex[0] = nullptr;
...
bool SV_CanSeeBox(int numtraces, vec_t enlarge, vec3_t eye, vec3_t entboxmins, vec3_t entboxmaxs)
{
	prvm_prog_t *prog = SVVM_prog;
	float pitchsign;
	float alpha;
...
static void SV_MarkWriteEntityStateToClient(entity_state_t *s)
{
	...
	if (s->number != sv.writeentitiestoclient_cliententitynumber)
	{
		...
		isbmodel = (model = SV_GetModelByIndex(s->modelindex)) != nullptr && model->name[0] == '*';
...
static void Download_CheckExtensions()
{
	int i;
	int argc = Cmd_Argc();
...
static void SV_Download_f()
{
	...
	bool is_csqc; // so we need to check only once
	...
	if (host_client->download_file)
	{
		...
		host_client->download_file = nullptr;
		...
		if (!sv.model_precache[i][0])
		{
			if (precachemode)
			{
				...
				if (sv.state == ss_loading)
				{
					...
					sv.models[i] = Mod_ForName (sv.model_precache[i], true, false, s[0] == '*' ? sv.worldname : nullptr);
				}
				else
				{
					if (svs.threaded)
					{
						...
						sv.models[i] = Mod_FindName (sv.model_precache[i], s[0] == '*' ? sv.worldname : nullptr);
					}
					else
					{
...
{
	return (modelindex > 0 && modelindex < MAX_MODELS) ? sv.models[modelindex] : nullptr;
}
...
static void SV_Prepare_CSQC()
{
	...
	svs.csqc_progdata = nullptr;
	svs.csqc_progdata_deflated = nullptr;
...
{
	prvm_prog_t *RESTRICT const prog = SVVM_prog;

	svs.serverflags = static_cast<int>(PRVM_serverglobalfloat(serverflags));
	int i;
	for (i = 0, host_client = svs.clients;i < svs.maxclients;i++, host_client++)
	{
		if (!host_client->active)
	...
	// call the progs to get default spawn parms for the new client
		...
		for ( size_t j = 0 ; j < NUM_SPAWN_PARMS; j++ )
...
void SV_SpawnServer (const char *server)
{
	...
	worldmodel = Mod_ForName(modelname, false, developer.integer > 0, nullptr);
	if (unlikely(!worldmodel) || !worldmodel->TraceBox)
	{
...
// serverflags are for cross level information (sigils)
	...
	if (sv_entpatch.integer && (entities = (char *)FS_LoadFile(va(vabuf, sizeof(vabuf), "%s.ent", sv.worldnamenoextension), tempmempool, true, nullptr)))
	{
...
static void SVVM_init_edict(prvm_prog_t *prog, prvm_edict_t *e)
{
	...
	if (num >= 0 && num < svs.maxclients)
	{
		...
		if(svs.clients[num].netconnection != nullptr)
		{
		...
		if(svs.clients[num].netconnection != nullptr && svs.clients[num].netconnection->crypto.authenticated && svs.clients[num].netconnection->crypto.client_idfp[0])
		...
		if(svs.clients[num].netconnection != nullptr && svs.clients[num].netconnection->crypto.authenticated && svs.clients[num].netconnection->crypto.client_keyfp[0])
		...
		if(svs.clients[num].netconnection != nullptr && svs.clients[num].netconnection->crypto.authenticated && svs.clients[num].netconnection->crypto.server_keyfp[0])
		...
		if(svs.clients[num].netconnection != nullptr && svs.clients[num].netconnection->crypto.authenticated && svs.clients[num].netconnection->crypto.use_aes)
		...
		if(svs.clients[num].netconnection != nullptr && svs.clients[num].netconnection->crypto.authenticated)
...
static bool SVVM_load_edict(prvm_prog_t *prog, prvm_edict_t *ent)
{
...
void SV_StartThread()
{
	...
	svs.thread = Thread_CreateThread(SV_ThreadFunc, nullptr);
}
