...
namespace cloture::vm
{
	...
	union getStringErrorParam
	{
		...
		constexpr getStringErrorParam(getStringErr e) : err(e)
		{}
	};
	static const char* nullVMString = "";
	[[gnu::cold]]
	__noinline const char* getStringError(Program prog, int num, register const getStringErrorParam err)
	{
		switch(err.err)
		{
		case negativeOffset:
			VM_Warning(prog.getPtr(), "vm::GetString: Invalid string offset (%i < 0)\n", num);
			break;
		case invalidTempOffset:
			VM_Warning(prog.getPtr(), "vm::GetString: Invalid temp-string offset (%i >= %i prog->tempstringsbuf.cursize)\n", num, prog->tempstringsbuf.cursize);
			break;
		case invalidZoneOffsetFreed:
			VM_Warning(prog.getPtr(), "vm::GetString: Invalid zone-string offset (%i has been freed)\n", num);
			break;
		case invalidZoneOffset:
			VM_Warning(prog.getPtr(), "vm::GetString: Invalid zone-string offset (%i >= %i)\n", num, prog->numknownstrings);
			break;
		case invalidConstantOffset:
			VM_Warning(prog.getPtr(), "vm::GetString: Invalid constant-string offset (%i >= %i prog->stringssize)\n", num, prog->stringssize);
			break;
		};
		return nullVMString;
	}
...
=================
*/
void vm::sprint(Program prog, const ptrdiff_t clientnum)
{
...
=========
*/
bool vm::localsound(Program prog, const char* s)
{
...
void vm::remove(Program prog, Edict* e)
{
	if( unlikely(PRVM_NUM_FOR_EDICT(e) <= prog->reserved_edicts) )
	{
		if (unlikely(developer.integer > 0))
			VM_Warning(prog, funcName() ": tried to remove the null entity or a reserved entity!\n" );
	}
	else if( unlikely( ed->priv.required->free ))
	{
		if (unlikely(developer.integer > 0)))
			VM_Warning(prog, funcName() ": tried to remove an already freed entity!\n" );
	}
	else
...
Edict* vm::find(Program prog, Edict* start, const char* field, const char* match )
{
	...
	{
		uint32 e =
				static_cast<uint32>(start - localEdicts) / static_cast<uint32>(sizeof(Edict));
...
void VM_findfloat(Program prog)
{
	int		e;
	int		f;
	float	s;
...
void VM_findchainflags(Program prog)
{
	prvm_int_t		i;
	prvm_int_t		f;
	prvm_int_t		s;
...
=========
*/
void VM_precache_sound(Program prog)
{
	const char *s;
...
=================
*/
void VM_precache_file(Program prog)
{
...
=========
*/
void VM_stackdump(Program prog)
{
...
=========
*/
void VM_eprint(Program prog)
{
	VM_SAFEPARMCOUNT(1,VM_eprint);

	PRVM_ED_PrintNum (prog, PRVM_G_EDICTNUM(OFS_PARM0), nullptr);
}
...
==============
*/
void VM_changelevel(Program prog)
{
	char vabuf[1024];
...
=================
*/
void VM_randomvec(Program prog)
{
	vec3_t temp;
...
static qfile_t *VM_GetFileHandle(Program prog, int index)
{
	...
	if (prog->openfiles[index] == nullptr)
	{
...
void VM_fopen(Program prog)
{
	...
	for (filenum = 0;filenum < PRVM_MAX_OPENFILES;filenum++)
		if (prog->openfiles[filenum] == nullptr)
			break;
...
void VM_fclose(Program prog)
{
	...
	prog->openfiles[filenum] = nullptr;
...
=========
*/
void VM_writetofile(Program prog)
{
	prvm_edict_t * ent;
	qfile_t *file;
...
void VM_substring(Program prog)
{
	int start, length;
	...
	*/
	...
	u_start = u8_byteofs(s, start, nullptr);
	if (u_start < 0)
	{
		PRVM_G_INT(OFS_RETURN) = PRVM_SetTempString(prog, "");
		return;
...
float	getostype(Program prog)
=========
*/ // not used at the moment -> not included in the common list
void VM_getostype(Program prog)
{
...
=========
*/
void VM_loadfromfile(Program prog)
{
	const char *filename;
	char *data;
	...
	data = (char *)FS_LoadFile(filename, tempmempool, false, nullptr);
	if (data == nullptr)
		PRVM_G_FLOAT(OFS_RETURN) = -1;
...
static void VM_Search_Init(Program prog)
{
	int i;
	for (i = 0;i < PRVM_MAX_OPENSEARCHES;i++)
		prog->opensearches[i] = nullptr;
}

static void VM_Search_Reset(Program prog)
{
	int i;
...
=========
*/
void VM_search_begin(Program prog)
{
	int handle;
	const char *pattern;
...
=========
*/
void VM_search_end(Program prog)
{
	int handle;
	...
	if(prog->opensearches[handle] == nullptr)
	{
	...
	prog->opensearches[handle] = nullptr;
...
=========
*/
void VM_search_getsize(Program prog)
{
	int handle;
...
=========
*/
void VM_search_getfilename(Program prog)
{
	int handle, filenum;
...
static void getdrawfontscale(Program prog, float *sx, float *sy)
{
	vec3_t v;
	*sx = *sy = 1;
...
========================
*/
void VM_altstr_count(Program prog)
{
...
========================
*/
void VM_altstr_prepare(Program prog)
{
	char *out;
	const char *instr, *in;
	int size;
...
========================
*/
void VM_altstr_get(Program prog)
{
	const char *altstr, *pos;
	char *out;
	int count, size;
...
========================
*/
void VM_altstr_set(Program prog)
{
    int num;
...
========================
*/
void VM_altstr_ins(Program prog)
{
	int num;
	const char *set;
	const char *in;
...
static void BufStr_Expand(Program prog, prvm_stringbuffer_t *stringbuffer, int strindex)
{
...
static void BufStr_Shrink(Program prog, prvm_stringbuffer_t *stringbuffer)
{
	// reduce num_strings if there are empty string slots at the end
	while (stringbuffer->num_strings > 0 && stringbuffer->strings[stringbuffer->num_strings - 1] == nullptr)
...
void BufStr_Set(Program prog, prvm_stringbuffer_t *stringbuffer, int strindex, const char *str)
{
	...
	stringbuffer->strings[strindex] = nullptr;

	if (str)
	{
		// not the nullptr string!
...
float buf_create(Program prog) = #460;
float newbuf(string format, float flags) = #460;
========================
*/

void VM_buf_create (Program prog)
{
	prvm_stringbuffer_t *stringbuffer;
	int i;
...
========================
*/
void VM_buf_getsize (Program prog)
{
	prvm_stringbuffer_t *stringbuffer;
...
========================
*/
void VM_buf_implode (Program prog)
{
	prvm_stringbuffer_t *stringbuffer;
...
========================
*/
void VM_bufstr_get (Program prog)
{
	prvm_stringbuffer_t *stringbuffer;
	int				strindex;
...
========================
*/
void VM_bufstr_set (Program prog)
{
	int				strindex;
	prvm_stringbuffer_t *stringbuffer;
	const char		*news;
...
========================
*/
void VM_bufstr_add (Program prog)
{
	int				order, strindex;
	prvm_stringbuffer_t *stringbuffer;
	const char		*string;
	...
	if(!PRVM_G_INT(OFS_PARM1)) // nullptr string
	{
	...
	if(order)
		...
		for (strindex = 0;strindex < stringbuffer->num_strings;strindex++)
			if (stringbuffer->strings[strindex] == nullptr)
				break;
...
========================
*/
void VM_bufstr_free (Program prog)
{
	int				i;
	prvm_stringbuffer_t	*stringbuffer;
	...
	if (i < stringbuffer->num_strings)
	{
		if(stringbuffer->strings[i])
			Mem_Free(stringbuffer->strings[i]);
		stringbuffer->strings[i] = nullptr;
	}
...
========================
*/
void VM_buf_loadfile(Program prog)
{
	size_t alloclen;
	prvm_stringbuffer_t *stringbuffer;
	char string[VM_STRINGTEMP_LENGTH];
...
void VM_buf_writefile(Program prog)
{
...
// todo: support UTF8
static bool match_rule(const char *string, int max_string, const char *pattern, int patternlength, int rule)
{
	const char *mid;
...
void VM_bufstr_find(Program prog)
{
	prvm_stringbuffer_t *stringbuffer;
	char string[VM_STRINGTEMP_LENGTH];
...
========================
*/
void VM_matchpattern(Program prog)
{
	const char *s, *match;
	char string[VM_STRINGTEMP_LENGTH];
	int matchrule, l;
...
void VM_buf_cvarlist(Program prog)
{
	CVar *cvar;
	const char *partial, *antipartial;
	size_t len, antilen;
	size_t alloclen;
	bool ispattern, antiispattern;
	int n;
	prvm_stringbuffer_t	*stringbuffer;
...
void VM_strstrofs (Program prog)
{
...
void VM_strncasecmp (Program prog)
{
	const char *s1, *s2;
					...
					switch(*s)
					{
						...
						case 'c':
							...
							else
							{
								...
								char charbuf16[16];
								const char *buf = u8_encodech(c, nullptr, charbuf16);
								if(!buf)
									buf = "";
								if(precision < 0) // not set
									precision = end - o - 1;
...
static void animatemodel(Program prog, dp_model_t *model, prvm_edict_t *ed)
{
	skeleton_t *skeleton;
	int skeletonindex = -1;
	bool need = false;
...
static void getmatrix(Program prog, prvm_edict_t *ed, matrix4x4_t *out)
{
	if (prog == SVVM_prog)
...
static void applytransform_forward(Program prog, const vec3_t in, prvm_edict_t *ed, vec3_t out)
{
	matrix4x4_t m;
	getmatrix(prog, ed, &m);
...
static void applytransform_forward_direction(Program prog, const vec3_t in, prvm_edict_t *ed, vec3_t out)
{
	matrix4x4_t m;
	getmatrix(prog, ed, &m);
...
static void applytransform_inverted(Program prog, const vec3_t in, prvm_edict_t *ed, vec3_t out)
{
	matrix4x4_t m, n;
	getmatrix(prog, ed, &m);
...
static void applytransform_forward_normal(Program prog, const vec3_t in, prvm_edict_t *ed, vec3_t out)
{
	matrix4x4_t m;
	float p[4];
	getmatrix(prog, ed, &m);
...
static void clippointtosurface(Program prog, prvm_edict_t *ed, dp_model_t *model, msurface_t *surface, vec3_t p, vec3_t out)
{
	int i, j, k;
