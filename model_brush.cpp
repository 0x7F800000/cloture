...
static bool Mod_Q3BSP_TraceLineOfSight(struct model_s *model, const vec3_t start, const vec3_t end);
...
using findnonsolidlocationinfo_t = findnonsolidlocationinfo_s;
...
__align(16) struct RecursiveHullCheckTraceInfo_s
{
...
using RecursiveHullCheckTraceInfo_t = RecursiveHullCheckTraceInfo_s;
...
static bool Mod_Q1BSP_TraceLineOfSight(struct model_s *model, const vec3_t start, const vec3_t end)
{
	trace_t trace;
...
static void Mod_Q1BSP_LoadFaces(sizebuf_t *sb)
{
	...
	//int i, j;
	size_t i, j;
	int count, surfacenum, planenum, smax, tmax, ssize, tsize, firstedge, numedges, totalverts, totaltris, lightmapnumber, lightmapsize, totallightmapsamples, lightmapoffset, texinfoindex;
...
static void AddPortalToNodes(portal_t *p, mnode_t *front, mnode_t *back)
{
	...
	if (unlikely(back == nullptr))
		Host_Error("AddPortalToNodes: NULL back node");
	if (unlikely(p->nodes[0] || p->nodes[1]))
...
bool Mod_CollisionBIH_TraceLineOfSight(struct model_s *model, const vec3_t start, const vec3_t end)
{
	trace_t trace;
