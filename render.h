...
void R_Init();
void R_UpdateVariables(); // must call after setting up most of r_refdef, but before calling R_RenderView
void R_RenderView(); // must set r_refdef and call R_UpdateVariables first
void R_RenderView_UpdateViewVectors(); // just updates r_refdef.view.{forward,left,up,origin,right,inverse_matrix}
...
typedef struct rsurfacestate_s
{
	...
	bool                    forcecurrenttextureupdate; // set for RSurf_ActiveCustomEntity to force R_GetCurrentTexture to recalculate the texture parameters (such as entity alpha)
	bool                    modelgeneratedvertex;
	...
	float modellight_ambient[3];
	float modellight_diffuse[3];
	float modellight_lightdir[3];
	// colormapping state from entity (these are black if colormapping is off)
	float colormap_pantscolor[3];
	float colormap_shirtcolor[3];
...
void R_SetupShader_Surface(const vec3_t lightcolorbase, bool modellighting, float ambientscale, float diffusescale, float specularscale, rsurfacepass_t rsurfacepass, int texturenumsurfaces, const msurface_t **texturesurfacelist, void *waterplane, bool notrippy);
...
typedef struct r_waterstate_s
{
	...
	bool renderingscene; // true while rendering a refraction or reflection texture, disables water surfaces
	bool hideplayer;
}
r_waterstate_t;
...
typedef struct r_framebufferstate_s
{
	...
	bool ghosttexture_valid; // don't draw garbage on first frame with motionblur
	bool usedepthtextures; // use depth texture instead of depth renderbuffer (faster if you need to read it later anyway)
}
...
dp_font_t *FindFont(const char *title, bool allocate_new);
void LoadFont(bool override, const char *name, dp_font_t *fnt, float scale, float voffset);

void Render_Init();

// these are called by Render_Init
void R_Textures_Init();
void GL_Draw_Init();
void GL_Main_Init();
void R_Shadow_Init();
void R_Sky_Init();
