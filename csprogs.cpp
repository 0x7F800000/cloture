...
{
	prvm_prog_t *const prog = CLVM_prog;
	if(!cl.csqc_loaded)
		return;
	CSQC_BEGIN
...
bool CSQC_AddRenderEdict(prvm_edict_t *ed, int edictnum)
{
	prvm_prog_t *prog = CLVM_prog;
	int renderflags;
	int c;
	...
	dp_model_t *model = CL_GetModelFromEdict(ed);
	if (model == nullptr)
		return false;
	...
	else if (c <= cl.maxclients && cl.scores != nullptr)
...
bool CL_VM_InputEvent (int eventtype, float x, float y)
{
	prvm_prog_t *prog = CLVM_prog;
	bool r;
...
bool CL_VM_ConsoleCommand (const char *cmd)
{
	prvm_prog_t *prog = CLVM_prog;
...
void CL_VM_Parse_StuffCmd (const char *msg)
{
	...
	if(msg[3] == 'c')
	{
		...
		const int crcflags = csqc_progcrc.flags;
		const int sizeflags = csqc_progcrc.flags;
...
bool CL_VM_Event_Sound(int sound_num, float volume, int channel, float attenuation, int ent, vec3_t pos, int flags, float speed)
{
	prvm_prog_t *prog = CLVM_prog;
	bool r = false;
	if(!cl.csqc_loaded)
...
static bool CLVM_load_edict(prvm_prog_t *prog, prvm_edict_t *ent)
{
	return true;
}
...
bool MakeDownloadPacket(const char *filename, unsigned char *data, size_t len, int crc, int cnt, sizebuf_t *buf, int protocol)
{
...
bool CL_VM_GetEntitySoundOrigin(int entnum, vec3_t out)
{
	prvm_prog_t *prog = CLVM_prog;
	prvm_edict_t *ed;
	dp_model_t *mod;
...
bool CL_VM_TransformView(int entnum, matrix4x4_t *viewmatrix, mplane_t *clipplane, vec3_t visorigin)
{
	prvm_prog_t *prog = CLVM_prog;
	bool ret = false;
	prvm_edict_t *ed;
