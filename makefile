...
#$(shell $(TARGET_GCC) -print-file-name=plugin)
LIB_ODE:=$(shell ode-config --libs)
CFLAGS_ODE:=$(shell ode-config --cflags) -DUSEODE -DLINK_TO_LIBODE
#$(shell $(ODE_CONFIG) --cflags)
#`$(ODE_CONFIG) --libs`
#CFLAGS_ODE=$(CFLAGS_ODE) -DUSEODE -DLINK_TO_LIBODE
#endif
...
CFLAGS_MAKEDEP?=-x c++ -ggdb -mmmx -msse3 -msse4a -m3dnow $(CLOTURE_OPT) -DCONFIG_MENU -DCONFIG_CD $(EXTRA_DEFINES) $(EXTRA_INCLUDES) -MMD -fomit-frame-pointer -fms-extensions $(CXXSTANDARD) -Wfatal-errors -w -fpermissive
CFLAGS_NOSYSHEADERS=
ifeq ($(CXX), icc)
